...
/**
 * LA ROTELLA SULL'ODOMETRO, misurata con un evento vero: la cifra si muove, la pagina NON scorre.
 *
 * Due incognite in un gesto solo, quindi due asserzioni: `preventDefault` su un listener `wheel` di un
 * ELEMENTO morde davvero (su window e document no), e la colonna sotto il puntatore e' quella che si
 * muove - una rotella che spostasse sempre l'unita' renderebbe il gesto inutile su un odometro.
 */
async function main() {
  const binary = BROWSERS.find((one) => existsSync(one));
  const { server, port } = await serve(DIST);
  const profile = await mkdtemp(join(tmpdir(), 'fant-wheel-'));
  ...
  try {
    ...
    await waitFor(session, () => document.querySelectorAll('[data-column]').length, 120);

    const before = await evaluate(session, () => {
      const cols = [...document.querySelectorAll('[data-column]')];
      const box = cols.map((c) => c.getBoundingClientRect());
      return {
        columns: cols.length,
        digits: cols.map((c) => c.querySelector('[data-digit]')?.value ?? '?'),
        // la SECONDA colonna da sinistra su quattro: le centinaia, cioe' il posto che rende
        // il gesto utile - se muovesse l'unita' non servirebbe a niente
        at: box[1] ? { x: box[1].left + box[1].width / 2, y: box[1].top + box[1].height / 2 } : null,
        width: box[0] ? Math.round(box[0].width) : 0,
        height: box[0] ? Math.round(box[0].height) : 0,
        scrollY: window.scrollY,
      };
    });
    console.log(`. l'odometro: ${before.columns} colonne da ${before.width}x${before.height}px · cifre «${before.digits.join('')}»`);
    if (!before.at) throw new Error('nessuna colonna da rotolare');

    for (const [label, delta, want] of [['su', -120, 1], ['giu', 120, -1]]) {
      const start = await evaluate(session, () => ({
        digits: [...document.querySelectorAll('[data-digit]')].map((d) => d.value).join(''),
        scrollY: window.scrollY,
      }));
      await session.send('Input.dispatchMouseEvent', {
        type: 'mouseWheel', x: before.at.x, y: before.at.y, deltaX: 0, deltaY: delta,
      });
      await wait(300);
      ...
      const moved = Number(after.digits) - Number(start.digits);
      console.log(`. rotella ${label}: «${start.digits}» -> «${after.digits}» (${moved >= 0 ? '+' : ''}${moved})`);
      if (moved !== want * 100) {
        problems.push(`rotella ${label}: la cifra si muove di ${moved} invece di ${want * 100} (la colonna sotto il puntatore e' quella delle centinaia)`);
      }
      if (after.scrollY !== start.scrollY) {
        problems.push(`rotella ${label}: la pagina ha scrollato (${start.scrollY} -> ${after.scrollY}), quindi preventDefault non morde`);
      }
    }
  } finally {
