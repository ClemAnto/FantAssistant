...
**Aggiornato: 22 settembre 2026 — IL PRIMO TURNO MISURATO HA DETTO CHE IL METRO ERA SBAGLIATO, E LA FOTOGRAFIA SI PRENDE PRIMA CHE IL TURNO COMINCI.** Il turno 5 leggeva fantacalcio **100,0%** degli undici veri, sosfanta 99,1%, Sky 92,3% su un null di 80,9%: con la presa a quindici minuti dal fischio **di ogni club** un club della domenica veniva fotografato la domenica, a formazioni ufficiali gia' uscite — e il log del foglio lo diceva con parole sue dieci volte («100% of the men are given at 100%: this reading may be the OFFICIAL line-up rather than a forecast»): *la guardia aveva funzionato, nessuno l'aveva letta.* **La regola nuova e' sua ed e' la sua richiesta originale**: «il meccanismo di lettura deve avvenire solo fino a quando la prima partita del turno non inizia» — la prima riga del 18/09 diceva gia' «subito prima dell'inizio del primo anticipo», e la precisazione della sera («15 minuti prima del fischio iniziale»), ambigua in italiano, era stata letta come il fischio di ogni partita. *Quando una frase nuova dell'operatore e' ambigua, la disambigua quella vecchia e non la comodita' di chi implementa.* **E il turno si e' potuto RIFARE**: `recover(round)` rigioca le fotografie su Drive (43, 90 giorni) — l'intestazione del file lo prometteva dal primo giorno e nessuna funzione lo faceva, che e' «una replica offline che nessuno chiama e' una cache che non esiste» — ed e' idempotente, **aggiunge senza togliere** (le letture tardive restano, lo scorer smette di leggerle: adottare la regola nuova non distrugge la misura vecchia) e legge l'apertura dal FOGLIO e non dal calendario di oggi. **Il risultato**: sosfanta 90,9% (+10,0), fantacalcio 89,1% (+8,2), Sky 89,1% (+8,2), Corriere 81,4% (+0,5) — e **il Corriere non si muove di un decimale**, perche' era l'unica fonte gia' misurata cosi', *che e' il controllo che vale piu' della tabella*. Il primo posto cambia mano e non e' quel risultato: appaiato per club sosfanta batte fantacalcio 6-3 con 11 pari (p = 0,51) e Sky 4-1 con 15 pari (p = 0,38), contro il Corriere **12-0 con 8 pari (p = 0,0005)**, e la distanza fra i primi tre e' piu' piccola del costo della nostra identita' — **tre fonti insieme, una staccata**; +10 punti sono **1,1 titolari per squadra** in piu' del ricopiare l'undici della giornata prima. **Quattro difetti curati**: un anticipo IGNOTO letto come «prima delle ufficiali» (33 letture su 80, perche' `Number('') < 30` e' falso e l'ammissibilita' recuperava il calcio d'inizio mentre il `lead` no — adesso **6 su 80 e zero ignote**); «TURNO 6 PERSO» in rosso che era il numero del turno confrontato con un calendario ancora fermo al 5 (la discriminante e' provabile: l'apertura mostrata e' un calcio d'inizio **gia' archiviato sotto un turno precedente**); l'intestazione del foglio vecchia di una colonna che mostrava `updated_utc: 9`; e `[object Object]` come ragione in 25 righe di Log su 36. **Verifica**: banco nuovo sulla scadenza, **controprova fatta** (rimettendo la regola vecchia cadono esattamente le due asserzioni che descrivono la cura), e la corsa vera dell'operatore — 1452 righe rimesse da due fotografie pre-apertura. `engine_*`, `SHEET_REVISION`, il pacchetto e l'app sono fermi. **Il rischio aperto**: se le pagine non pubblicano il calendario del turno 6 in tempo, il pianificatore non arma e li' `recover` NON salva — senza presa non c'e' la fotografia. Dettaglio: [attendibilita-probabili-v1.md](attendibilita-probabili-v1.md) §5.

· **precedente: 19 settembre 2026 — LE PROBABILI DELLA STAMPA HANNO UN FOGLIO CHE LE CATTURA, E LA SEZIONE SQUADRE UNA TERZA LETTURA.** Dalla sua richiesta: «crea uno script anche su google-sheet che... scansioni i siti principali che mostrano le probabili formazioni... poi alla fine del turno crei un report con i calciatori corretti e quelli sbagliati». `scripts/gas/probabili-sheet.gs` (Apps Script su un foglio Google, fuori dal repository) cattura **quattro fonti** a QUINDICI MINUTI dal fischio di ogni club - il numero e' suo, e «le formazioni ufficiali rientrano nel computo» - legge la verita' dalla pagina dei voti e misura turno dopo turno quale sito ci prende di piu'; la terza voce della sezione Squadre («Prossimo turno», `core/next-round.ts`) legge il suo endpoint DAL VIVO e disegna quello che le fonti hanno pubblicato. **E' una TERZA cosa e non un terzo orizzonte**: la stampa qui e' il GIUDICE delle nostre due board, quindi `BoardHorizon` non la contiene, il payload dichiara di se' cosa e' e nessun percorso del motore la rilegge. **Le sue decisioni**: la linea e' il FISCHIO; lettura diretta e non dal pacchetto (che sarebbe vecchio di un giorno proprio per le partite per cui serve, e che l'origine dell'app possa leggere quell'endpoint e' stato MISURATO prima di scrivere una riga di vista: 200, `type: cors`, 20 club, da gh-pages E da localhost); la lettura si tiene in `localStorage` e la **rete e' un tastino**; via la lista dove c'e' il campetto; **moduli REALI e non quelli del regolamento mantra**; e «utilizza i nostri algoritmi delle formazioni per piazzare le 11 scelte». **I numeri**: il secondo salto di Apps Script risponde **404 due volte su sei** (gettone monouso, fino a 27 secondi), quindi tre tentativi - e prima della cura il banco era verde **una corsa su due**, che e' il modo in cui un difetto del trasporto si fa scambiare per uno della vista; il campetto disegna **19 club su 20** (coi soli ruoli classic erano 12 e quattro undici veri non stavano in nessun modulo, coi posti tipati mantra 6). **Quattro regole che restano**: un matching MASSIMO risponde a «quanti» e non a «dove» (si prova prima la riga di CASA di ognuno, e la si tiene solo se piazza tanti uomini - un posto vuoto e' una bugia piu' grossa di un uomo fuori posto); i CODICI di un uomo vengono prima di una posizione presa altrove (`Pc` e' un'affermazione su di LUI, la `x` del nostro campetto e' su un ALTRO undici); una riga si dispone CERCANDO e non ordinando (`laneCost` risponde zero a chi non parla, e zero in un ordinamento crescente vuol dire «primo» invece di «dove capita»); un marcatore non puo' rivendicare un lato che non e' il suo. **E due errori di metodo**: il verdetto su SOS Fanta era sbagliato DUE volte - una fonte archiviata come rifiutata smette di essere misurata - e la prima controprova del banco era INERTE, cioe' un fixture che col difetto rimesso darebbe lo stesso risultato non prova niente. **Verifica**: build pulito, **1020 test** su 58 file, `e2e-next-round` verde su otto club (ogni numero contro il payload scaricato da Node, mai contro lo schermo), cache provata **bloccando `script.google.com` nel browser**. `engine_*`, `SHEET_REVISION` e il pacchetto sono fermi. Dettaglio: [attendibilita-probabili-v1.md](attendibilita-probabili-v1.md).

· **precedente: 16 settembre 2026 (notte) — UNA GIORNATA VOTATA A META' SI LEGGEVA COME COMPLETA, E LE EURO NON ERANO INDIETRO.** Due domande dell'operatore. La prima («c'e' qualcosa da allineare sulle leghe euro?») ha una risposta MISURATA e negativa: spaccando le tre prese pre-registrate del 12-13/09 per piattaforma, gli **11 club esteri leggono 80,0% degli uomini e 10 moduli su 11** contro il 74-79% e l'8 su 12 dei 12 club di Serie A, `club_match_lineups.formation` e' al **100% su tutti e cinque i campionati** (quindi nessuna cura di settembre e' arrivata a una piattaforma sola) e il foglio euro prezza PIU' righe di quello Serie A (`engine_pv_pred` 100% contro 70%). Quello che euro non ha e' un GIUDICE — `press_formations` porta 20 club, tutti di Serie A — e le pre-registrazioni sono l'unico pulito che abbia mai avuto. Chiusa la voce sul buco del Como (37 quotati, 37 sul foglio).
La seconda («non vedo le ultime partite della serie a») ha trovato il difetto: il resume di `ratings` chiedeva `SELECT DISTINCT matchday`, quindi **una riga bastava a dichiarare scaricata una giornata**, e i voti arrivano partita per partita — **Serie A md4 a 8 club su 20, euro md3 a 20 su 37**, con `update --daily` che i voti non li rilegge per definizione. `ratings.matchdays_done`: completa = almeno tanti CLUB quanti la MEDIANA delle ALTRE giornate di quella (stagione, piattaforma), costo misurato 5 giornate su 145. Recuperate **+1354 righe su Serie A e +1832 su euro**: cambiano `engine_pv_pred` su **245 righe di 953 (euro)** e **140 di 561 (Serie A)**, sempre verso l'alto (Kane 24,2 → 27,3), 41 gradini di titolarita' su euro, il surplus su 117 righe di Serie A — e i campetti **zero** su 37 club, perche' la board sceglie sui minuti e la scala sulle apparizioni. Commit `0248180`; dettaglio in spec «Novita' v9.97» e `formazioni-tipo-v1.md` §18.3-bis.
E QUATTRO ISTRUZIONI SULLA RIGA DELLA STRATEGIA, dell'altra sessione, tutte misurate in un browser vero (`pagina-strategia-v1.md` §21). **Le icone tornano dal nome**: erano già sue nel DOM e le spostava il layout - col nome `flex-1` la sua cella si prendeva lo spazio libero e i marchi finivano appiccicati alle pastiglie - quindi nome e marchi sono un gruppo solo e il `flex-1` passa al gruppo: **4px dall'ultima lettera**, misurati con un `Range` sul CONTENUTO perché il bordo della CELLA sarebbe stato «vicino» anche prima. **Il surplus diventa una pastiglia** che si accende e si spegne, con un interruttore SUO perché dentro `strategy.readings` «acceso» vuol dire ELENCATO e una preferenza salvata mesi fa avrebbe spento in silenzio la colonna che ordina. **I valori restano tutti in riga e cede il NOME**, decisione sua presa col prezzo davanti (mantra: riga da 38 a 24px e 182 nomi tagliati su 255; classic con sette pastiglie 249 su 250), quindi via il pavimento e via la regola del 04/09 sul contenitore stretto - e i due banchi che asserivano l'OPPOSTO sono stati rovesciati citando la sua frase, col prezzo che resta STAMPATO a ogni corsa. **E la banda ogni n righe c'era dal 04/09 e valeva DUE punti su 255** (`control` e `surface` distano otto punti per canale, il 25% ne lascia due): adesso è `bg-fg/5`, **11,1 punti misurati**, appoggiata all'inchiostro perché quello segue il tema da sé. **La sonda che l'ha misurata era rotta nello stesso modo del difetto che cercava** - leggeva 1,4 punti perché componeva un `oklab(0.23 …)` con un `rgb(20, 20, 28)`, una L fra 0 e 1 sommata a un canale fra 0 e 255. 990 test app, tre banchi della Strategia verdi sull'albero COMBINATO, `engine_*` e pacchetto fermi.
E una TERZA richiesta, una feature: sulla Strategia il **doppio click su un valore ordina per quello** e un secondo inverte, il nome compreso (`6eecbc2`, `pagina-strategia-v1.md` §20). La chiave esisteva già (`reading.id` E' quella del selettore), il verso ha un naturale per chiave, i vuoti restano in fondo in tutt'e due i versi, e la guardia sul click non è un di più: togliendola il banco passa da 0 problemi a 10, perchè la card aperta si prende il secondo click.
· **precedente: 16 settembre 2026 — LE TRE PRESE SI SCORANO, E IL GIUDICE PARLAVA UN'ALTRA LINGUA.** Dalla domanda dell'operatore «ci sono cose da chiudere?»: quattro, e la prima aveva una scadenza appena arrivata. (a) **IL PRIMO GIUDIZIO DELLA BOARD BREVE CONTRO IL FATTO.** Le tre pre-registrazioni del 12-13/09 (rev 63, 64, 65) pronosticavano le partite del 13-14 settembre: sono la 4a giornata, le distinte sono in archivio per tutti e cinque i campionati, e `press --score-preregistration` le ha scorate **tutte e tre** come la §12.9.5 imponeva. Uomini **195, 194 e 172** contro un null («chi ha cominciato l'ULTIMA partita», che non e' debole: sulle probabili dell'11/09 ci batteva 180/220) di **189, 189 e 161**; moduli 18, 18 e 16 contro 16, 16 e 14. **La meta' scomoda e' il test dei segni**: la presa che si spedisce fa meglio in 8 club, peggio in 2, pari in 10 — **p = 0,109**, la direzione giusta e non la prova — e le prime due sono un pareggio secco (6-6, 6-5) pur vincendo l'aggregato, perche' il margine viene da tre code (Villarreal +4, Lipsia +2, Napoli +2). *Un null si legge anche PER UNITA'.* Il confronto fra le due versioni del codice resta **non decidibile** (195 contro 194), esattamente come la presa aveva scritto prima di conoscere l'esito: averlo dichiarato prima e' cio' che impedisce di leggere quel −1 come un verdetto. **Niente si ricalcola**: `--against round` ridisegna le board col pannello di oggi (il 15/09 si e' misurato che vale 154 → 152 sulla stessa cartella), mentre una pre-registrazione si scora **come e' scritta** — quello che resta condiviso e' il METRO (`compare`, i nomi, l'unione per identita'), perche' un secondo conteggio di «quanti uomini in comune» darebbe due risposte a una domanda sola. E **l'ORA dei calci d'inizio era su disco** (`positions.kickoff_times`, dal payload dei turni): il limite che la terza presa dichiarava si e' applicato ESATTAMENTE, tre club scartati (Lecce, Monza, Brighton delle 15:00) invece di buttare via le diciotto partite del 13/09. (b) **IL GIUDICE DELLA BOARD DI STAGIONE PARLAVA IL VOCABOLARIO SBAGLIATO**, ed e' il difetto grosso della giornata, trovato preparando lo scoring: `round_reference` costruiva il modulo dai TRE CONTEGGI mentre dal 12/09 la board legge il modulo **dichiarato** dal club, quindi ogni 4-2-3-1 vero era confrontato con un «4-5-1» costruito da noi e letto come previsione sbagliata. A/B a una variabile (stessa cartella, stesso pannello, Serie A 4a giornata): **7 MATCH / 13 DIFF → 15 / 5**, con gli uomini **identici** (161/220) — otto club di venti erano vocabolario. `declared_or_counted` prende il **quarto lettore** e `compare` un `on="reference"` che sceglie la rappresentazione **riga per riga**, perche' il dichiarato e' su file solo dopo l'11/09 e l'archivio dietro non e' riletto. Dichiarato: i verdetti sui MODULI del 24/08 non sono confrontabili con uno preso adesso, quelli sugli uomini si'. (c) **I DUE BANCHI ROSSI.** `e2e-board-horizon` cercava la didascalia che l'operatore aveva fatto togliere il 15/09 — cioe' **misurava il proprio ricordo**: il compito di dichiarare l'orizzonte e' passato al pulsante e adesso il banco chiede a lui (con la controprova che morde). `e2e-player-card` e' **verde** con un build fresco e il bundle di stamattina, e l'attribuzione della differenza e' dichiarata come ipotesi invece che affermata, perche' i dati di ieri non ci sono piu'. (d) **LA CADENZA DI CLUBELO ERA QUELLA DI UNA FONTE MORTA**: `fetch --stale` chiedeva un giorno, che e' la cadenza dell'API (502 da gennaio), mentre a rispondere e' l'archivio adottato l'11/09 — che pubblica **il 1º e il 15 di ogni mese**, misurato sulla coda del file e non dedotto dalla descrizione. Da 1 a 17 giorni, e con lei e' caduto un commento in `elo.py` («l'archivio arriva a ieri») falso e in contraddizione con quello della costante cento righe sopra. *Un allarme che suona tutti i giorni non e' un allarme.* **Verifica**: 902 test toolkit verdi (+7 nuovi, ognuno con la controprova che rimette il difetto), build app pulito, i due banchi verdi, `--against round` rieseguito sul foglio vero. **`engine_*`, `SHEET_REVISION` e il pacchetto sono fermi**: e' tutto giudizio e lettura. Dettaglio: `formazioni-tipo-v1.md` §18, spec «Novita' v9.94». **E IL POMERIGGIO HA SMENTITO UN NUMERO DELLA MATTINA.** Andando avanti sugli aperti: (f) **il livello di quel 15/20 era CONTAMINATO** - `--against round` giudica la board che il foglio di OGGI disegna, e quella board legge le giornate giocate, quindi sulla 4a giornata stava leggendo in parte una copia. Misurato contro il foglio AUTENTICO del 10/09, stessa giornata e stesso pannello (una variabile sola, e per una volta e' il ridisegno a isolarla): **6 MATCH di 20 e 144/220 contro 15 e 161/220**. L'A/B fra i due metri resta valido - stessa board, uomini identici - il LIVELLO no, e la correzione sta accanto alla tabella invece di sostituirla. Il progetto l'aveva scritto il 04/09 su un'altra colonna e non l'aveva esteso a questa. (g) **IL BRACCIO PULITO E' STATO QUASI DISTRUTTO PER FABBRICARLO**: `snapshot --date 2026-09-10` stava per sovrascrivere la cartella che il 10 settembre aveva gia' lasciato, fermata coi byte intatti - *prima di generare un artefatto datato si guarda se quel giorno non ne abbia gia' lasciato uno*, che oltretutto e' evidenza migliore di un back-dated. (h) **E DALL'ERRORE E' USCITO IL RISULTATO DELLA GIORNATA: i DUE ORIZZONTI a parita' di conoscenza**, sugli stessi 10 club e la stessa giornata - board BREVE pre-registrata **7 moduli su 10 e 87/110 uomini**, board di STAGIONE con le stesse tre giornate viste **2 e 73/110** (8 club meglio, nessuno peggio, **p = 0,0078**), e la breve batte anche quella che di giornate ne ha viste QUATTRO (+10 uomini). Primo confronto diretto fra i due orizzonti sulla domanda per cui il corto e' nato: non esisteva perche' mancava il giudice del corto. (i) **IL PRENDITORE** (`press --take-preregistration`), nello stesso file del lettore e con un test di round-trip, e **un turno e' quello che la fonte NUMERA**: due forme dedotte scritte e buttate - «primo giorno futuro piu' quattro» tagliava la domenica a tutte le leghe (la Liga gioca 16-20, le altre 18-20), «giorni contigui» non ha una soglia difendibile (gli scarti fra giornate consecutive valgono 1 giorno 124 volte, 2 sette, 3 sette, 4-6 quarantasei, 7 novanta). Validata sul caso storico: «dal 10/09» restituisce 11-14 settembre, la finestra che le prese avevano scelto a mano. **Presa fatta**: 47 club, turno del 17-20, da scorare dopo il 20. **906 test toolkit verdi.** (j) **E LA SERA, UNA REGOLA DELL'OPERATORE SULLE DATE**: «con le date lavoriamo sempre in UTC, solo la visualizzazione deve essere in orario locale». Misurarla prima di eseguirla ha CAPOVOLTO il lavoro: l'aperto diceva «`realToday` e' UTC, fra mezzanotte e le due oggi e' ieri» e la conclusione implicita (metterla locale) era sbagliata, perche' il toolkit data gia' in UTC e l'app legge in UTC - spostare l'app avrebbe CREATO la discrepanza. Il lavoro vero era l'opposto: **sei righe del toolkit rimaste sull'orologio locale** (fra cui `calendar.observed_on` e `injuries.observed_on`, che viaggiano nel bundle e che l'app confronta col proprio `realToday`), una scritta da me quella mattina. Con loro: **le due meta' di una sottrazione stanno sullo stesso orologio** (in `injuries` erano gia' d'accordo perche' locali tutt'e due, e spostarne una sola l'avrebbe rotta) e **la guardia e' un test sul SORGENTE** (`tests/test_one_clock.py`), perche' due orologi divergono solo nelle due ore dopo la mezzanotte - un difetto che non si manifesta mai davanti a chi potrebbe riconoscerlo non ha un momento in cui fallire a comando. **La seconda meta' e' lo schermo**, e li' la cosa che si sbaglia e' un'altra: *una DATA non ha un fuso e un ISTANTE si'*. Misurato sul manifest, gli unici campi con un'ora sono i `generated_at`; le date pure (`observed_on`, `match_date`, `decided_on`...) non si convertono, o si inventerebbe loro un'ora che non hanno. La distinzione vive DENTRO `itDate` e non nei punti di chiamata, e il difetto che cura (`iso.slice(0, 10)`, il giorno UTC) era in tre posti di cui due nei TEMPLATE. **Anche un'ETA' e' visualizzazione**: «oggi / ieri / 3 giorni fa» si conta fra giorni locali (`todayShown`, ACCANTO a `today()` e non al suo posto - il difetto da evitare non e' avere due date, e' non sapere quale si sta usando), o all'01:30 la pastiglia direbbe «scritto il 16, oggi» a chi ha il 17 sul calendario. **988 test app, 907 toolkit, sei commit su origin, `engine_*` e `SHEET_REVISION` fermi.**
· **precedente: 16 settembre 2026 (notte) — L'ASSE DELLA STRISCIA «ULTIME PARTITE», E DIETRO DUE RICHIESTE C'ERANO QUATTRO DIFETTI.** Tre richieste dell'operatore sulla vista Squadre in un messaggio solo — «sistema la linea che evidenzia il cambio allenatore perche' e' rotta», «vedo dati strani relativi alla partita piu' recente delle squadre... quando i dati sono corrotti o incompleti, non visualizzarli», «mostriamo sempre una colonna con la prossima partita da giocare della squadra» — che sono tre facce di **una domanda sola: di quale partita una colonna parla**. (a) **LA LINEA ERA UN BORDO IN PIU', NON UNO INTERROTTO.** Fotografata a 14x, scendeva con un piolo rosa a ogni riga: `border-double` e `border-primary/70` scrivono stile e colore su **tutti e quattro i lati**, e antd usa il `border-bottom` da 1px di ogni cella come separatore di riga — misurato con `getComputedStyle`, quel separatore usciva `1px double` del colore primario. Tailwind non ha una forma per-lato per stile e colore, quindi la scorciatoia naturale e' proprio quella che sporca gli altri tre. La cura e' una classe nel layer dei componenti e la linea e' uno **SFONDO** e non un bordo (un `border-left` si ferma sopra il bordo inferiore e si interrompe di un pixel a ogni riga), col separatore reso trasparente sulla sola colonna larga dieci pixel. E la linea «doppia» adesso lo e': a 2px `double` e' una barra piena. (b) **UN DOCSTRING PROMETTEVA DALL'11/09 UNA COSA CHE IL CODICE NON FACEVA**: un cambio di panchina a cavallo di due stagioni produceva un SECONDO separatore attaccato a quello di stagione (22px affiancati sull'Atalanta), perche' una colonna di confine ha `date` nulla e il cambio scattava sulla prima colonna vera dopo — e non e' un angolo, un allenatore nuovo quasi sempre arriva d'estate. Ora il confine e' **uno e porta i due fatti** (`ColumnSlot.bench`, il `⇄` e la frase nel titolo). **Il test che ci provava passava col difetto**, e il suo commento diceva la cosa giusta: il fixture non era un confine (`divider` nullo), cioe' un caso che a un confine somiglia in tutto tranne che nel campo che lo definisce. (c) **LA «PARTITA PIU' RECENTE» ERA LA GIORNATA IN CORSO, E NASCONDEVA UN DIFETTO NOSTRO PIU' GRAVE.** I voti arrivano prima del livello per-partita, quindi la testa scriveva `Ata 1 - ??? 2` senza avversario, campo, modulo ne' click: misurato col join per `fc_id` che l'app usa davvero, **6 slot su 1.588** di tutta la storia di `default` non hanno nessuna riga del tabellino, e sono tutti e sei la 4a giornata del 2026-27. Sotto c'era il difetto vero: la testa si costruiva dalla **prima cella in ordine di rosa** e il livello per-partita copre il **94%** dei convocati, quindi **un portiere senza quella riga cancellava l'identita' dell'intera colonna** — `Inter - Hellas Verona` della 37a leggeva `Inter - Ignota 0-0` con 15 dei 17 compagni che la riga ce l'avevano. Ora si sceglie la cella che sa dire QUAL E' la partita, e quello che resta senza nome si toglie **con le sue celle** (`dropUnnamed`): **da 7 colonne senza avversario su 200 a 0 su 220** sui venti club di Serie A. Il prezzo e' detto — i voti della 4a giornata degli 8 club che l'hanno giocata spariscono finche' il tabellino non arriva, ed e' reversibile in una riga — e il taglio si ricuce da se', perche' la finestra pesca una colonna in piu' dalla stagione prima. (d) **LA PROSSIMA PARTITA** (`core/next-match.ts` + `LeagueCalendar.next`): prima colonna a sinistra, perche' la tabella si legge dalla piu' recente e il futuro viene prima del passato piu' vicino; scelta **per data** e non per giornata (con un rinvio la 16a si gioca dopo la 20a) e con `>= today`, perche' di un calendario si sa il giorno e non l'ora; **celle vuote** e non un trattino; nessun click promesso; niente colonna a stagione finita invece di una testa vuota. Vive nella VISTA e non in `PlayersStore`, che e' lo store del calcio giocato. (e) **IL BANCO NON MORDEVA, E L'HA DETTO LA CONTROPROVA**: rimettendo il markup vecchio, `e2e-coach-break.mjs` ha stampato «tutto a posto» **dopo aver guardato zero confini** — riconosceva un confine dalla classe che stava verificando, e un `every` su un elenco vuoto e' vero. Ora si riconosce dal TITOLO (il dato) e si pretende di averli guardati tutti; lo stesso difetto era in `e2e-clubs.mjs` dal lato opposto («senza testo» legge zero da quando il confine porta il `⇄`). **UN ERRORE DI MISURA MIO, a verbale**: la prima misura della copertura ha unito `match_ratings.team` a `external_match_stats.club` **per nome** e ha letto «quattro club interi senza tabellino» — erano Milan, Napoli, Roma e Verona, settima istanza del join per nome e la prima commessa dentro una MISURA invece che dentro il codice. **Verifica**: 983 test app (57 file), build pulito, banchi `coach-break` (riscritto e allargato ai tre casi), `clubs`, `clubs-lineup`, `matches-tip`, `table`, `nav` verdi su Serie A **e** su EuroLeghe; due banchi restano rossi (`player-card`, `board-horizon`) e sono **preesistenti**, verificati a HEAD con `git stash`. Nessun numero del motore si muove: `engine_*`, `SHEET_REVISION` e il pacchetto sono fermi — e' tutto lettura. Dettaglio: `letture-app-v1.md` §44.
· **precedente: 15 settembre 2026 — LA BOARD DI STAGIONE IMPARA QUELLO CHE LA BREVE SAPEVA GIA', E UNA CURA APPLICATA A UN ORIZZONTE SOLO LASCIA IN PIEDI IL DIFETTO PER CUI ERA STATA SCRITTA.** Giornata guidata interamente da segnalazioni dell'operatore sui campetti STAGIONALI («le formazioni dell'ultimo periodo mi sembrano buone, le formazioni tipo stagionali non ancora»): sei incoerenze in un messaggio, altre tre dopo la prima consegna. **Quattro dei sei casi avevano una causa sola, e la cura era gia' scritta**: il 12/09 la board dell'ultimo periodo aveva imparato a scegliere l'undici sulle righe che il modulo DICHIARA, mentre la stagione continuava a usare le tre linee del listone, dove un 4-2-3-1 collassa a (4, **5**, 1) - e il caso Soule' che lui segnala era scritto PER NOME nel commento di quella cura, come la ragione per cui esiste. Cinque adozioni: le righe dichiarate anche sulla stagione; il **POSTO che la finestra CONFERMA** (riga e fianco letti dal numero della distinta dentro il modulo di quella partita - `held_side`/`held_lane`, la sua regola «3 partite intere o quasi sono una conferma», con la soglia a MAGGIORANZA perche' quella severa perdeva uno dei due casi da cui nasce); `_flanked` che ripara solo le fasce SCOPERTE; un'ala candidata alla trequarti solo se e' il suo PRIMO codice; e le tre riparazioni che non possono piu' sacrificare un uomo che una dritta dichiara titolare - quella su Pinamonti era **a verbale dall'07/09 e il disegno la disfaceva due funzioni dopo**, rimettendo in campo esattamente l'uomo che la dritta nominava. Numeri: disegno concorde col modulo scelto **17/20 -> 19/20** (la breve stava gia' a 20/20), giudice stampa DIFF **5 -> 3** e uomini **152 -> 154** su 220, accordo fra le due board **171 -> 175** su 220, e i moduli senza trequarti fermi su **180 celle di 180** - la controprova che la variabile mossa e' una sola. `engine_*` non si muove; `SHEET_REVISION` 68 -> **70**, bundle rigenerato e caricato nell'app. **Alla sua domanda «nuovo ruolo in acquisizione?» la risposta e' NO: il dato c'era** (`desc_recent_slots`, la stessa colonna con cui la board breve sceglie l'undici) - ennesima istanza di «il dato c'era e mancava un lettore». **UN DIFETTO MIO, trovato da lui il giorno dopo**: la revisione 69 rendeva candidato trequartista chiunque avesse `RW`/`LW` fra i codici, e Malen (`ST;RW`) finiva a centrocampo. **IL RIFIUTO DELLA GIORNATA**: il caso Juventus (Yildiz al centro, Gonzalez a sinistra) e' reale, diagnosticato - su una trequarti il lato pesa 6 e un codice successivo costa 1, quindi il secondo mestiere vale quasi quanto il primo - e **non curato**: tre forme diverse guadagnano 3-5 posti su 166 sul giudice dei lati e rompono da 3 a 4 guardiani della COPERTURA DELLE FASCE, che e' `formazioni-tipo-v1.md` §16.2 ritrovata da un'altra strada (un prezzo PER UOMO non puo' esprimere un vincolo sull'UNDICI). Aperti: Pobega al Bologna (0,498 contro 0,455, e una cura provata ha effetto ZERO), Cisse' e Koopmeiners nel reparto sbagliato - **e il limite che i due hanno in comune: le dritte governano QUANTO gioca e non DOVE**, quindi non esiste un canale per dichiarare la posizione di un uomo. Dettaglio: `formazioni-tipo-v1.md` §17.
· **precedente: 12 settembre 2026 — IL MODULO ERA UN FATTO PUBBLICATO E LO DEDUCEVAMO.** Giornata nata da una richiesta piccola («nella pagina CLUBS, quando seleziono una partita cliccando sul nome della colonna, ricostruisci nel campetto la formazione che ha giocato quella partita») e finita su tre acquisizioni e la riscrittura della board dell'ULTIMO PERIODO. Il filo è uno solo, e questo repository lo ha già pagato otto volte: **il dato c'era e mancava un lettore**. (a) **IL MODULO DELLA FONTE.** Le linee del campetto venivano dai conteggi `G/D/M/F`, che ne hanno TRE: misurato, **122 disegni su 178 (68,5%)** discordavano dal modulo vero, e il più frequente è proprio il `4-2-3-1` che tre linee non possono esprimere (81 su 178). `home.formation` è nel payload dalla prima corsa e il downloader lo scartava. Adottata la sua regola — «utilizziamo il modulo della fonte, altrimenti non ci troviamo più con il resto» — con il taglio per INTERVALLI DI POSTO e non per fette, perché a fette un nome mancante faceva scivolare tutta la linea e il disegno tornava ai tre conteggi proprio su `euro`, dove i nomi mancano per costruzione. (b) **DOVE HA GIOCATO UN SUBENTRATO, E DI CHI HA PRESO IL POSTO**, dal suo caso: «la Juve ha cominciato con un 442 ma poi si è trasformato in un 4-2-3-1, e sembra che González abbia giocato nella stessa posizione di Kolo Muani, **che non è uscito**». Kolo Muani ha giocato 90 minuti: il suo posto non si è mai liberato. `/event/{id}/average-positions` porta **due fatti in una risposta** — la posizione media di chiunque abbia messo piede in campo, subentrati compresi, e l'elenco dei cambi con chi esce e chi entra — e i minuti non lo ricostruiscono (un'uscita trova un ingresso complementare unico nel **30,3%**, e su quella partita nemmeno torna: cinque entrati contro quattro usciti, perché Cambiaso è entrato al 76′ ed è uscito lui stesso al 86′). Tre colonne (`avg_x`, `avg_y`, `came_for`), la catena risalita, e **ZERO** subentrati che risultino entrati per un uomo rimasto in campo 90′. `avg_y` è nello stesso verso di `lineup_slot`, misurato: **86,0%** di accordo su 683 linee. (c) **LA BOARD DELL'ULTIMO PERIODO SUI MODULI OSSERVATI**, sua richiesta esplicita, con la precisazione che conta: «deve rappresentare la formazione tipo della squadra reale nel breve termine — e non c'entrano CLASSIC o MANTRA». `formation_shapes_recent` leggeva i tre conteggi: **10 board su 20** disegnavano un modulo che il club non ha dichiarato in nessuna delle ultime tre (Atalanta `4-3-2-1` contro un `4-3-3` detto tre volte su tre, Napoli `4-2-1-3`, Parma `3-4-1-2` contro `4-4-2`). Adesso **20 su 20**, con `_two_rows` e `_reshape` spenti su quella finestra — un modulo che la fonte NOMINA non si deduce, e uno giocato la settimana scorsa la rosa lo copre per costruzione. **A/B su UNA variabile sola**: board di STAGIONE 0 moduli e 0 disegni su 20 club, board breve 11 moduli e 12 disegni. (d) **E LA SUA CORREZIONE HA TROVATO UN DIFETTO CHE AVEVO APPENA INTRODOTTO**: «perché nella Roma vedo Wesley sulla trequarti?». Il modulo era giusto, la SELEZIONE no — `lines('3-4-2-1')` chiede «sei centrocampisti e un attaccante» mentre i trequartisti finiscono in ATTACCO, quindi Soulè contendeva l'unico posto d'attacco a Malen e la trequarti la riempiva un esterno. Sull'orizzonte corto la selezione chiede ora le righe del modulo dichiarato (P·D·M·T·A): Wesley torna all'`Es`, Soulè va sulla trequarti, fuori ruolo **15 → 14** (erano 17 col solo punto c). **La forma larga è stata scritta e BOCCIATA dalla misura**: ogni attaccante candidato alla trequarti porta i fuori ruolo a **22** e fa schierare al Milan Pulisic, che nelle ultime tre ha ZERO minuti. (e) **LA SUA GRIGLIA A SEI LINEE, MISURATA**: con `avg_x` le quattro righe di un modulo leggono 40,7 · 48,7 · 61,4 · **63,1**, cioè trequarti e attacco non si separano — quindi i nomi delle righe restano una convenzione DICHIARATA e non una misura. E la quinta riga non serve: **nessun modulo a cinque numeri esiste** nell'archivio (0 su 178). **Acquisizioni**: `--layer formations` (una richiesta per partita, nessuna per turno, dalla stagione in corso all'indietro perché una scansione da nove ore si abbandona appena la fonte rifiuta) ha chiuso **178 → 2.267 partite-lato** col modulo; `--layer places` si è fermato sulla guardia dei rifiuti dopo 210 partite e ha comunque completato la stagione in corso (Serie A 31/31, Liga 41/41, Premier 30/30, Ligue 1 28/28; manca la Bundesliga e le stagioni vecchie, e riprende a una corsa successiva). **Verifica**: 860 test toolkit, 918 app, build pulito, i banchi `e2e-clubs-lineup`, `e2e-board-horizon`, `e2e-board-sidelined`, `e2e-clubs` verdi; `SHEET_REVISION` 62, tre fogli + export + `data:pull` rifatti. `engine_*` fermo: nessuna colonna nuova entra in `evaluate`. **Due asserzioni nuove provate RIMETTENDO il difetto**: col codice vecchio il banco legge «Dovbyk e Mbangula disegnato sotto Piccoli, che ha giocato 90′». **APERTO, con il suo numero**: Chukwueze non è nell'undici del Milan e dovrebbe esserci — 2 partite su 3 da titolare, la fonte lo mette a CENTROCAMPO con `avg_y` 16 (fascia destra) e quel posto va a Rabiot, un centrale; la causa è che l'undici mette un uomo in una linea guardando il suo CODICE e non la linea MISURATA, e `external_match_stats.position` è il nostro vocabolario a quattro linee, quindi la trappola che l'11/09 aveva fatto tenere «solo il lato» lì non c'è. Più le staffette dai cambi VERI (43 coppie con due o più cambi reciproci in tre giornate — Hojlund↔Lucca, Scamacca↔Krstović, Malen↔Castro S. — mentre `desc_relay` poggia ancora sull'assunzione «entra al minuto 90 − giocati», che al Milan separa due uomini stati in campo insieme 55 minuti), e le coppe sulla board (`desc_cup*` esiste, la board non lo legge, e oggi è INERTE: la Coppa d'Asia è 07/01→05/02/2027). **TRE ERRORI DI MISURA MIEI, a verbale perché sono il modo in cui si sbaglia**: il primo A/B muoveva DUE variabili (il codice e i dati, cambiati dalle acquisizioni della stessa giornata) e ho dovuto rifarlo con un interruttore temporaneo; ho ricostruito la formula della staffetta a mano invece di CHIAMARLA, e la mia ricostruzione dava un numero che non era quello del foglio; e un fixture dei test generava `fc_id` collidenti (`MedianoA`/`MedianoB`/`MedianoC` lo stesso id), quindi il primo test della catena sembrava un difetto del codice. **DUE SESSIONI SU UN ALBERO, e questa volta HEAD era ROSSO**: `club-eleven.ts` a HEAD legge `outToday` sei volte e `valuation-store.ts` a HEAD non lo definisce — l'altra sessione ha committato i consumatori e lasciato fuori le definizioni. Quindi questo commit porta anche i loro due file, perché separare avrebbe lasciato il rosso che la regola esiste per evitare.
· **precedente: 7 settembre 2026, sera tardi — LE PARTITE DI QUEST'ANNO ARRIVANO DOVE NON ARRIVAVANO, E
...
vincolava una partita sola) e poi DODICI segnate a mano su due caselle, **tutte e dodici in casa**.
Quella regolarita' era una misura sul modello: il semi-scarto casalingo passa da 14,5 (fittato sul
RISULTATO) a **35** (fittato sulla PORTA INVIOLATA, misurato il giorno prima e lasciato in panchina
per un buon argomento), la logistica e' RIFITTATA sull'edge nuovo, e cosi' la sua regola del 10/08
regge a una soglia piu' bassa - un campo piu' grande rende le trasferte piu' difficili, quindi nessun
club di Serie A legge tutta la stagione facile. (b) I GOL, sua richiesta con il meccanismo dentro
(«una squadra forte non e' detto che segni tanto»): la porta inviolata legge anche i gol delle ultime
DIECI partite dei due club, log-loss fuori campione **0,54749 -> 0,54282, 7 stagioni su 8**, lift
1,95 -> 2,01. Tre cose decise contro la formulazione della richiesta: la finestra e' 10 e non 5 (la
curva e' piatta da 10 a 38 col minimo sul BORDO, che non si adotta), il casa/fuori non paga, e
l'attacco avversario vale sei volte la propria difesa. **E la prima misura era mia e sbagliata**: le
due righe di UNA partita condividono la data, quindi la seconda leggeva la partita dentro il proprio
predittore - quattro volte il guadagno e «piu' corta la finestra meglio e'», monotono fino al bordo,
che e' il campanello di casa. La soglia si sposta sulla PROBABILITA' (0,3002, la sua soglia letta sul
modello a solo Elo) perche' con tre predittori un margine sull'edge non puo' piu' dire «facile», e le
due sono appaiate per costruzione cosi' un club senza forma non cambia colore per essere promosso.
(c) UDINESE E LECCE, che lui dava per assurdi: l'Udinese passa **da 0 a 10** partite facili su 38 (ha
la miglior difesa delle ultime dieci e l'Elo non lo vedeva), il Lecce resta a **0 ed e' un fatto** -
e' 18o su 20, i soli club sotto di lui valgono 19 punti Elo e la sua partita migliore e' +56.
(d) IL TOTALE DI UNA COPPIA NASCONDE CHI L'HA PORTATO, sua osservazione: **111 caselle su 190
aggiungono due giornate o meno** al migliore dei propri due club e 5 su 190 battono il miglior
singolo di tre - il marginale e' ora a schermo, e serve DUE zeri con due nomi (nella lista il mio
calendario, sulla griglia il migliore dei due), perche' con lo zero sbagliato un compagno forte
finisce sotto uno debole - preso da un test, non da una rilettura. (e) LA GRIGLIA: diagonale VUOTA
(ATA/ATA non e' una coppia; il «da solo» passa accanto al nome della riga), sigle di tre lettere,
modale 1137px su 1574 e compatta in una riga, colori a CINQUE CLASSI PER QUANTILE (la distribuzione e'
ammassata in alto: cento caselle su 190 fra il 68% e il 100% del massimo, che una scala lineare
dipinge tutte uguali), pannello sul CLICK, e il difetto piu' grosso trovato da una sua casella - **le
colonne del popover erano invertite sotto la diagonale**, perche' la casella e' condivisa con la sua
speculare e `a`/`b` sono l'ordine della COPPIA e non riga/colonna. (f) I SUGGERIMENTI: la sua regola
**«non consigliare i portieri di squadre che da sole hanno gia' 25 o piu' partite facili»**, che ha
RITRATTATO il primo filtro (i TOP per prezzo, che escludeva undici dei quattordici migliori compagni,
Skorupski compreso) - «scontato» e' una proprieta' del CALENDARIO, non del cartellino, e la soglia vive
come quota della finestra. (g) LA PLANCIA: niente tooltip, l'hover accende la coppia alternativa con
uno SFONDO rosso leggerissimo, i suoi uomini in cima al blocco col grigio piu' chiaro (un PREFISSO:
sotto, l'ordine misurato non si muove), e un numerino nuovo - il delta a partita giocata con le
**partite attese fra parentesi**, dopo che la versione a giornata e' stata ritirata da lui («i punteggi
non tornano»): *una colonna che spiega un ordinamento deve ESSERE quello stesso ordinamento*, e due
fatti in una cifra non si leggono. (h) LA CARD DI UN CALCIATORE (`views/plancia/man-card/`): 288px,
trascinabile dall'intestazione, chiudibile, con le statistiche essenziali dal foglio - e il click ora
fa UNA cosa sola per tutti, mentre nominare il lotto e gli abbinamenti sono BOTTONI suoi. **PIU' DI
UNA insieme** per confrontare, affiancate e non a cascata (28px di sfalsamento le coprivano per il
90%), ognuna col suo POSTO tenuto per tutta la vita - chiudere non sposta le altre - e quella TOCCATA
passa davanti. Misurato tutto in un browser vero, compresi i tre modi in cui il banco ha mentito prima
di dire la verita': contare i bordi (250 su 250), leggere il canale rosso (Chrome computa `color-mix`
come `color(srgb …)`) e i pointer events che non scattano su un hover CDP. **652 test toolkit, 603 app
su 38 file, `ng build` senza avvisi, banco e2e verde con console pulita. `engine_*` e i `pi_*` fermi;
`SHEET_REVISION` 40 (la 39 e' questa sessione, la 40 dell'altra). E l'albero di oggi porta DUE
sessioni: il commit `0c34cc7` ha messo dentro anche la mia parte, quindi la storia va letta sapendolo.
(i) E L'ULTIMA: **l'offerta scende su un club che ho gia'** - sua istruzione, -10% sul secondo uomo di
quel club e -25% dal terzo, su TUTT'E DUE i posti che chiamano `offerBand`. La forma e' sua e i valori
sono dichiarati contro una misura che dice altro (il banco: primi DUE gratis, poi 45%), e la ragione
per cui il banco non decide qui e' scritta: la sua sd e' FRA stagioni, il rischio che si compra
diversificando e' DENTRO una. E' uno sconto sull'OFFERTA e non sul valore.
(j) E LE ROSE SI SCRIVONO A MANO (l'altra sessione, tre sue richieste in fila): un tasto **azzera le
rose** dietro conferma - il tavolo finto arriva con un terzo dell'asta giocata da un fixture e la sua
comincia da zero, quindi quel foglio non era usabile - e il **doppio click su una card di squadra le
assegna il lotto** al prezzo scritto nella riga, che e' l'unico fatto che la plancia non puo' ricavarsi
da se' quando non e' collegata. La regola nuova e' sul PREZZO: zero vuol dire «nessuno ha ancora
offerto» e non «un credito», quindi assegnare a zero sarebbe inventare un acquisto e falsare ogni tetto
che poggia sui crediti - si rifiuta, e i tre rifiuti (prezzo zero, reparto completo, borsa corta)
DICONO perche', perche' un rifiuto muto e' indistinguibile da un gesto rotto. Si scrive solo sul tavolo
nostro: i pick di un'asta vera sono del banditore e la prima riga in arrivo li cancellerebbe, e la
guardia sta nel FEED e non nelle viste. Piu' la terza richiesta, che e' un'ASSENZA e si misura come
tale: **via il `title` nativo dalle righe** («da' fastidio»), con le due funzioni che lo scrivevano -
prezzo e banda stanno sulla card, l'alternativa e' l'evidenziazione all'hover. Misurato in un browser
vero (`scripts/e2e-plancia-award.mjs`, puntatore CDP con `clickCount` 1 e 2): 10 rose su 10 a 1000 cr e
3-8-8-6, avanzamento P 0/30 D 0/80 C 0/80 A 0/60, **0 `title` su 250 righe**, prezzo 45 -> paga 45, un
posto in meno nel ruolo del lotto, lotto vuoto, barra del proprietario sulla riga. **621 test app su 39
file** dopo questa meta', tre banchi e2e della plancia verdi. E due lezioni dell'arnese, tutt'e due gia'
scritte qui: le coordinate del campo prezzo lette PRIMA del rifiuto (che aggiunge un avviso e sposta la
riga) facevano accusare la pagina di un difetto dello strumento, e **un fixture condiviso che un altro
test MUTA non e' un fixture** - `applyStreamEvent(mirror, 'put', '/', STATE)` restituisce l'oggetto
stesso e scriveva un terzo pick dentro lo `STATE` di tutto il file. Aperto: un «annulla ultimo
acquisto», perche' oggi un doppio click sbagliato si corregge solo azzerando tutto.
(k) E DUE COMMIT PER LA GIORNATA, che vanno letti insieme: `d118ab6` (questa meta': lo spec, il banco
`e2e-plancia-award.mjs` e i verbali) e **`bf8b192`**, l'altra meta' chiusa alle 09:13 - la DATA DI
RIENTRO che diventa un numero (`core/injury-window.ts`, `injuries.end_date`, `RETURN_SLIP` 0,25 e
`MIN_PLAY_SHARE` 0,60 dichiarati) e la prosa della pagina *indisponibili* che `upsert_availability`
buttava: quinta istanza di «il dato c'era, nessuno lo ha chiesto». Numeri, rifiuti e limiti stanno nel
suo verbale (`assistente-asta-v1.md` §36, `letture-app-v1.md`, spec «Novita'»), non qui, perche' sono
sue misure e questa riga e' solo il puntatore che mancava all'ingresso. Albero PULITO e i due gate
interi sulla stessa fotografia (09:14): **app 635 test su 39 file, toolkit 665**. Aperto: uno `snapshot` per
allineare `desc_easy_matches` e `desc_calendar_margin` al campo nuovo (qui il display Tk e' instabile e
un test lo salta), un `export` dopo di quello, e la scala numerica della difficolta' che aveva proposto
(`(0,3002 - P) / 0,3002`), da decidere se e dove.** · precedente: **3 settembre 2026, sera tardi (4) — I TRE PUNTI APERTI, E UNA SOGLIA DI FILTRO PRESA PER UN RISULTATO. Nessuna domanda nuova: i tre «prossimi passi» della chiusura (3). (a) `injuries` ripreso, e si e' scoperto che **`--refresh` e «riprendibile» si contraddicono** su una camminata di migliaia di pagine: interrotta a meta' lascia 2564 pagine lette oggi e 2100 lette due giorni fa, e `--refresh` ripaga tutt'e due le meta' mentre senza non ne paga nessuna. `--stale-days N` mette al posto del booleano l'ETA' della lettura — la stessa quantita' di `injuries.observed_on` — misurata sulla cache PRIMA di lanciare (0 · **2100 su 4664** · 24): la corsa ripresa annuncia 1891 giocatori su 3721 invece di 3721, e ha CHIUSO 1891 su 1891 senza un rifiuto — `injuries` porta ora **35.995 righe tutte con `observed_on` = 03/09**, cioe' il debito ereditato dalla chiusura (2) e' saldato; quello che la rilettura ha portato e' piccolo e va detto per intero, **9 assenze cominciate dal 1º settembre e 4 dal 2**, perche' il valore della cura non e' il conteggio di oggi ma che da oggi la tabella sa quando e' stata guardata. (b) **`update --daily`**: 7 passi, **~40 minuti contro 22h34**, una SELEZIONE dell'ordine unico e mai una seconda lista (test: `DAILY ⊆ plan()`), col criterio che non e' il costo ma COSA IL PASSO OSSERVA — e la DERIVAZIONE e' fuori per un'affermazione sul grafo, perche' `stats:derive`/`matchdays`/`synth`/`arrivals` leggono i voti e gli aggregati che una corsa quotidiana non rilegge. I 24 passi esclusi sono STAMPATI col loro costo. (c) **Il piano a due cambi**, e per arrivarci si e' dovuta correggere una riga del verbale di poche ore prima: «nessuno scambio guadagna piu' di 0,15 a giornata» era la **SOGLIA DI FILTRO** dello script, per giunta in fantapunti sulle TRE giornate, mentre il log prodotto dalla stessa riga aveva in cima +0,591 — trovata rieseguendo la funzione invece di citare il log. Il meglio di UN cambio e' +0,58; la coppia vale **+1,13 a giornata** perche' la prima mossa LIBERA i dieci crediti che rendono comprabile la seconda, e due cambi recuperano **1,40 dei 2,44** che separavano la sua rosa dalla costruita, dimezzando i buchi (0,739 → 0,428). La riga da consigliare NON e' la migliore: le prime due vendono un `anchor`, e «vendi l'uomo che non vedo» non e' un consiglio — misurato spegnendo la demozione, la cecita' vale 0,35 a giornata e la riga anchor-free (Bayo V. + Kean → Adams A. + Simeone) legge identica nelle due letture. **647 test + 1 skip, `engine_*`, i fogli e le revisioni fermi.** Aperto: un `export`, che adesso vale doppio - il pacchetto e' del 01/09 e il DB porta gli infortuni riletti oggi piu' il foglio Serie A ricostruito a 638 righe.)** · precedente: 3 settembre 2026, sera (3) — IL CLUB DEL LISTONE E' UN FATTO DI PIATTAFORMA, e l'operatore l'ha trovato con tre nomi. «Io mi trovo come portieri del como Butez+Sanchez+Vigorito» mentre il foglio portava Butez+TORNQVIST+Vigorito e Sanchez Ro. non c'era: dietro c'erano QUATTRO difetti con una radice sola. `listone_quotes` aveva la piattaforma per i PREZZI dal 07/08, il CLUB era rimasto in `rosters` (una riga per player-season, e `ratings:euro` gira dopo `ratings:default`), quindi **221 righe su 289 portavano il prezzo EURO**, la `league` era congelata al primo valore mai scritto da un COALESCE nel verso sbagliato, e cinque quotati Serie A stavano fuori dal perimetro perche' filati a Bournemouth, Aston Villa, Bayer, Lipsia e Stoccarda. Il quarto e' il piu' insidioso perche' e' una regola giusta portata fuori dal suo dominio: con `squad_source='squad'` un uomo che la fonte vede in un club ESTERO cadeva fuori da ENTRAMBI i rami della popolazione, e cosi' **35 quotati Serie A erano assenti dal foglio, 7 dei quali la pagina probabili dava titolari** (Woltemade 23 crediti schierato dalla Juventus e dato al Newcastle, Beto 14 alla Fiorentina e dato all'Everton). Il 17/08 dice che l'autorita' sulla rosa e' la fonte, e quella regola serve a leggere un'ASSENZA: **essere quotato su questo listone e' prova positiva di essere in questo campionato**. Migrazione + backfill dei due listoni 2026-27, lettori con COALESCE (inerti sullo storico): assenti **35 -> 22** e i 22 sono rimozioni giuste (tre fonti concordi), club smentiti **8 -> 0** (c'era Kean dato Fiorentina e oggi al Como, 24 crediti). Piu' la pagina probabili che si CANCELLAVA da se': la lettura euro (stagione ignota di proposito) sovrascriveva le righe Serie A dello stesso giorno via `INSERT OR REPLACE` sulla PK `(fc_id, valid_from)` - 479 probabilita' su 20 squadre ridotte a 250 su 10, e Maignan, Svilar, Dimarco e Malen che leggevano «assente» su una pagina che li portava; l'`update` completo ripara per caso, ogni corsa che si ferma prima lascia il giorno cancellato. **643 test + 1 skip, `--verify` 22/22, `engine_*` e le revisioni fermi.** E la sessione era nata per una rosa: 250 crediti sulla Qt.A, 3/8/8/6, giornate 3-5, regolamento arrivato in SEI messaggi - mod.difesa su 4 difensori + portiere fino a +3, capitano solo sui bonus, nessun voto d'ufficio (un buco e' uno ZERO), 5 sostituzioni, UNO switch. Sette misure in [rosa-3-giornate-v1.md](rosa-3-giornate-v1.md), fra cui: il **+3 del modificatore vale lo 0,02%** e i voti dei difensori sono asimmetrici a SINISTRA (skew -0,275), la correlazione dentro il club e' reale (+0,359 contro un null di -0,011) e NON aiuta perche' la scala e' localmente lineare, di un club gioca **esattamente un portiere** (curato in obiettivo E simulazione), e la sua terna di **tre PRIMI portieri di tre club per 18 crediti batte il trio di un club per 24** - proposta sua, e ha scoperto che ordinavo la distinta una volta sola invece che ogni giornata)** · precedente: 3 settembre 2026, notte (2) — UN'ASSENZA RECENTE HA DUE SIGNIFICATI, E LA PIU' CARA E' «NON ABBIAMO GUARDATO». Da una sua domanda: «come mai non abbiamo nessun infortunio di serie a che risale a oggi o ieri?». Cinque interrogazioni e un elenco di file per scoprire che il 96% delle pagine in cache era stato letto il 1º settembre — una pagina letta l'1 NON PUO' contenere un infortunio cominciato il 2, e il buco era uniforme sui cinque campionati, che e' la firma di una causa nostra. Da qui `injuries.observed_on` (la data della LETTURA, presa dal file e mai dall'orologio; migrata sul DB vivo alle 13:17). Poi la richiesta vera: «recepire anche la notizia di corridoio per alzare almeno un alert». IL CANALE C'ERA GIA' E MANCAVA UNA RIGA IN UNA ALLOWLIST — `availability` era nel DB, nel contratto e in `data/export/`, non in `TABLES` di `pull-bundle.mjs`: TERZA istanza del difetto dei campetti. Vale 114 indisponibili sul listone di cui **70 senza un infortunio aperto sull'ufficiale**, McTominay compreso. `unavailableNow()` e' una TERZA domanda con la sua soglia (45 disegna un'icona, 30 merita una busta, questa decide per sabato) e agisce come VINCOLO e mai come peso su plancia, pannello draft e buste — perche' una voce di corridoio non dice la durata, quindi riprezzare sarebbe inventare. E siccome una lettura scaduta SPEGNE il marchio, la freschezza sta a schermo su ogni pagina (`ui/data-freshness`): due date, e il colore dice la CONSEGUENZA — oltre tre giorni «allarmi spenti», perche' uno schermo senza allarmi si legge come «non c'e' nessuno fuori». 593 test app su 37 file, 638 toolkit, `ng build` verde; 15 righe barrate sulla plancia tutte in posizione 9-10 su 10, misurate in un browser. `engine_*`, i fogli e le revisioni fermi)** · precedente: 3 settembre 2026, notte (DUE PORTIERI SI ACCOPPIANO SUL CALENDARIO, E LA SOGLIA SCELTA A OCCHIO ERA GIA' LA RISPOSTA — cliccando un portiere sulla plancia si apre con chi accoppiarlo (e un tasto apre la griglia club x club, con la V sulle facili nell'hover). Il risultato non e' la funzione: `EASY_MARGIN` = 200, che l'operatore aveva congelato il 10/08 su un criterio DIVERSO, misurato contro la frase di oggi («e' probabile che non subisca 0 gol») vale esattamente **P(porta inviolata) = 0,401** su 5354 partite-club di Serie A — e quel 40% e' anche `club_defence.CLEAN_SHEET_SHARE`, misurato un mese prima su un criterio scorrelato. Il conteggio pero' SATURA IN BASSO (12 club di 20 senza nessuna partita facile in stagione), quindi la sua regola DECIDE e una colonna continua rompe il pareggio; abbassare la soglia e' stato respinto perche' sarebbe sceglierla in base al risultato che piace. Il vantaggio campo della porta inviolata e' 30-35 punti Elo contro i 14,5 del RISULTATO: misurato e NON adottato (0,0004 di log-loss). Piu' il NG8011 che il terminale stampava da tredici giorni: l'imbuto del filtro non e' mai arrivato nello slot di antd, e non si vedeva perche' la nostra CSS lo compensava — pixel giusto, markup sbagliato, ora asserito da `e2e-table` e provato rimettendo il difetto. **Commit `3ef01e0`, che porta anche la meta' dell'ALTRA sessione** («chi oggi non gioca»: `player-status.ts`, `ui/player-flags`, `injuries.observed_on`, l'ordine `outNow`), misurata e non creduta: 637 test toolkit, 593 app su 37 file, `ng build` senza avvisi, cinque banchi e2e verdi. `engine_*`, i fogli e le revisioni fermi)** · precedente: 3 settembre 2026, sera (LA PLANCIA E' SCRITTA, E LA SUA ASTA NON E' A REPARTI — correzione dell'operatore sulla sua stessa asta: «estrae calciatori random NON per reparto, tutti insieme», mentre il banco la modella con `bench.PHASES` adottato perche' 16 delle 20 aste reali con la sua configurazione portano quella firma. Non cambia la parte grossa — sconto di fine asta, tempismo, scala per (ruolo, slot), tetti — perche' sono indicizzati sulle MANI ALZATE e non sull'ordine; cambia tutto cio' che il §16 spiegava con le fasi, e il §29 lo scrive. E la pagina `/plancia` esiste: il lotto e' una RIGA sotto l'intestazione, le squadre una COLONNA a destra a griglia, e la plancia porta TUTTE le 250 righe con la max offerta su ogni riga e la coppia dello slot sotto sull'hover — niente si apre e niente si chiude, perche' a estrazione libera non c'e' una fase da espandere. La connessione a fanta-asta-live diventa FACOLTATIVA su tutt'e due le pagine d'asta: si apre su un tavolo finto coi settaggi standard e ci si collega con un bottone. Quattro difetti trovati dalla misura in un browser vero, e il piu' caro e' di UNITA': sommavo il 2º e il 3º portiere in un totale, ma di portieri se ne schiera uno e quei due sono alternative FRA LORO. 568 test su 36 file, `ng build` verde, hover provato con un puntatore vero; `engine_*`, i fogli e le revisioni fermi)** · precedente: 2 settembre 2026, notte tarda (IL VANTAGGIO INFORMATIVO PAGA, E PAGA DENTRO LA FASCIA - all'urna il braccio motore non leggeva UN SOLO numero del motore, contato invece che dedotto dal codice: `engine_worth`, `cover_value`, `alternative` e il tetto di reparto fanno 1436 chiamate a chiamata e ZERO a estrazione, perché la scala di mercato prezza `richiesta × passo(fascia)` e la fascia la definisce il PREZZO. Una fascia è larga dieci uomini, e dentro una fascia il prezzo varia dello 0,08 della sua mediana mentre le presenze attese variano dello 0,33 del calendario, cioè dodici giornate: quello è lo spazio, e la quantità non è una scelta - il nostro vantaggio sulla quotazione è largo un numero solo (`pv_pred | Qt.I` +0,243 contro il -0,077 del surplus). `INSIGHT` 0,80, che CONSERVA per costruzione: **+1,02% robust su 800 stagioni** (appaiato +26,4 ± 4,5, t 5,9, 9 finestre su 10), buchi 22,9 → 18,8, titoli 175 → 257 su 800 - e a differenza del margine della scala **sopravvive a tre bracci al tavolo** (+30,0, t 7,4), perché non alza un'offerta: sposta gli stessi soldi dentro una fascia. Più l'etichetta STRICT ritirata dal campione doppio, tre cure misurate e respinte, e i portieri come contro-esempio del §19.1. 47 prove del banco, 620 test verdi, `engine_*` e i fogli fermi, `682343b`)** · precedente: 27 agosto 2026, sera (LE LISTE SI RIORDINANO A MANO E I MODULI MANTRA VALGONO L'1% - l'ordine personale dei blocchi e' un PREFISSO (i suoi nomi in cima, sotto il gain), verificato con un puntatore vero contando gli eventi che arrivano; e la valutazione dei moduli: 5 difensivi + 5 offensivi in tutti e undici - lo dichiara il rulebook, che rifiuta i modificatori classici perche' «gli schemi sono gia' bilanciati» - da 3 a 5 posti da BONUS, tetto in punti che varia dell'1,15%, e il surplus per credito che scende da Por 44 a Pc 8,5: il modulo non si scegli prima dell'asta. 533 test. Piu' due sessioni sullo stesso file, curate con un worktree)** · precedente: 27 agosto 2026 (LA PAGINA STRATEGIA - `/strategy` dichiara il regolamento della lega e disegna un blocco per ruolo coi migliori nomi: il GAIN e' il surplus a rilanci e il VALORE in un draft (misurato, non scelto), e la lunghezza di ogni lista e' la domanda della stanza - slot x partecipanti su classic (8 difensori x 8 = 64), le quote delle forme su mantra. E la regola dell'operatore sul «posto piu' arretrato»: vera nel problema che descrive - 15 dei primi 16 trequartisti possono giocare da C - e RESPINTA in tutt'e due le forme che la applicano, perche' il filtro lascia i braccetti a ZERO nomi e il riordino porta la somma dei gain di quel blocco da 256 a MENO 9. Quindi MARCA e non filtra, con la lettura letterale a un clic: una lista per ruolo non puo' esprimere un vincolo congiunto, ed e' il §16 della metrica visto dal lato del display. 509 test dell'app, `ng build` verde, arnese e2e nuovo, `70c8ed6`; `engine_*`, i fogli e le revisioni fermi)** · precedente: 25 agosto 2026, notte (LA PAGINA DELLE BUSTE SI CHIUDE DA SOLA, E «SOLITARIA» ERA LA PAROLA CHE NON SI LEGGEVA - un export nuovo chiude il round, quindi le buste vanno agli atti PRIMA di essere sostituite: il registro si scriveva solo a mano, e un azzeramento silenzioso si legge esattamente come «non c'era niente da tenere»; `RoundLog.auto` DICHIARA quale delle due prove è, e il caricamento dice cosa ha fatto - zero aggiudicazioni nuove non è un round, quindi lo dice e nomina «Annulla l'ultimo» invece di inferire il regolamento. Più la regola dei portieri portata dall'uomo al REPARTO (`keeperAnchored`: un `ballottaggio` comprato accanto a un `bandiera` non è una scommessa solitaria) e `expectedHoles` che sa che due portieri di un club non giocano la stessa partita - 0,87 di calendario letti dove coprono 1,00. 487 test, `ng build` verde, `f35b082` pushato; `engine_*`, i fogli e le revisioni fermi)** · precedente: 20 agosto 2026, tarda sera (LA TITOLARITA' IN UNA PAROLA, E LA PAROLA STESSA DEFINITA - sei gradini dettati dall'operatore, letti come DUE assi (quota di partite + pavimento di minuti) e non come una probabilita' congiunta, che e' stata costruita per prima e misurata inservibile; la quota e' CONDIZIONATA e la board e' un CANCELLO, che e' anche il classificatore migliore (q75 0,512 contro 0,328 a parita' di claim); resa misurata su quattro finestre retrodatate, `bandiera` e `titolare` mantengono la promessa 4 volte su 4. `SHEET_REVISION` 35, `engine_*` fermo, colonna `Tit.` a tre caratteri in tabella, v0.1.21. E il termine definito dall'operatore: titolarita' = prende il voto, quota da titolare = parte dall'inizio, con la bonifica del codice e una nota datata sui verbali storici)** · precedente: 20 agosto 2026, notte (LA MVa ERA LA METÀ DERIVATA DELLA COPPIA, ED ERA LA METÀ SBAGLIATA — trovata dall’operatore su due righe del foglio classic: `est_mv` era `engine_fm_pred` (già regredita) meno il suo tasso di bonus GREZZO, quindi tutta la regressione finiva sul voto base e Malen leggeva 5,67 contro i 6,75 misurati. Adesso la MVa è la metà PREVISTA e il tasso è quello che ne esce; tre parametri fuori campione con cross-fit UNANIME, un canale rifiutato come zero dichiarato, e la richiesta dell’operatore («chi segna ha sempre o quasi un voto buono») che era essa stessa una misura: r = +0,787 dentro il ruolo. `SHEET_REVISION` 32, `engine_*` fermo, v0.1.20 pubblicata)** · precedente: 20 agosto 2026 (IL GRAFICO DI Fπ NON SI VEDEVA MAI, e la lezione sono le tre verifiche che l'hanno mancato: una `computed` che SCRIVE un signal è vietata in Angular, quindi sollevava un'eccezione a ogni lettura del template — e `ng build` compila i template senza eseguirli, non compila gli spec, e un errore di tipo in uno spec faceva fallire la costruzione della suite intera, cioè `ng test` non girava da `d7d0fbf` e le prove dichiarate la sera prima non venivano da una corsa. Più una classe CSS che non esiste e un tooltip il doppio del massimo. 317 prove verdi, motore non toccato)** · precedente: 19 agosto 2026, sera (Fπ — UNA COLONNA CHE PRONOSTICA INVECE DI SOMMARE: nasce dalla verifica di coerenza fra FVM e Overall, che ha trovato una divergenza SISTEMATICA e non un errore di conto — Overall è un totale senza zero, quindi premia chi gioca sempre a 5,8 e lascia indietro chi in Italia non ha mai giocato. Tre parametri misurati fuori campione benché nessun gate li possieda (l'equivalente estero regredito, il calendario della finestra come DEVIAZIONE, l'investimento del club rispetto a chi gli contende la maglia), la scala 0-99 dettata dall'operatore in cinque passaggi con una curva sotto l'ancora, quattro idee respinte e otto club stranieri archiviati come `serie_a`. `SHEET_REVISION` 31, v0.1.19 pubblicata)** · precedente: 19 agosto 2026 (UN VECCHIO PV NON È UNA PREVISIONE DI PRESENZE — `SHEET_REVISION` 29: il
...
## VOCABOLARIO: «titolarità» qui vuol dire PRENDERE IL VOTO (definita dall'operatore il 20/08/2026)

Prima di leggere qualsiasi altra pagina di questa cartella, la definizione che le governa tutte:
**«nel linguaggio comune "titolarità" è il fattore che indica se un calciatore parte dall'inizio in campo,
nel nostro progetto invece dobbiamo usarlo per indicare che un calciatore gioca abbastanza da prendere il
voto (anche se non parte dal principio in campo)»**. Una rosa di fantacalcio incassa quello che incassano
i voti, e un subentrato fisso il voto lo prende ogni settimana: la quantità di cui parla il progetto è la
PRESENZA A VOTO, e la parola segue la quantità.

| si dice | vuol dire | dove sta |
|---|---|---|
| **titolarità** | quota di giornate in cui prende un VOTO | `presence.voto_share` (su tutte le giornate), `presence.appearance_share` (su quelle per cui è disponibile), `engine_pv_pred`, `desc_titolarita_play` |
| **quota da titolare** | quota di giornate in cui parte dall'inizio | `presence.standing`, `claim`, `presence.presence`, `starting_record`, `desc_start_share` |

La scala a sei gradini (`engine/status.py`, `desc_titolarita`, §16 di
[letture-app-v1.md](letture-app-v1.md)) è nominata in questo senso: il suo asse è la quota a voto, e lì
`titolare` significa «gioca quasi ogni partita, e per quasi tutta», non «è in distinta».

**Come leggere le pagine scritte prima del 20/08/2026.** Il codice e le stringhe dell'app sono stati
bonificati quel giorno; i VERBALI di questa cartella — il gate, la continuità, le todolist, e i paragrafi
storici qui sotto — no, e apposta: dove dicono «titolarità» registrano una misura fatta sulla quota **da
titolare** (il gate ha sempre spazzato i parametri contro le titolarità di partenza realizzate, ed è
giusto così: `standing` prevede chi parte). Riscriverli cambierebbe il verbale invece di chiarirlo. Quindi:
...
Per la STRATEGIA (cosa si prepara PRIMA di sedersi al tavolo): **`pagina-strategia-v1.md`** — la pagina
`/strategy`, le impostazioni della lega, la MONETA per tipo d'asta (surplus a rilanci, valore in un draft:
...
in `simulatore-asta-rilanci-v1.md` **§15-§21** prima di toccare il banco, perché ha cambiato il MECCANISMO
...
dice cosa fare dopo e cosa **non** rifare, con i numeri. **E il §21 è la cosa più grossa che quella lista
non conteneva**: all'urna quella scala non legge nessun numero del motore (contato, non dedotto), quindi
...
> **3 settembre, sera — la plancia è SCRITTA e la sua asta è a ESTRAZIONE LIBERA.** `/plancia` esiste
> (`views/plancia/`, `core/plancia*.ts`): dettaglio in [assistente-asta-v1.md](assistente-asta-v1.md)
> **§33** e [simulatore-asta-rilanci-v1.md](simulatore-asta-rilanci-v1.md) **§29**. La correzione conta
> più della pagina: il banco modella la sua asta **a reparti** e la sua non lo è, quindi va rimisurato
> tutto quello che il §16 spiegava con le fasi — mentre le misure indicizzate sulle **mani alzate**
> (sconto §23.1, tempismo §24, scala §19.3, tetti §27) reggono intatte. La connessione a fanta-asta-live
> è ora **facoltativa** su `/plancia` e `/auction`: si apre sul tavolo finto e ci si collega con un
> bottone. 568 test verdi; `engine_*`, i fogli e le revisioni fermi.
...
**Il 27/08 sera in quattro righe.** Le liste della pagina Strategia **si riordinano a mano** e l'ordine è
suo e resta (modello a PREFISSO: i nomi che ha sistemato in cima, sotto continua il gain; chiave
`listone|gioco|ruolo`, quindi un export nuovo la conserva). **533 test in 34 file**, `ng build` verde,
arnese e2e con un solo rosso, che è di un'altra sessione (sotto). Il codice di `core/` e l'arnese sono
...
BONUS vanno da **3 a 5** (4-1-4-1 cinque; il 4-2-3-1 ne OBBLIGA quattro, che è una pretesa sulla rosa e non
una probabilità). Il tetto in PUNTI però varia dell'**1,15%** e il vincitore cambia con la lettura, con il
listone e con il budget: **il modulo non è una leva**. Quello che cambia è il prezzo - surplus per 100
crediti da `Por` 44 e `B` 38 a **`Pc` 8,5** - quindi due posti da `Pc` sono il modo più caro di riempire i
cinque offensivi; e la risposta si RIBALTA in un draft, dove si spendono scelte e il `Pc` dà il surplus
assoluto più alto di tutti. Il modulo non si scegli prima dell'asta: si comprano i nomi per surplus tenendo
copribili due posti da T/W e uno o due da Pc.

**DUE SESSIONI SULLO STESSO FILE.** L'altra ha spostato il regolamento della lega in `core/global-options.ts`
+ `ui/global-options/` (una dichiarazione sola per tutte le viste, le Buste chiuse ne tenevano una copia) e
ha cancellato la finestra dalla mia pagina. I due lavori si sono fusi bene, ma per due ore l'albero non
compilava: **si verifica in un worktree su HEAD più i propri file** (`node_modules` e `public/data` per
giunzione), che è il solo modo di avere un cancello verde. E l'arnese ha trovato il loro capo sciolto: il
bottone «Impostazioni lega» chiamava `GlobalOptions.open()`, che alza `panelOpen`, e **nessuno leggeva quel
segnale** - due segnali per una porta. L'operatore l'ha visto lo stesso giorno; **curato** nel pannello (che
ora quel segnale lo legge), e la cura è in un file loro quindi non è in un mio commit. La cura ha poi
smascherato un difetto dell'ARNESE che vale per ogni futura verifica: il click su «Annulla» arrivava zero
volte perché il modale, entrando, SPOSTA i suoi bottoni per ~200ms - le coordinate valgono nel momento in
cui si clicca, non in quello della misura (`clickSteady`: due letture uguali di fila, poi si clicca).

## STATO AL 27 AGOSTO 2026 — LEGGI QUESTO PRIMA DI TUTTO

Le sezioni sotto sono un **registro cronologico**: dove una contraddice questo blocco, vince questo.

**Il 26-27/08 in quattro righe.** Sessione tutta in `app/` (le modifiche del toolkit nell'albero erano di
un'altra sessione e sono rimaste fuori dal commit). C'è una pagina nuova, **`/strategy`**: si dichiara il
regolamento della lega — listone, gioco, rose, budget, rilanci o draft, partecipanti — e la pagina disegna
un blocco per ruolo con i migliori nomi. **509 test** (erano 487), `ng build` verde, arnese e2e nuovo
(`scripts/e2e-strategy.mjs`) senza problemi, `70c8ed6` **non pushato**. `engine_*`, i fogli e le revisioni
non si muovono: qui non si prevede nessun calciatore, si decide quanti nomi servono e in che ordine
leggerli. Verbale: **[pagina-strategia-v1.md](pagina-strategia-v1.md)**.

**Due decisioni dichiarate, ognuna con la sua misura alle spalle.** Il **GAIN dipende dal tipo d'asta** —
surplus a rilanci (la risorsa scarsa è il credito, cioè quello che il surplus sottrae), VALORE in un draft
(−4,0% per il surplus sulle cinque finestre del banco) — e la **lunghezza di una lista è la domanda della
stanza**: `slot × partecipanti` su classic (P 30 · D 80 · C 80 · A 60 con dieci squadre e rose 3/8/8/6),
le quote delle forme su mantra (Dc 51 · M 27 · C 26 · A 24 · E 23 · W 17 · T 16 · Pc 15 · Dd/Ds 13 · Por
20 · B 10 col pavimento «almeno uno per partecipante»).

**La richiesta della sera del 26/08 e il suo verdetto.** «Conviene sempre schierare un calciatore nella
posizione del modulo più difensiva rispetto ai suoi ruoli ... che ne pensi se nei blocchi consideriamo solo
la posizione più difensiva?» — il problema è **reale e misurato** (fra i primi 16 trequartisti del foglio
Serie A mantra **15** possono giocare da C, e 12 sono anche fra i primi 26 dei centrali; gli esterni 18 su
23; i braccetti 10 su 10), e tutt'e due le forme che lo curano sono **respinte**: tenere solo il posto più
arretrato lascia i BRACCETTI a **zero** nomi su tutt'e due i listoni, mettere i nativi davanti porta la
somma dei gain dei trequartisti da 256 a **−9** e degli attaccanti esterni da 116 a **−94**, facendo
sparire McTominay 27,8 e Dimarco 37,0. Adottato: **la regola marca e non filtra** (`↓C` accanto al gain),
con la lettura letterale a un clic e il prezzo scritto. La ragione sta nel §16 della metrica vista dal lato
del display — *una quota per ruolo non può esprimere quello che il rulebook raziona* — e dove quel
principio decide davvero, l'ASSEGNAZIONE, l'app lo applica già in modo esatto col matching di
`mantra-legal.ts`.

**Una cosa sul codice che vale oltre la pagina**: il foglio è scelto da **(listone, gioco)** e non dalla
piattaforma (il bundle ne porta tre, e il surplus è un fatto sul GIOCO per cui compri), quindi
`ValuationStore` espone `sheets` e `expectationsFor(sheet)` — il lettore delle colonne del motore era già
uno e resta uno, si è solo aperto a un foglio NOMINATO.

**Aperti**: la pagina non sa cosa hai già in rosa (agganciarla a `expectedHoles` / `fanta-eleven` è l'item
più grosso), «Solo di mestiere» come POLITICA è pre-registrabile sul banco dei draft, il budget non entra
ancora in nessun numero, e la moneta del draft su CLASSIC non è mai stata misurata.
...
**487 test** (erano 406), `ng build` verde, e2e senza problemi sul round vero, e il sito è stato
...
**E la notte ha chiuso due richieste, entrambe della stessa famiglia delle tre di sopra.** «Caricare le
nuove rose conservando la tornata precedente»: il bottone c'era già (i round sono le differenze fra gli
export), quello che non restava era il round che si CHIUDEVA — il registro delle buste, unico ingresso di
`settle`, si scriveva solo premendo «Registra queste buste». Adesso `closeRound` le mette agli atti PRIMA
che l'export le sostituisca (l'ordine è forzato: `setSnapshots` cancella tutto quello da cui `plan()` è
costruito), un round registrato a mano non si sovrascrive, `RoundLog.auto` dichiara quale delle due prove
è, e il caricamento dice che cosa ha fatto — **zero aggiudicazioni nuove non è un round**, quindi lo dice
e nomina «Annulla l'ultimo» invece di decidere al posto suo. «Il consiglio di offrire Falcone + Provedel è
ottimo, vorrei che ci fosse anche nelle buste consigliate»: la regola dei portieri era ancora **un binario
per uomo**, quindi un `ballottaggio` a sconto comprato ACCANTO a un `bandiera` veniva scambiato via e
contato come scommessa — la parola che nessuno leggeva era «solitaria». `keeperAnchored` (un portiere che
gioca, oppure una maglia posseduta per intero), nessuna soglia nuova, riletta a ogni passo del riparo; e
`expectedHoles` sa adesso che due portieri di un club non giocano la stessa partita, quindi la copertura è
la somma del reparto e non una convoluzione — Milinkovic-Savic + Meret leggevano **0,87** di calendario
dove coprono **1,00**. La forma che l'operatore voleva **il piano ci arriva da solo appena la può pagare**
(a 257 di tetto Provedel + Palmisani, con 17 crediti in più Falcone + Provedel: reparto 26,8 → 34,9, buchi
0,18 → 0,07): non è un giudizio sulla porta, è il budget. **487 test verdi**, `ng build` verde; l'e2e non è
stato rigirato dopo `keeperAnchored` e sta scritto nella todolist.

**Commit**: `a0f249d` (la pagina), `7772493` (i sette difetti), `165dc27` (il caso Martinez),
`427c4a0` (il compagno di club e il reparto come insieme), `f35b082` (il registro automatico e
«solitaria») più i tre
...
`backtest --verify` **22/22** — non si muove
un decimale di quello che è gated, perché la seconda colonna nasce reporting. **v0.1.17 pubblicata** col
bundle vero. Poi il **prior personale del pannello** è stato misurato e respinto su 12 fogli su 12
(§7-tretricies), quindi in testa alla lista aperta ora c'è la TENDENZA della curva del valore.
...
**I DUE ZERI SONO SUL FOGLIO** (16/08 sera tardi, metrica **§21.3**, spec «Novità v9.55»; commit
`bbcc0c6` il foglio e l'app, `089339b` il pannello Tk e i filtri, `ea19d01` la pubblicazione).
...
**Il PRIOR PERSONALE del pannello è stato misurato e RESPINTO** (gate **§7-tretricies**), ed era la voce
in testa a questa lista. Su tutti e **dodici** i fogli retrodatati, sostituendo la sola variabile (il
prior verso cui la restrizione tira) sulla vista vera: muove metà dei claim (mediana 0,08 a febbraio,
0,19 a settembre) e 385 posti di board, cambia **zero moduli**, e contro l'esito che il foglio stesso
porta è **peggiore o uguale 12 volte su 12** (2.142 contro 2.164 di 3.322). Il meccanismo è misurato e
non raccontato: il prior personale correla **+0,523** con lo standing grezzo di quest'anno — metà di
quello che porta è già nei minuti che il pannello guarda, l'altra metà è vecchia di una stagione — ed è
la stessa forma del rifiuto del canale ETÀ. E i moduli non si muovono per una ragione strutturale: dentro
un club gli uomini hanno quasi tutti lo stesso campione, quindi la restrizione è una mappa AFFINE e
l'ordine non può cambiare.

**Cosa resta aperto**, in ordine: la TENDENZA della curva
