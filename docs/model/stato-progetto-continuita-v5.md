...
**Aggiornato: 6 settembre 2026 (UNA SUITE VERDE NON VEDE UN DIFETTO DI ORDINE FRA MODULI - da «fai una code-review su tutto il progetto». Punto di partenza misurato: 689 test del toolkit, 736 dell'app, build pulito, e QUATTRO cose rotte lo stesso, tutte nel percorso di SCRITTURA. I primi due li ha trovati un audit MECCANICO e non una rilettura - le colonne dello schema meno quelle nominate, per ognuno dei 14 `INSERT OR REPLACE` del toolkit, coi dodici innocenti elencati accanto ai due colpevoli: `recent_form.store` si portava via `mv_synth` e i quattro BONUS che quel modulo paga UNA RICHIESTA A PARTITA da un secondo endpoint, `stats` si portava via `clean_sheets` (509 stagioni di portieri euro azzerate da un `stats` lanciato da solo, invisibile perche' `rebuild` e `update` richiamano la derivazione subito dopo). Il TERZO rompeva un invariante della spec: `rebuild` non chiamava `recent_form.reingest_from_cache` - la funzione c'era, era testata, nessuno la invocava - quindi un rebuild lasciava a ZERO tutte le 1.731 partite di `sofascore_recent`, cioe' il calcio giocato altrove da chi qui non ha storia. Il QUARTO: `backfill_bonuses` scriveva nel DB e mai nella cache, quindi 1.086 di quelle 1.731 partite portavano bonus che il disco non aveva e un rebuild li avrebbe rimessi in vendita uno per uno - ora la scrittura sta in un `finally` accanto al commit, perche' una cache aggiornata solo sul percorso felice lascia da ricomprare le richieste del giocatore interrotto. E uno nell'APP dove il commento accanto dichiarava il contrario: `pool` (seicento righe) leggeva l'ELENCO delle pastiglie accese, quindi si rifaceva a ogni click su una qualunque delle undici; ora passa da un computed suo e accendere `Bpm` non tocca niente. `--from-cache` sul comando, 739 test app e 693 toolkit verdi in un WORKTREE su HEAD piu' i soli file miei, `engine_*` e i fogli fermi. E un SECONDO commit (`a413dd6`) sui due punti che la review aveva trovato e non finito: `export` poteva scrivere ovunque - la domanda aperta dal 25/08 non si chiude trovando chi digito' cosa, si chiude rendendolo impossibile, quindi ora rifiuta una destinazione dentro il repo e fuori da `data/` (su `data_dir` e non su un `data/` letterale, o rifiuterebbe l'unica cartella che protegge) - e il «14 siti / 12 innocenti» che avevo pubblicato e' RITIRATO: sono 8 e 6, perche' il mio parser spezzava le istruzioni SQL scritte su piu' literal Python adiacenti. Prima di pubblicare un conteggio, chiedersi se non sia il numero del proprio arnese. Dettaglio: la sezione qui sotto)** · precedente: 5 settembre 2026, notte (LA QUOTA DA TITOLARE ERA IN UNA FINESTRA DI DUE GIORNATE - da un'osservazione dell'operatore su tre attaccanti in cima alla Strategia, «i minuti previsti a partita sono molto bassi: Thuram 39, Krstovic 36, Castro 38», con la proposta di un malus sul GAIN. Quei tre numeri erano un DIFETTO e non un fatto sul calcio - Thuram aveva giocato 29 partite, 24 da titolare, 1913 minuti: la miscela del 04/09 aveva spostato `desc_season_starts` e `desc_season_matches` e lasciato `desc_start_share` sulla sola stagione in corso, quindi `minutes.per_appearance` prendeva i minuti da una finestra e la quota da titolare da due giornate. 267 righe su 602 in disaccordo col proprio starts/matches, 100 a 0,000 esatto, 94 righe che attraversano un pavimento della scala della titolarita' - e nei DUE VERSI, perche' chi le sue due partite le aveva cominciate da titolare leggeva 1,000. IL GIUDICE ABITUALE E' STATO CAMBIATO PRIMA DI LANCIARLO: `press --against press` giudica la board, che quella colonna non la legge, quindi avrebbe stampato due numeri identici da leggere come conferma; misurato invece sulla quota stessa fuori campione (prevedere starts/appearances sulle giornate che RESTANO), MAE 0,2749 -> 0,1873 a k=2, +31,9%, 6 stagioni su 6 e 5 campionati su 5, inerte su una pre-stagione. IL MALUS CHIESTO E' MISURATO E NON ADOTTATO: il meccanismo e' vero in attacco (r +0,424 fra cambio dei minuti e cambio del tasso bonus) ma la fantamedia lo contiene gia' - la correlazione grezza +0,322 diventa +0,082 a parita' di fantamedia precedente e zero per gli altri tre ruoli, cioe' +0,04 a giornata contro un buco che ne costa 4,7 - piu' la circolarita' di `desc_minutes_next`, che legge `engine_pv_pred` nel proprio denominatore. E LA FAVICON: a 16 pixel era una STELLA a cinque punte, il commento del file dichiarava di aver gia' corretto quella figura, e il controllo automatico diceva «nessun problema» perche' misurava l'AREA - dopo la riscrittura l'area legge gli STESSI 142 e 38 con una figura tutt'altra. Cuciture ad archi tangenziali, invariante nuovo sulla CONNESSIONE (7 macchie contro 1), sette varianti guardate a 16px e l'SVG aperto in un Chrome vero, cosa che nessuna misura aveva mai fatto. 689 test toolkit verdi, `SHEET_REVISION` 44, `engine_*` fermo. Dettaglio: spec «Novita' v9.74», `letture-app-v1.md` §27-§28)** · precedente: 5 settembre 2026, sera (UNA PROMESSA SI RICONOSCE DALLA MAGLIA, NON DAL PREZZO - da tre domande dell'operatore su Palestra e da una sua istruzione che ha deciso la taratura. Nessuno lo considerava un top e nemmeno noi: Qt.I 3 = 145o difensore su 232, il motore non lo prezzava affatto («only 7 votes of 15») e il ripiego dava est_pv 16,5 su 38 col gradino `riserva`; finisce 4o per fantapunti e 2o per FVM. I segnali PRE-ASTA esistono e sono troppo deboli - su 636 uomini e 6 stagioni il valore di mercato vale 1,44x, il rating 1,67x, l'`avg_x` 2,67x ma su 2 casi di 9, e l'ESSERE GIOVANE va al contrario (0,25x) - mentre il segnale forte sono le GIORNATE GIA' GIOCATE: quotato <=5 e titolare in tutt'e due le prime = 19,3% contro 3,5% (5,46x), 6 stagioni su 6, e non e' l'effetto delle promosse. Il marchio ESISTEVA (`starter_signs`, 14/08) e aveva due buchi: taceva prima della 4a giornata (zero righe su 602, per costruzione, cioe' proprio nella finestra in cui si compra) e si disegnava SOLO in /auction. Ora sono QUATTRO letture su un'icona sola - `preseason` 1,64x, `rising` 1,78x, `early` 1,86-2,09x, `yes` 2,26x - e la cascata e' il «consolidare o ripensarci» richiesto. Il pavimento della fascia e' stato TOLTO e non era una concessione: allo sweep costa precisione zero e COMPRA lift (30 -> 1,93x, 0 -> 2,26x, monotono, +7 uomini a stagione). L'istruzione «tarare i limiti perche' Palestra rientri» non e' allargare un criterio caduto - quella regola riguarda l'ADOZIONE di una regola, qui e' la POPOLAZIONE a essere ridichiarata - e il prezzo e' detto: 1,64x con lui contro 1,86x senza. Verificato CHIAMANDO la funzione sull'agosto 2025: 56 marchi su 663, Palestra dentro, e quella lista realizza 39,3% contro 26,2% (1,50x). Le AMICHEVOLI non sono un grilletto per COPERTURA e non per merito - 20 club su 20 solo per il 2026-27 contro 2 e 4 nelle due stagioni prima - quindi viaggiano nella frase e la misura e' pre-registrata per l'estate 2027. 703 test app + 683 toolkit; la meta' TOOLKIT non e' committata perche' `snapshot.py` porta anche il lavoro in corso dell'altra sessione. Dettaglio: `letture-app-v1.md` §26)** · precedente: 5 settembre 2026 (DUE PARTITE NON SONO UNA STAGIONE - da cinque correzioni dell'operatore su nomi concreti, e l'audit che ne e' nato ha trovato che 313 righe su 358 avevano il gradino di titolarita' in disaccordo col proprio Pa: le `bandiera` promettono >90% delle partite e la loro mediana leggeva 0,58, i `riserva` 0,50, cioe' la scala non ordinava piu' niente. Causa: TRE quantita' sulla stessa domanda su TRE campioni diversi - il gradino leggeva le DUE giornate giocate, il claim lo standing regredito, il Pa la stagione scorsa - perche' `measured_season` commutava contando 10 giornate di CINQUE campionati contro una soglia di 5 pensata per uno. Cura: `presence.blend_seasons`, una miscela con ogni finestra al PROPRIO denominatore (che cura da se' l'errore di unita': Douvikas diviso per 2, Kean per 38, stessa colonna) e il peso NON scelto qui - 10 giornate di prior e' la K che il gate ha adottato per R20 su default, 6 su euro. Giudice esterno (`press --against press`): uomini 137 -> 153 su 220 contro un null di 104, moduli 10 -> 8 MATCH, prezzo detto. E il CALENDARIO: il foglio prezzava 38 giornate quando ne restavano 36, perche' `matchday_count` conta quelle gia' in archivio - ogni Pa e ogni surplus gonfi del 5,6%. Meta' APP: `core/expected-play.ts`, UNA formula per plancia e strategia - il metro della PLANCIA dove il motore ripiega sull'ancora, la finestra dell'infortunio aperto (un fatto) e l'ASSICURAZIONE dell'operatore (un rischio), misurata: 533 quotati e tre stagioni, in media 4,93 giornate perse a stagione e p75 sette, e chi ha due stagioni di storia ha media 8,4 e massimo 13,7 - la stagione brutta costa 1,63 volte quella media, e si sottrae lo SCARTO perche' il Pa contiene gia' lo sconto medio. Piu' il filtro del listone sulla strategia (Cheddira non e' quotato affatto: 70 righe su 602 entravano dal foglio) e un errore di unita' MIO trovato il giorno dopo da una sua domanda - il fattore divideva per la base della board invece che per la pv del foglio, e Kolo Muani passa da #27 a #16; piu' due sue correzioni su nomi - Malen a 14 Pa perche' l'assicurazione gli leggeva un'operazione al ginocchio del 2019 (finestra: tre stagioni COMPLETE, come `fragilityOf`), e Diao a 11 che invece e' VERO (20,4 giornate perse l'anno scorso) ma ha trovato la stagione IN CORSO che entrava come un anno sano, cioe' regalava a tutti un'annata perfetta e gonfiava lo scarto su chi sta male da sempre. `SHEET_REVISION` 41, `--verify` 22/22, 671 test toolkit + 661 app, otto banchi e2e)** · precedente: 4 settembre 2026 (DUE TAGLI DELLA PLANCIA - slot MERCATO e slot PERSONALI, questi ultimi ritagliati per la MIA max offerta, col TETTO che NON si ricalcola sulla griglia nuova perche' la scala e' misurata su uno slot definito come rango PER PREZZO e rileggerla altrove sarebbe un parametro fuori popolazione piu' una circolarita'. Cinque correzioni sue, quattro su cose spedite lo stesso giorno: (1) «togli Bernabe e Casadei» fatto e poi RITIRATO da lui - la causa stava nell'INCHIOSTRO e non nella lista, un fatto da una giornata su 36 disegnato come una cancellazione, quindi il barrato ora dice «infortunato di lunga data» e passa da 12 righe a 2; (2) «Hojlund prima di Martinez» - la discesa si rompeva solo sulle righe di chi e' di qualcuno, perche' la colonna porta due significati e l'ordine usava il tetto: UNA COLONNA PUO' PORTARE DUE SIGNIFICATI SOLO DOVE NON E' ANCHE LA CHIAVE DELL'ORDINAMENTO; (3) «i calciatori della squadra precedente restano accesi» - era una MIA eccezione, e il banco era cieco per costruzione perche' nei filtri avevo scritto la stessa eccezione della pagina: un'asserzione che porta dentro di se' l'eccezione che dovrebbe provare non puo' fallire su quell'eccezione; (4) l'ink delle righe accese, grigio perche' «di un altro» e' smorzato di proposito; (5) il «bordo sinistro blu» del toggle, che erano DUE blu di antd - il divisore `::before` e l'alone del fuoco, il secondo invisibile in ogni screenshot perche' esiste solo dopo un click. Piu' la LENTE su una rosa (i suoi acquisti in chiaro, il resto al 30%) e una review a effort alto: 14 rilievi, 12 corretti - `opacity` si MOLTIPLICA lungo l'albero, quindi le righe accese leggevano al 50% dentro i blocchi esauriti - e UNO SBAGLIATO nella sua conseguenza, misurato: togliendo la registrazione dell'icona, 73 icone e 0 vuote, perche' ng-zorro ne ha una lista di default. 651 test app, nove banchi e2e verdi, `engine_*` e i fogli fermi) · precedente: 4 settembre 2026 (UNA DATA DI RIENTRO E' UN NUMERO - dal caso McTominay, e la risposta era in archivio due volte: `injuries.end_date` porta la data di rientro STIMATA da sempre e l'app la stampava nel tooltip senza che una cifra la leggesse, e gli articoli che l'operatore leggeva a mano sono la riga di prosa della pagina *indisponibili* che scarichiamo ogni giorno e che `upsert_availability` buttava. Ora si contano GIORNATE del suo club e non giorni, col denominatore da oggi, e la quota e' LINEARE per misura: il rodaggio nelle presenze non esiste (-0,002 +- 0,006 su 2872 rientri). Quattro cose DICHIARATE su sua richiesta e tenute separate dalle misure: `RETURN_SLIP` 0,25 (l'azzardo crescente e' misurato - da 7 giorni ne restano 14 di mediana, da 120 ne restano 62 - ma l'entita' no, perche' la previsione non viene archiviata), `MIN_PLAY_SHARE` 0,60 (chi torna a gennaio non entra: esce la RIGA e non il posto, cosi' la numerazione degli slot non si muove), `BET_SHARE` 0,70 col tetto di 20-30 crediti, e `HURT_SLOT_STEP` (chi e' infortunato oggi si paga come lo slot sotto, con o senza una data - legarla alla quota sarebbe un premio all'ignoranza). Piu' `est_confidence` che entra nel TETTO e non nell'ordine: la plancia era l'unica a ignorarla, quindi offriva su una costante con l'autorita' di una misura (Mora 70 -> 35, Pulisic invariato perche' il suo 19,1 e' misurato). E IL CANALE NUOVO: `fc_site.parse_return`, stretto perche' sulla stessa pagina un'ancora di mese sta anche sull'inizio dell'assenza, sul giorno dell'operazione e su quello di un controllo - 23 date su 45, che CORREGGE il «42 su 45» che avevo pubblicato, e concordi con Transfermarkt a +1 giorno di mediana su 15 uomini, con 8 che TM non data affatto. Il rodaggio c'e' ed e' nei MINUTI (-20,2' alla prima presenza, voto 80,8% contro 90,8%) e vale 1,8 fantapunti su 140: misurato e NON implementato. 665 test toolkit, 635 app, tre banchi e2e verdi, `engine_*` e le revisioni ferme)** · precedente: 4 settembre 2026 («FACILE» E' UNA FRASE SUL CALCIO, e la card di un calciatore - la sessione parallela: `EASY_MARGIN` 200 -> 75 in due sue decisioni, il campo rifittato sulla PORTA INVIOLATA, i gol delle ultime dieci come terzo predittore, e la card trascinabile; il suo verbale sta nella sezione qui sotto) · precedente: 4 settembre 2026, notte (L'ASTERISCO DEL LISTONE - l'operatore ha visto Lukaku nello slot A3 della plancia e la sua correzione vale piu' della segnalazione: «non deve essere tolto per la nota "fuori rosa" ma perche' non gioca piu' in serie A». Il dato c'era e lo fondevamo da anni: il file delle quotazioni ha un foglio `Tutti` e un foglio `Ceduti` - sul sito l'asterisco accanto al nome - e `parse_listone` li leggeva tutt'e due e li univa, con una ragione buona (un ceduto ha comunque giocato e i suoi voti vanno attribuiti) e una conseguenza mai notata: «si puo' ancora comprare?» non stava in nessuna colonna. `listone_quotes.sold` con migrazione, backfill offline da 21 file di cache (16.533 righe), fatto PER PIATTAFORMA come il prezzo e il club - 57 ceduti in Serie A, 79 su euro, e sette sono ceduti in Serie A e comprabili su euro. `SHEET_REVISION` 40: 36 righe fuori dal foglio Serie A, 36 dal mantra, 48 da euro, bundle rigenerato, zero asterisco sopravvissuto sui tre fogli. La prima cura era la ragione sbagliata - far RIFIUTARE l'uomo dalla nota dichiarata - ed e' stata RITIRATA: fuori rosa e' uno stato dentro un club, chi ha lasciato il campionato lo dice il listone; la nota e' tornata a essere un'icona. Piu' le ICONCINE sulle righe della plancia (due per riga, taglio in coda e `+N` detto) e il MENU' che le accende - `core/flag-prefs.ts`, preferenza sola per tutta l'app, tiene la lista degli SPENTI cosi' un marchio nuovo nasce acceso. Misurato in browser: 62 icone su 55 righe, menu' 23 voci, «nessuno» -> 0, «tutti» -> 62, spegnendo `fragile` -> 25, e dopo il reload restano 25. 603 test app, 650 toolkit, `engine_*` fermo. E due zeri da CHIAVE SBAGLIATA commessi nella stessa ora - `row.get` su righe che sono liste, e `source='squad'` che non esiste - da cui la regola: prima di credere a uno zero, stampa la FORMA di cio' che stai leggendo)** · precedente: 3 settembre 2026, sera (IL CLUB DEL LISTONE E' UN FATTO DI PIATTAFORMA - quattro difetti con una radice sola, trovati dall'operatore con tre nomi di portieri. `listone_quotes` aveva la piattaforma per i prezzi dal 07/08 e il CLUB era rimasto in `rosters`: 221 righe su 289 col prezzo EURO, la `league` congelata al primo valore da un COALESCE al contrario, cinque quotati Serie A fuori dal perimetro. E il quarto, una regola giusta fuori dal suo dominio: chi la fonte vede in un club ESTERO cadeva fuori da ENTRAMBI i rami della popolazione, 35 quotati assenti di cui 7 titolari di oggi (Woltemade 23, Beto 14). Essere quotato su un listone e' prova POSITIVA di essere in quel campionato; il 17/08 riguarda l'ASSENZA. Assenti 35->22 (i 22 giusti, tre fonti concordi), club smentiti 8->0, migrazione + backfill, lettori inerti sullo storico. Piu' la pagina probabili che si cancellava da se' (la lettura euro sovrascriveva le righe Serie A dello stesso giorno: 479 su 20 squadre -> 250 su 10). 643 test + 1 skip, `--verify` 22/22, `engine_*` e le revisioni fermi. La sessione era nata per una ROSA da 3 giornate - 250 crediti sulla Qt.A, regolamento in sei messaggi - e ha prodotto sette misure nuove: il +3 del modificatore vale lo 0,02%, i voti dei difensori sono asimmetrici a SINISTRA, la correlazione dentro il club non aiuta perche' la scala e' localmente lineare, di un club gioca esattamente UN portiere, e la sua terna di tre PRIMI portieri di tre club batte il trio di un club solo)** · precedente: 3 settembre 2026 (IL TETTO DI UN'OFFERTA E' UNA QUOTA DEL BUDGET, DIVERSA PER RUOLO - una sessione di sole domande al tavolo, e sei conclusioni misurate. LEGGERE I RIVALI NON PAGA: quattro forme provate CON UN ORACOLO che legge il massimo dei rivali prima di offrire, tutte respinte (-7,6% il prezzo ombra, -2,7/-4,9% rilanciare per prendere il lotto, -25% riallocare sul buon affare), perche' a secondo prezzo il tetto e' quasi gratis e il bilancio e' gia' impegnato - e il NULL ha battuto il canale: «hands sui crediti dei rivali» +2,0% contro «abbassa hands leggendo niente» +4,8%, quindi era PAZIENZA e non informazione. ADOTTATO `DEPTH_TIER`=2 e `DEPTH_HANDS`=9 (+1,88% strict su 800 stagioni, t 11,6, +1,54% con tre bracci, inerte a chiamata): il braccio lascia passare la profondita' a tavolo aperto, e non e' un risparmio - sono 1,6 TITOLARI IN PIU' a rosa, e fa valere `INSIGHT` +29,9 -> +37,0, cioe' i due canali si COMPONGONO. NUOVO GIUDICE `bench.auction.advice` che il tavolo non lo usa: dentro uno slot la quotazione vale -1,0 contro un dado e il motore +18,1 (t 5,3, 10 stagioni su 10), ma su una ROSA fanno +26 a stagione perche' ne schieri undici e la copertura satura. E i tetti: un credito vale 0,0055 punti a giornata, la soglia e' il 18% del budget invariante a 500/1000/2000, portieri 13% (il mercato chiede il 6%: si compra), DIFENSORI nessun prezzo, attaccanti 18% contro il 25% chiesto - e con due slot di profondita' esauriti il tetto sale al 32%. Piu' la Qt.A che entra nella serie datata (`fvm_history.price`, era uno stato volatile in un campo fisso) e la PLANCIA A SLOT decisa. `engine_*`, i fogli e il bundle fermi, 54 prove del banco)** · precedente: 2 settembre 2026, notte tarda (IL VANTAGGIO INFORMATIVO PAGA, E PAGA DENTRO LA FASCIA - la richiesta era «migliorare l'engine che consiglia le offerte per un'asta random», e la diagnosi non era in nessuna lista: all'urna il braccio motore non leggeva UN SOLO numero del motore. Contato invece che dedotto dal codice - `engine_worth`, `cover_value`, `coverage_need`, `alternative` e `role_cap` fanno 1436 chiamate a chiamata e ZERO a estrazione - perche' l'adozione della scala di mercato manda il braccio sul ramo umano, che prezza `richiesta x passo(fascia) x scala` con la fascia definita dal PREZZO. Una fascia e' larga dieci uomini: dentro una fascia il prezzo varia dello 0,08 della sua mediana e le presenze attese dello 0,33 del calendario, cioe' dodici giornate. E la quantita' non e' una scelta - il nostro vantaggio incrementale sulla quotazione e' largo un numero solo (`pv_pred | Qt.I` +0,243 su Serie A contro il -0,077 del surplus). `INSIGHT` 0,80 sulla magnitudine, che CONSERVA per costruzione (la media della deviazione su una fascia piena e' zero): **+1,02% robust su 800 stagioni**, appaiato +26,4 +- 4,5 con t 5,9, 9 finestre su 10, buchi 22,9 -> 18,8, posto 4,20 -> 3,52, titoli 175 -> 257 su 800 - e a differenza del margine della scala **sopravvive a tre bracci** (+30,0, t 7,4), perche' non alza un'offerta: sposta gli stessi soldi dentro una fascia. E' la prima cosa che questo banco trova che paghi per la nostra OPINIONE invece che per il modo in cui offriamo. Piu' l'etichetta STRICT ritirata dal campione doppio (una finestra su dieci era una monetina), tre cure misurate e respinte, e i portieri come contro-esempio del §19.1: la loro meta' del TILT valeva niente, la loro meta' di QUESTO termine vale +7,4 punti su 32,9. 47 prove del banco, 620 del toolkit, `engine_*` e i fogli fermi, a chiamata nemmeno un decimale. `682343b`, pushato con `master`)** · precedente: 2 settembre 2026 (REVIEW DEL QUINTO BANCO: otto rilievi, cinque in codice o documentazione appena spediti e tre trovati MISURANDO. Il piu' grosso penalizzava il braccio giudicato - il pavimento «non tenere crediti» era tagliato dal tetto di reparto, quindi il motore teneva 67 crediti mentre il tavolo ne teneva 0-20: togliere il taglio vale +6,7 punti, spesa 933 -> 988, posizione 1,90 -> 1,70, e in campionato 62,1 punti con posizione media 1,70. Il «+2,4 di R-Factor per un uomo» NON si riproduce ed e' +1,5, mentre il compagno +10,8 si riproduce esatto - che e' quello che localizza l'errore nel quantile e non nel meccanismo; e la costanza passa da «costa 8,2 punti» a INERTE, perche' a peso 0, 1 e 5 il braccio legge la stessa cifra e il tetto di reparto morde prima. Piu' il commento spedito nell'app che diceva 27 punti dove sono 18,8 per un undici non comprabile, due tabelle su calendari diversi affiancate senza dirlo, un budget che meta' della classe ignorava, e `bars_for` che regge solo sul CASO delle lettere - `a` = ala contro `A` = attaccante - ora puntato da un test. 594 test del toolkit, 541 dell'app)** · precedente: 1 settembre 2026 (IL QUINTO BANCO, E UNA CLASSIFICA CHE NON E' LA GRADUATORIA DEI PUNTI - `bench/auction` giudica le STRATEGIE d'asta a rilanci e la sera stessa un campionato a 36 giornate sopra di esso; il prezzo del top EMERGE dal secondo prezzo (48-75% del budget) e il costo di un buco e' misurato (4,73), quindi il valore di un uomo ha DUE termini che si sommano - surplus + copertura - e la prima versione col solo surplus fini' ultima di undici. Col calendario esce la quantita' che la media stagionale non poteva vedere: la scala dei gol tronca a 66, quindi in 4 stagioni su 10 il campione non e' chi fa piu' fantapunti, e i punti in classifica correlano -0,802 con le giornate buttate contro +0,815 coi fantapunti - la copertura paga DUE volte. Diversificazione fra club adottata (costa 1,6 punti, taglia il 9% della dispersione), COSTANZA misurata e messa a ZERO (un uomo muove l'R-Factor di 2,4 punti l'anno, e il modificatore lo governano i buchi: r = -0,821), pareggio d'offerta che si rompeva in ordine alfabetico. `engine_*` e i fogli fermi, 593 test. La meta' APP - la colonna Costanza - NON e' committata: quei file li ha editati anche un'altra sessione)** · precedente: 27 agosto 2026, sera (LE LISTE SI RIORDINANO A MANO, E I MODULI MANTRA VALGONO L'1% - l'ordine personale e' un PREFISSO (i nomi che ha sistemato in cima, sotto continua il gain) perche' salvare la lista intera metterebbe un arrivo nuovo prezzato 40 sotto ottanta difensori; gesto verificato contando gli eventi che ARRIVANO (pointermove 9 su 9), preferenza che sopravvive al ricaricamento, 533 test. E la valutazione dei moduli chiesta dall'operatore: tutti e undici schierano 5 difensivi + 5 offensivi - lo dice il rulebook, che rifiuta i modificatori classici perche' «gli schemi sono gia' bilanciati» - mentre sui posti da BONUS vanno da 3 a 5, e il tetto in punti fra il primo e l'ultimo modulo vale l'1,15%: quello che cambia e' il PREZZO, con il surplus per credito che scende da Por 44 a Pc 8,5. Piu' una collisione fra due sessioni sullo stesso file, curata con un worktree)** · precedente: 27 agosto 2026 (LA PAGINA STRATEGIA, E UNA LISTA PER RUOLO CHE NON PUO' ESPRIMERE UN VINCOLO CONGIUNTO - `/strategy` dichiara il regolamento della lega e disegna un blocco per ruolo coi migliori nomi: il GAIN e' il surplus a rilanci e il VALORE in un draft (misurato, non scelto), e la lunghezza di ogni lista e' la domanda della stanza - slot x partecipanti su classic (8 difensori x 8 = 64), le quote delle forme su mantra (Dc 51 - M 27 - C 26 ... B col pavimento a 10). La regola dell'operatore sul «posto piu' arretrato» e' vera nel problema che descrive (15 dei primi 16 trequartisti possono giocare da C) e RESPINTA in tutt'e due le forme che la applicano: il filtro lascia i braccetti a ZERO nomi, il riordino porta la somma dei gain di quel blocco da 256 a MENO 9 - quindi marca e non filtra, con la lettura letterale a un clic. `engine_*` e i fogli fermi, 509 test dell'app, `70c8ed6`)** · precedente: 25 agosto 2026 (UNA SECONDA FONTE PER LO STESSO FATTO - i minuti di un campionato che copriamo li porta anche Transfermarkt dove l'aggregato di stagione ha un buco: stessa quantita', stesso denominatore, stessa retta, ZERO parametri nuovi, +6,0% fuori campione, e l'equivalenza delle due fonti misurata su 10.580 coppie (differenza mediana +0,0000, correlazione +0,9957). E il caso da cui la domanda e' nata - Varela, 34 partite di Primeira Liga - e' RIFIUTATO per misura: fuori perimetro la retta perde -6,9%, perche' non ha un termine di livello. `SHEET_REVISION` 37, `engine_*` fermo, --verify 22/22, 550 test)** · precedente: 20 agosto 2026, tarda sera (LA TITOLARITA' IN UNA PAROLA, E LA PAROLA STESSA DEFINITA - sei gradini dettati dall'operatore, letti come DUE assi (quota di partite + pavimento di minuti) e non come una probabilita' congiunta, che e' stata costruita per prima e misurata inservibile; la quota e' CONDIZIONATA e la board e' un CANCELLO, che e' anche il classificatore migliore (q75 0,512 contro 0,328 a parita' di claim); resa misurata su quattro finestre retrodatate, `bandiera` e `titolare` mantengono la promessa 4 volte su 4. `SHEET_REVISION` 35, `engine_*` fermo, colonna `Tit.` a tre caratteri in tabella, v0.1.21. E il termine definito dall'operatore: titolarita' = prende il voto, quota da titolare = parte dall'inizio, con la bonifica del codice e una nota datata sui verbali storici)** · precedente: 20 agosto 2026, sera (IL REPARTO IN CUI ARRIVA — R23 adottata (robusta 9/10 su Serie A, 5/5 su euro/mantra, cade su euro/classic), R22 respinta sulla sua stessa aspettativa pre-registrata, e la fee in rapporto alla spesa del club adottata su euro; più il difetto che l'adozione ha reso visibile: `minutes.start_rate_next` divideva il numeratore del PANNELLO per il denominatore del MOTORE, quindi una regola che alzava le presenze ABBASSAVA i minuti previsti a partita — corretto su euro, +1,44% su 4 finestre su 4; e la revisione dei pacchetti del viaggio nel tempo, che c'era e veniva cancellata in transito. `SHEET_REVISION` 34, `--verify` 22/22, 510 test del toolkit e 318 dell'app)** · precedente:20 agosto 2026, notte (LA MVa ERA LA METÀ DERIVATA DELLA COPPIA, ED ERA LA METÀ SBAGLIATA — trovata dall’operatore su due righe del foglio classic: `est_mv` era `engine_fm_pred` (già regredita) meno il suo tasso di bonus GREZZO, quindi tutta la regressione finiva sul voto base e Malen leggeva 5,67 contro i 6,75 misurati. Adesso la MVa è la metà PREVISTA e il tasso è quello che ne esce; tre parametri fuori campione con cross-fit UNANIME, un canale rifiutato come zero dichiarato, e la richiesta dell’operatore («chi segna ha sempre o quasi un voto buono») che era essa stessa una misura: r = +0,787 dentro il ruolo. `SHEET_REVISION` 32, `engine_*` fermo, v0.1.20 pubblicata)** · precedente: 20 agosto 2026 (il grafico di Fπ non si vedeva MAI: una `computed` che scriveva un signal — vietato in Angular — e una suite dell'app che non compilava da `d7d0fbf`, quindi le prove dichiarate ieri non venivano da una corsa; 317 verdi)** · precedente: 19 agosto 2026, sera (Fπ: una colonna che PRONOSTICA invece di sommare — tre parametri misurati fuori campione, la scala dettata dall'operatore in cinque passaggi, quattro idee respinte, e otto club stranieri archiviati come `serie_a`; `SHEET_REVISION` 31, v0.1.19 pubblicata)** · precedente: 19 agosto 2026 (un vecchio PV non è una previsione di presenze: il gradino `older` regrediva la fantamedia e consegnava le presenze intatte, e l'Overall dell'app è un PRODOTTO — `SHEET_REVISION` 29; e l'attesa sul lock del DB in una definizione sola, con la regola per due sessioni in parallelo)** · precedente: 18 agosto 2026 (le DEFINIZIONI dell'operatore — Overall, Lead, Margine, Bonus — applicate ovunque; UN campetto solo per asta e Squadre, con l'item-posto e i moduli alternativi scritti dal toolkit; tre difetti della tabella misurati in browser)** · precedente: 17 agosto 2026, notte (la coppa continentale in mezzo al campionato: misurata, sul foglio e RESPINTA dal gate; l'app riscritta a voce; il surplus che aveva due aritmetiche)
