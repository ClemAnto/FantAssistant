...
LE DUE BARRE ERANO GIA' `fixed`: a occupare spazio era la PAGINA (`pb-14` sulla plancia, `pb-11` sull'ultima lista della Strategia). Tolte: la griglia riprende **48px** (a 900px di finestra la plancia passa da 727,7 a 775,7px) e sta intera fino a ~660px invece che ~710. **«Non occupa spazio» non e' `position: fixed` scritto in una classe**: e' che il contenuto arrivi SOTTO la scatola, e il banco lo misura contando quante righe stanno nel suo rettangolo — con la riserva non ce n'era nessuna.
`ui/bottom-dock` e' UNA cornice per tutte e due (erano due copie della stessa) e la regola e' **si nascondono i CONTROLLI, mai gli ALLARMI**: a sinistra sopravvivono la freschezza e le squadre escluse (525×42 → 123×38px, cinque bottoni → uno), a destra la data **solo mentre si viaggia** (386×42 → 42×35). Lo stato e' per ANGOLO, in `localStorage`, e sopravvive al ricaricamento; aperte all'inizio, perche' una barra che nasce piegata e' una funzione che nessuno trova.
IL PREZZO, DICHIARATO: da aperte le barre coprono **12 righe la sinistra e 4 la destra su 249**, e li' un click arriva alla barra. Non e' un difetto da nascondere, e' la conseguenza della richiesta: il rimedio e' la freccia. L'ha trovato `e2e-plancia-injury` cliccando proprio una di quelle righe; adesso piega le barre come farebbe l'operatore e STAMPA quante ne coprivano. Su Strategia il passo «la coda» e' stato RIBALTATO con data e ragione (era il guardiano del `pb-11`): pretende adesso che piegando la scatola il nome si scopra, e cosi' e' — resta coperto il bordo destro della riga, cioe' il gain.
DIFETTO TROVATO PER STRADA E CURATO ALLA RADICE: riaperta dopo un collasso, la barra leggeva **541px invece di 525**. Dentro c'erano due figli in piu', `<nz-tooltip>` e `<nz-popconfirm>`: elementi OSPITE che antd crea e stacca dal DOM, che Angular continua a contare fra i nodi della vista e reinserisce quando una vista si riattacca — larghi zero e con il loro `gap` da otto. **E' la stessa causa che il 20/08 si mangio' una colonna della griglia dell'intestazione della tabella**, curata li' spostando il tooltip; qui `nz-tooltip, nz-popconfirm { display: none }` in `ng-zorro.css`, verificato facendo girare i QUATTRO banchi che i tooltip li aprono davvero. E il numero era nel `said` di un passo VERDE: *un numero stampato accanto a un verdetto verde va guardato lo stesso.*
DUE BANCHI ACCUSAVANO LA PAGINA DEL PROPRIO DIFETTO, tutt'e due per un selettore STRUTTURALE (`ui-global-options > div`, `ui-time-machine div`) che il wrapper ha spostato: ora si agganciano all'ATTRIBUTO che la scatola dichiara (`[data-dock]`). Stessa famiglia dell'`header span:nth-of-type(2)` del 07/09.
VERIFICATO: build pulito, **1053 test su 61 file**, **12 banchi e2e verdi su 14**, e due controprove rimettendo il difetto (col template di HEAD cadono 4 asserzioni del passo nuovo e nessun'altra; con l'`always` dentro l'`@if` cade il test della regola del collasso e nessun altro). **Una nota di misura**: lanciando la suite unitaria INSIEME a dodici browser headless 4 test su 1053 fallivano, e da sola e' verde — *misurare mentre l'ambiente si muove non e' misurare*, stavolta con la macchina a muoversi e non il codice.
APERTI: (a) **`e2e-why` e' rosso e non e' di questa sessione** — la pagina legge 26,4 presenze dove il foglio dice 22,9, cioe' ×38/33, e `season-scale.ts` dichiara da se' che su `/why` il riporto NON si applica; sta in `ValuationStore` dentro `EngineExpectation`, e la cura e' una decisione dentro quella feature (o `/why` divide per la scala, o l'interfaccia porta anche il numero grezzo). L'identita' che la pagina verifica sopravvive al riporto, ed e' per questo che nessuno se n'era accorto. (b) **Le 16 righe coperte** sono una sua decisione: le alternative misurabili sono far reagire al puntatore i soli controlli (recupera poco: la barra e' quasi tutta bottoni) o tenere un margine pari alla sola freccia (~35px invece di 56). (c) `e2e-player-card` resta il rosso preesistente gia' verificato su HEAD. (d) In una corsa in batch `e2e-nav` ha stampato 2 problemi (il titolo di `/clubs`) e cinque corse successive sono verdi: non lo do per spiegato.
· precedente: **Aggiornato: 22-23 settembre 2026 — OGNI NUMERO IN GIORNATE SU UNA STAGIONE PIENA, e «35 presenze» non e' la previsione per l'anno dopo.** Da una domanda dell'operatore sui dieci attaccanti piu' cari: «hanno partite attese fra 20 e 26, e i migliori attaccanti dell'anno scorso ne hanno fatte fra 30 e 36 — come mai?». Dettaglio: `letture-app-v1.md` §50, spec «Novita' v9.101».
LA DIAGNOSI: lo scarto che sembrava del motore era per tre quarti ARITMETICA. Scomposto — **−2,9 la lista scelta ex post** (i «migliori attaccanti dell'anno scorso» sono selezionati DALL'aver giocato: su 11 stagioni la top-10 per FVM finale realizza 32,2 presenze, quella per Qt.I pre-asta **29,3**), **−3,9 il denominatore** (il foglio prevede le 33 giornate che RESTANO, non 38), **−0,5 il motore**, **−2,9 l'assicurazione**. Il FVM di una stagione passata e' l'ULTIMA LETTURA, quindi conosce l'esito: la contaminazione e' esattamente in quella direzione.
ADOTTATO (richiesta sua, e vale per tutte le pagine): «tutti i valori che leggiamo sulle pagine devono essere rapportati all'intera stagione ... anche se il mercato dura 3 giorni, su base 38 (sia partite attese che surplus)». **`core/season-scale.ts`**, una definizione e due punti d'ingresso. **«Base 38» ha DUE significati** — riscalare, o sommare le giornate viste — e divergono di ±2,6 (mediana 0,17): si adotta il **riscalamento**, perche' il SURPLUS un passato da sommare non ce l'ha e le due colonne della stessa riga devono stare sulla stessa base. **La base e' la stagione della PIATTAFORMA e non il numero 38**: su euro `platform_input` e' **31**. Si riporta quello che e' ESTENSIVO nelle giornate (presenze, surplus, valore, margine, coppa, finestra d'infortunio) e NON l'intensivo (fantamedia, minuti a partita, quote, SWING), i prezzi, ne' le MISURE — proiettare un gol gia' fatto trasformerebbe un fatto in una previsione. **La prova che la lista e' giusta e' l'identita' di `/why`**: `(fm − rimpiazzo) × pv × conf = surplus` regge, perche' le due meta' si riportano in modo diverso.
IL DATO C'ERA, e mancava il lettore: i due calendari il foglio li dichiara accanto alle righe e il caricatore faceva gia' `JSON.parse` dell'oggetto intero (`BundleTable.matchdays`). E **curava anche un rapporto gia' sbagliato**: `sealed-bid` dichiara `matchdays: 38` mentre `expected` viveva su 33, quindi la quota leggeva 0,63 dove la verita' e' 0,73. **Sette viste** leggevano `matchdays_target` per conto loro e ora chiedono a `ValuationStore.seasonRoundsFor()`.
IL NULL, dalla sua seconda domanda («Dimarco 35 → 29, Douvikas 36 → 28, Paz 35 → 30, Pulisic 27 → 17: troppo basse»), su **1.898 casi in 11 stagioni**: chi fa 26-38 presenze, l'anno dopo **il 17% non ha una riga di Serie A** e chi resta ne fa **26,4** di media (mediana 29, sd 9,2). Sui **165 quotati** di quella fascia il MOTORE prevede **26,2 contro 26,4 — in centro, a due decimi**; lo schermo leggeva 22,3. **Trentacinque presenze non sono la previsione per l'anno dopo: la previsione e' 26-29.**
SPENTO per sua decisione: **il pavimento dell'assicurazione** (`INSURANCE_FLOOR_ROUNDS` = 0, era 2,1). Una costante uguale per tutti non protegge da niente — **il 46%** dei titolari la pagava identica — e abbassa senza distinguere. Resta accesa la meta' che lavora, lo scarto fra la stagione tipica di UN uomo e la sua peggiore (fino a 13,3 giornate). **E il numero che avevo pubblicato era sbagliato**: avevo detto 24,4 sommando 2,1 a chi stava al pavimento, ma chi ci sta sopra guadagna solo la differenza col PROPRIO scarto — misurato **22,3 → 22,8**, e Dimarco non si muove di un decimo. *Un numero dedotto da una percentuale non e' un numero misurato: qui la differenza era un fattore quattro.*
VERIFICATO: **1048 test app, 931 toolkit, banco Strategia senza problemi**, e un'asserzione nuova **falsificabile e non circolare** — se il foglio prevede 33 e la stagione ne ha 38, un numero sopra 33 e' impossibile senza il riporto, e a schermo il piu' alto legge **34**. Prima della cura lo stesso banco segnalava **130 righe fuori scala**: e' la prova che il cambiamento era arrivato allo SCHERMO. Due controprove rimettendo il difetto, e cadono esattamente i test che descrivono le cure. **Il primo verde non aveva guardato niente** (1038 test passavano perche' nessun fixture porta `matchdays`, quindi la scala valeva 1 ovunque).
APERTO: (a) **lo schermo resta −3,6 dal null** ed e' ormai tutta assicurazione PERSONALE (3,4 giornate di media): e' prudenza per costruzione, quindi la colonna non e' la previsione neutra — spegnere anche quella lascerebbe il numero del motore, ed e' una decisione sua. (b) I **rossi preesistenti** dichiarati e verificati su albero pulito: due test di `press` che scadono con l'orologio e un riepilogo del banco della card (5,7 contro 5,5). (c) Il **manifest porta `matchdays_input` solo dopo un `export`** — non serve aspettarlo, la base si legge dal foglio.
· precedente: **Aggiornato: 22 settembre 2026 (sera) — LE SETTE PAROLE DELL'OPERATORE, E LA SCALA CHE C'ERA E NON SI VEDEVA.** Dalla sua richiesta di dare a ogni calciatore un'etichetta, con sei definizioni dettate a voce. Dettaglio: spec «Novita' v9.100», `letture-app-v1.md` §48, gate §7-unsexagies e §7-duosexagies.
PRIMA DI COSTRUIRE: **la scala c'era gia' dal 01/09 e non era MAI arrivata a schermo** — `snapshot` scriveva `desc_category`, l'app aveva vocabolario, colonna e filtro, e la colonna non era in `export.SHEET_COLUMNS`: vuota su tutte le 562 righe per tre settimane. La famiglia dei campetti e di `availability`, qui su una COLONNA. La richiesta nasceva da quel vuoto, e metterglielo davanti ha cambiato la domanda in una scelta sua.
ADOTTATO (`SHEET_REVISION` 72): **sette parole — super · top · semi · solido · riserva · scommessa · scarto** — su una quantita' NUOVA. **L'asse non e' il tasso di bonus**, provato per primo e respinto dai suoi stessi nomi (35 configurazioni, massimo 4 verdetti su 6), ma la **fantamedia attesa dentro il ruolo**, che ordina i suoi sei primi nomi esattamente come li ha etichettati. **Il livello si ri-miscela con una K PER RUOLO** (P 16,6 · D 32,6 · C 43,5 · A 18,5, leave-one-season-out su dieci stagioni): a cinque giornate la stagione in corso pesa il 21-23% per attaccanti e portieri e il 10-13% per gli altri, e **senza quella la scala non distingue `solido` da `riserva`** (Varela e Pinamonti hanno la stessa fantamedia sul foglio e finiscono in parole diverse; ri-miscelati distano 36 percentili). **Sbarre PER PIATTAFORMA** (la scala euro sta piu' in alto: mediana attaccanti 7,14 contro 6,61) e **una sbarra dentro un plateau si sposta** — su euro 80 portieri su 110 portano la stessa identica fantamedia, un fatto sul MOTORE che va saputo. `engine_*` non si muove.
NELL'APP: la categoria e' un **pallino colorato con l'icona del suo mestiere prima del nome** nella card (sua richiesta) e la **dodicesima pastiglia** in Strategia, che ordina per la scala. Il colore riusa la grammatica di `ui/gain-chip` — neutro al centro, **mai il rosso**, `scommessa` senza tinta perche' non e' un giudizio.
VERIFICATO: **929 test toolkit** (i 2 rossi sono i `press` preesistenti che scadono con l'orologio), **1038 app**, i **diciotto verdetti** dell'operatore riprodotti, e i conteggi delle sette parole pubblicati perche' una categoria vuota o troppo grossa si vede solo li'.
QUATTRO DIFETTI, E NESSUNO DA UNA RILETTURA: la **code-review** ha trovato le sbarre uguali per due piattaforme e le soglie di `squad-table` scritte sull'INDICE della scala (`rank >= 4` smorzava `scommessa`, con sette parole smorza `riserva`, meta' listone); i **CONTEGGI** hanno trovato `scommessa` **vuota per costruzione** (il foglio da' una fantamedia a tutti, quindi 147 uomini sull'ancora prendevano un giudizio: `bandiera` su una parola, da 0 a 83 righe); **lui guardando lo schermo** ha dato tre nomi con DUE cause diverse; il **compilatore** tre volte su cose che nel diff sembravano complete.
E DUE ERRORI MIEI: ho tarato una sbarra su un numero **arrotondato** due volte, la seconda dopo averlo scritto nel codice (Scamacca e' 0,718 e la sbarra a «0,72» lo tagliava); e **un `str.replace` senza `assert` e' un no-op silenzioso** — due test non sono mai entrati nel file mentre riportavo che c'erano, che e' il difetto peggiore perche' riguarda cio' che ho detto a lui.
APERTO: **R28**, la K per ruolo dentro il motore, da pre-registrare — questa misura e' sul predittore NUDO e un baseline piu' debole fa sembrare un canale piu' forte (la lezione di R24). E il ramo `top` non ha pavimento di presenze: un uomo fuori scala che gioca il 5% resta `top`, caso su cui lui non ha ancora deciso.
· precedente: **Aggiornato: 22 settembre 2026 — I TURNI GIOCATI ENTRANO, E UN AGGREGATO ERA CONGELATO ALLA PRIMA GIORNATA.** Dalla richiesta dell'operatore «aggiorna il db alla luce dei turni serie-a e euro giocati», e poi «risolvi le anomalie» sulla riga che `validate` stampava in coda a ogni corsa.
ACQUISITO: **Serie A md5** (20 club, 336 righe) e **euro md4** (37 club, 607 righe) coi voti, e il layer per-partita al **20/09 su tutti e cinque i campionati** — erano fermi al 14-17/09 perche' `update --daily` i voti non li rilegge per definizione e l'ultima corsa completa era del 16. Corsa MIRATA invece del piano intero (18h50 dichiarate a freddo): fuori gli archivi settimanali (`injuries`, `market`, `performance`, `recent_form`), `transfers` e i layer `complete`/`extra`/`heatmap`; dentro voti, layer per-partita, le quattro letture di oggi, la catena di derivazione, i tre fogli, il bundle e la copia dell'app. Il resume di `ratings` ha esercitato **per la prima volta sulla rete** la cura di v9.97 (salta le giornate COMPLETE, si ferma al primo foglio vuoto), il che chiude l'aperto del 16/09. **Il denominatore lo prova**: le giornate da prevedere passano a **33 = 38 − 5** su Serie A e **27 = 31 − 4** su euro, cioe' i turni sono entrati nel MOTORE e non solo in archivio.
CURATO: **un aggregato DERIVATO si ri-deriva quando i suoi voti si muovono** (`e9691f5`, `stats.derive_from_ratings`). Scriveva solo dove la riga NON esisteva, quindi una riga creata a stagione in corso restava congelata al conteggio di quel giorno e nessuna corsa poteva piu' toccarla: tutta la stagione bersaglio leggeva `pv` in **{0, 1}** su ENTRAMBE le piattaforme — **574 righe euro e 319 default che coincidono con i voti della GIORNATA 1 esattamente, il 100%**, contro il 16% che coincide coi voti totali — piu' 38 righe di due stagioni chiuse con lo stesso congelamento (fc_id 7287, euro 2025-26: `pv` 5 contro 22 voti, con le due Mv d'accordo al centesimo). E' «un derivato stantio e' peggio di uno vuoto» (v9.73) applicato a un aggregato invece che a una colonna. **La condizione e' «i voti ne contano DI PIU'», mai «i voti non sono d'accordo»**: meno voti vuol dire che la nostra copertura e' parziale e il listone sa di piu' (111 righe di euro 2024-25), e li' il listone resta autoritativo — che e' quello che `check_ratings_consistency` gia' diceva saltando quelle e segnalando solo l'altro lato. Effetto: anomalie **653 → 0**, copertura completa 5844 → **6497** (tutti Mv- e FM-consistenti), `pv` 2026-27 da {0,1} a **0-5** su default e **0-4** su euro.
VERIFICATO: **`backtest --verify` 22/22 reproduced** (blocco letto per intero e non troncato), **918 test toolkit**, bundle riscritto (506.823 righe) e consegnato all'app. La cura e' stata misurata PRIMA su una **copia privata del DB** e la copia ha predetto il vero **identico**. Controprova fatta rimettendo il difetto: cade esattamente il test nuovo e nessun altro.
DICHIARATO: **i numeri del gate si muovono, di poco** — `beta_mantra_T2` 0,446 → 0,444 (367 → **369** coppie), `pv_gain_vs_naive_T1` 0,0211 → 0,0197, `pv_gain_crossfit_T2` −0,0209 → −0,0194, `pv_bias_naive_starters_T2` 5,47 → 5,50. Ogni controllo resta dentro la sua tolleranza. **Non e' una regola che peggiora, e' un dato che cambia**: il confronto non e' modello contro modello, sono 38 righe che dicevano una cosa falsa. Se i valori `expected` vanno riallineati e' una decisione dell'operatore, non una conseguenza di questa cura.
APERTO: due difetti trovati strada facendo e **non** toccati perche' fuori dalla richiesta, con la diagnosi gia' fatta. (a) **Due test di `press` scadono con l'orologio** — `test_the_taker_and_the_reader_agree_on_the_format` e `test_a_club_that_does_not_play_is_left_out...` fissano una partita al 2026-09-18 e chiedono una presa FUTURA, ma oggi e' il 22: il test vicino passa un `today=` esplicito, questi due no. Verificato mettendo da parte la mia modifica: **cadono lo stesso**, quindi sono preesistenti. Una riga per test. (b) **`positions` stampa `stop at round N` e non si ferma**: fa `continue` e cammina fino a fine calendario. La sostanza e' giusta (un turno vuoto puo' essere un rinvio, e i successivi essersi giocati — «l'unita' e' la PARTITA, mai la giornata»), ma la parola dice una cosa che non succede e il prezzo e' ~31 richieste a vuoto per lega sui turni futuri, ~7 minuti a corsa.
· precedente: **Aggiornato: 16 settembre 2026 — L'AUDIT DELLE FUNZIONI DI SURPLUS: il calcio giocato entrava in META' della catena.** Dalla richiesta dell'operatore «adesso abbiamo formazione a breve termine e formazione stagionale tipo … analizza le varie funzioni surplus e similari per vedere se e' tutto ok». L'aritmetica regge — l'invariante `(fm − rimpiazzo) × pv × confidenza` riproduce le due colonne su **1.533 righe motore e 2.075 stimate, 0 fallimenti** — e il calcio di quest'anno entrava in una colonna si' e in quella accanto no.
ADOTTATO: (a) **`est_fm` e `est_mv` leggono le partite giocate** (`SHEET_REVISION` 71, `est.value_with_seen`): `est_pv` le leggeva dal 07/09 e la fantamedia no, cioe' due colonne della STESSA riga su due campioni diversi — **145 righe su 561**, scarto mediano **0,488** di fantamedia, 32 oltre 1,0 (Raimondo 9,62 contro 6,37). Le due meta' si miscelano INSIEME o `fm − mv` smette di essere un tasso (v9.59). Fuori campione, K scelta sulle altre tre finestre: fantamedia **+2,8%** (3 su 4, la quarta ha DUE giornate viste su 35) e voto base **+4,7%** (4 su 4), ottimo interno a K=20 in entrambe; si adotta la K di R25 (`snapshot.seen_matches`, da `evaluate.ADOPTED`) col prezzo dichiarato (+2,5% invece di +2,8%) perche' la coppia deve pesare le stesse partite allo stesso modo — su `euro` si spegne da se'. Effetto: **56 righe su e 89 giu'**. (b) **La media minuti di un portiere ha un campione minimo** (`keeper_prior_matches` = 6): il suo ramo non ha ne' livello ne' tasso, quindi con la finestra miscelata il denominatore vale una frazione di presenza e **16 righe leggevano 120 minuti esatti** (Adrian, 12,0 / 0,1) e altrettante 3-10. Retto su `START_MINUTES["P"]` con la taglia del campione: **MAE 3,18 → 1,88 minuti, +40,9%, 4 finestre su 4**, ottimo interno (banda 5-8 piatta), e **sopra le dieci presenze non cambia un decimale** — che e' la meta' che rende la cura sicura. (c) **La nota della riga dichiara le partite lette**, con la condizione agganciata a cio' che e' stato davvero miscelato e non a una sua somiglianza: sei righe dicevano «nothing measured anywhere» portando dentro quattro partite. (d) **L'app smette di correggere cio' che il foglio ora porta** — lo SWING miscelava proprio le righe stimate, quindi la (a) avrebbe creato un doppio conteggio: `swing.sheetBlendsSeen`, una domanda sulla PIATTAFORMA scritta **una volta** e letta dai tre punti che prima la ripetevano.
MISURATO E RESPINTO: **la stessa shrinkage sui giocatori di MOVIMENTO** — 0 finestre su 4 e monotona su tutta la griglia (11,29 → 11,53). Il loro errore a campione corto e' grande (20-25 minuti fra mezza e due presenze, e sempre verso l'ALTO: D 68' previsti contro 46' veri) ma sta nel **livello**, cioe' nel tasso di partenza, non nel residuo — *due popolazioni con lo stesso sintomo non hanno per forza la stessa causa.*
PRE-REGISTRATO E NON ADOTTATO: **R27, l'undici che il club schiera non arriva al motore** (gate §7-septquinquagies). `evaluate` non importa `presence`, e il motore e' compresso verso il centro: sottostima di ~2 giornate su 36 chi la board disegna (−0,061 di calendario) e sovrastima chi non nomina mai (+0,050); correggere per gruppo OLTRE una correzione di livello vale **+1,0 … +1,6% su euro (4 su 4)** e +1,05/+0,99/−1,20/+0,65% su Serie A. Non adottata per la regola aurea **e** per un ostacolo strutturale che decide il lavoro: **il gate non la raggiunge** — la board nasce guidando il pannello Tk sopra un foglio gia' scritto. Quello che si raggiunge e' il `claim` (`sweep.build_inputs` costruisce `presence.Inputs` dal DB senza display), ed e' anche la forma con piu' probabilita' di fallire, perche' legge gli stessi minuti di R3.
VERIFICATO: **`backtest --verify` 22/22 reproduced** (letto per intero e non troncato), 918 test toolkit, 992 app, build pulito; **controprova su entrambi i lati** — rimettendo il difetto cadono esattamente i test che descrivono la cura e nessun altro. Un guardiano esistente e' caduto ed e' stato riscritto **sull'intento** e non sul valore. Artefatti rigenerati offline (`snapshot --no-refresh` × 3, `export`, `data:pull`): fogli a revisione 71, **0 minutaggi sopra i 95'** su 2.073, 0 righe che si contraddicono.
APERTO: i **quattro pacchetti del viaggio nel tempo restano a revisione 70** — la scatola dell'app lo dice, che e' il comportamento giusto, e rifarli e' una corsa di `timepack --all`. E il **rimpiazzo si misura sulle stagioni di INPUT** mentre la fantamedia del motore legge anche il campionato in corso: oggi passa ~0,02 nella sottrazione (a quattro giornate la miscela pesa il 9%), da ri-guardare a stagione inoltrata.
· precedente: **Aggiornato: 16 settembre 2026 (notte, II) — LA RIGA DELLA STRATEGIA: TRE RICHIESTE, E LA QUARTA COSA C'ERA GIA' E NON SI VEDEVA.** Quattro istruzioni dell'operatore sulla stessa riga, tutte misurate in un browser vero.
ADOTTATO: (a) **le icone tornano dal nome** — erano gia' sue nel DOM e le spostava il layout (col nome `flex-1` la sua cella si prendeva lo spazio libero e i marchi finivano appiccicati alle pastiglie): nome e marchi sono un gruppo solo e il `flex-1` passa al gruppo, quindi **4px dall'ultima lettera** (massimo e mediana su 158-161 righe di 250), il bordo sinistro del nome fermo e la fila incolonnata per costruzione. La distanza si misura con un `Range` sul CONTENUTO, perche' il bordo della CELLA sarebbe stato «vicino» anche prima. (b) **Il surplus e' una pastiglia come le altre**, prima della fila: NON entra in `READINGS` (una lettura e' un fatto sull'uomo che `readingsOf` ricava da sola, il gain dipende dal tipo d'asta dichiarato e dalle giornate del foglio — quarta eccezione dichiarata dopo etichetta, filtro e selettore) e ha un interruttore suo, perche' dentro `strategy.readings` «acceso» vuol dire ELENCATO e una preferenza salvata mesi fa avrebbe spento in silenzio la colonna che ordina. Nasce acceso, come `FlagPrefs` che tiene l'elenco degli SPENTI. (c) **I valori restano tutti in riga, incolonnati**, e cede il NOME: decisione sua presa col prezzo davanti — a mantra la riga passa da 38 a **24px** con **182 nomi tagliati su 255**, a classic con sette pastiglie **249 su 250**. Via il pavimento e via la regola del 04/09 sul contenitore stretto; `flex-wrap` resta come ultima risorsa, o il gain uscirebbe dal blocco. (d) **La banda ogni n righe c'era dal 04/09 e valeva DUE punti su 255**: `bg-control/25` sulla carta del blocco, perche' `control` e `surface` distano otto punti per canale su questo tema. Adottato `bg-fg/5`, **11,1 punti misurati**, appoggiato all'INCHIOSTRO perche' quello segue il tema da se' (su un tema chiaro scurisce invece di schiarire).
MISURATO E RESPINTO: il **pavimento sul NOME** (`min-w-12`) — la sua cella smette di stringersi sul testo e su un nome corto i marchi si staccano di **31,8px**, cioe' il difetto che il gruppo cura, rifatto un livello piu' in basso. Il pavimento sul GRUPPO funzionava ed e' uscito con la decisione (c).
DUE ERRORI DI MISURA MIEI, a verbale: **la sonda della banda era rotta nello stesso modo del difetto che cercava** — leggeva 1,4 punti su una banda da undici perche' componeva un `oklab(0.23 …)` con un `rgb(20, 20, 28)`, cioe' una L fra 0 e 1 sommata a un canale fra 0 e 255; ora converte in sRGB prima di comporre e la soglia (6 punti) e' asserita. E il primo A/B sul pavimento **ha attribuito a me un rosso che era del pacchetto**: il passo xG/xA leggeva 6 problemi, verificati identici su un worktree a HEAD — un'altra sessione stava rinfrescando il bundle mentre misuravo (la prima riga e' passata da Carnesecchi a Svilar) e il rosso e' sparito da se'.
DUE SESSIONI SU UN ALBERO, sesta istanza e stavolta a parti invertite: **l'altra sessione ha committato anche la mia meta'** (`6eecbc2`), dichiarandola nel messaggio e nominandone i file. Verificato dopo: tutto il mio lavoro e' a HEAD, build pulito, **990 test app**, e i tre banchi della Strategia (`e2e-strategy`, `-filters`, `-sort`) verdi sull'albero COMBINATO.
APERTO: il prezzo del «tutto in riga» e' dichiarato e non ridotto — a mantra 182 nomi su 255 sono tagliati, e la terza via misurata e non presa e' stringere i riquadri (~30-55px, una-due pastiglie in piu' prima del muro). `engine_*`, `SHEET_REVISION` e il pacchetto sono fermi: e' tutta lettura. Dettaglio: `pagina-strategia-v1.md` §21.
· precedente: **Aggiornato: 16 settembre 2026 (notte) — UNA GIORNATA VOTATA A META' SI LEGGEVA COME COMPLETA, E LE EURO NON ERANO INDIETRO.** Due domande dell'operatore: «c'e' qualcosa da allineare sulle leghe euro?» e «aggiorna anche il db, non vedo le ultime partite della serie a».
CURATO: il resume di `ratings` chiedeva `SELECT DISTINCT matchday`, quindi **una riga bastava a dichiarare scaricata una giornata** — Serie A md4 a **8 club su 20**, euro md3 a **20 su 37**, e `update --daily` i voti non li rilegge per definizione. `ratings.matchdays_done`: completa = almeno tanti CLUB quanti la MEDIANA delle ALTRE giornate di quella (stagione, piattaforma), costo misurato **5 giornate su 145**. Recuperati **+1354 righe su Serie A e +1832 su euro**: cambiano `engine_pv_pred` su **245 righe di 953 (euro)** e **140 di 561 (Serie A)**, sempre verso l'alto (Kane 24,2 → 27,3).
MISURATO: **le euro non sono indietro** — sulle tre prese pre-registrate gli 11 club esteri leggono **80,0% degli uomini e 10 moduli su 11** contro il 74-79% e l'8 su 12 dei 12 club di Serie A; `club_match_lineups.formation` e' al 100% su tutti e cinque i campionati. Quello che euro non ha e' un GIUDICE (`press_formations` = 20 club, tutti di Serie A). Chiuso il buco del Como sul foglio euro (37 quotati, 37 sul foglio).
APERTO: la cura del resume **non e' ancora stata esercitata sulla rete** (stasera si e' usato `--refresh`, che la scavalca): la prova e' il prossimo weekend. Dettaglio in spec «Novita' v9.97» e `formazioni-tipo-v1.md` §18.3-bis.
· precedente: **Aggiornato: 15 settembre 2026 — IL DENOMINATORE PER CLUB, IL MODULO DICHIARATO, IL RITIRO CONTATO DUE VOLTE — E DUE CANDIDATI MISURATI E RESPINTI.** Giornata nata da una domanda dell'operatore («riusciamo ad affinare la formazione tipo stagionale man mano che acquisiamo i dati dell'ultimo periodo?») e proseguita su quattro suoi casi concreti sul campetto. **Tre adozioni e due rifiuti**, e i rifiuti valgono quanto le adozioni.
ADOTTATO: (a) **il denominatore della stagione in corso è PER CLUB** (`SHEET_REVISION` 66) — contava le giornate del CAMPIONATO, quindi al 13/09 leggeva 4 anche ai diciotto club che ne avevano giocate 3 (Malen 3 partite su 3 della Roma, letto 3 su 4): 506 righe su 562, quota +0,062 di mediana, **124 gradini**, tre campionati su cinque colpiti, `engine_*` fermo e `--verify` 22/22. (b) **Il modulo è quello che il club DICHIARA**, in tutti e tre i lettori (rev. 67): i due board erano in disaccordo sul modulo in **13 club su 20** perché i tre conteggi di linea non sanno dire un 4-2-3-1. (c) **Il ritiro non è calcio giocato e finiva DUE VOLTE** nella stessa miscela (rev. 68) — `club_context` chiama `typical_formation` due volte e da quando la stagione bersaglio è quella in corso le due chiamate sono identiche (8 club su 8): 7 club cambiano modulo e tre dei quattro casi dell'operatore tornano (Milan `T:Rabiot`, Roma `T:Dybala T:Soulé Pc:Malen` con Castro fuori, Atalanta invariata).
RESPINTO: **K = 5** per il peso della stagione sul modulo — misurato bene (ottimo interno, cinque pieghe su cinque, −34% di Brier sull'incumbent `trust` 0,90 fisso, che è K = k/9) e **non adottato** perché il prior del banco non è quello del codice: applicato al pannello riporta 3 club al vocabolario a tre linee e disfa la correzione (c). Si riapre a rilettura d'archivio finita. E **il lato misurato sulla board di stagione**: la sostituzione è la forma MIGLIORE come previsione (−41% sul 4,5% dei casi dove codice e misura discordano, ottimo sul bordo) e rompe l'undici, perché **un prezzo per UOMO non può esprimere un vincolo sulla STRUTTURA** — il Milan ha tre uomini misurati a sinistra e l'unico misurato a destra il claim non lo disegna.
APERTO: il **caso Juventus** (`Ad:Celik`), da provare con la misura come SPAREGGIO e non come peso; la **rilettura d'archivio** ferma al 19,6% perché la fonte dà 403, e non è nel piano di `update`, quindi non la riprova nessuno; il **decadimento dentro la stagione** per il gradino, mai misurato. Commit: `1f52896`, `9ce4a69`, `f0a37a6`, `66ff20d`, `29df649`, `9e5fe8a`. Dettaglio in `formazioni-tipo-v1.md` §14-§16 e nei banchi `toolkit/bench/panel/shape_prior.py` e `side_pull.py`.
· precedente: **Aggiornato: 13 settembre 2026 (notte) — I NUOVI ACQUISTI TITOLARI: UNA REGOLA RESPINTA DAL GATE E UN MARCHIO SPEDITO.** Dalla domanda dell'operatore su Kessié. **R26** («le giornate viste sono quelle in cui era IN ROSA») e' pre-registrata e **NON PASSA**: +0,151% contro un pavimento di 0,5%, 9 finestre su 13, e l'attesa che avevo scritto prima della corsa e' SMENTITA (i guadagni stanno a febbraio e non a settembre, perche' domina la numerosita' e non la proporzione). `--verify` 22/22, `ADOPTED` intatta. Quello che si spedisce e' un MARCHIO: `signing` su ogni riga di giocatore, con `transfers_history` che era nel bundle e non arrivava all'app (dodicesima istanza). Il difetto piu' grosso l'ha trovato il banco ed era mio - Bakker leggeva «nuovo acquisto» sulla rosa che aveva LASCIATO. Diagnosi completa in `letture-app-v1.md` §42.
· precedente: **Aggiornato: 13 settembre 2026 (sera) — UN VETO CHE CANCELLAVA I RIVALI, E UN PAVIMENTO CHE NON SA DISTINGUERE.** Dalla segnalazione dell'operatore sul campetto del Milan («la formazione ultimo periodo e' coerente ma vorrei che Rabiot e Pulisic, due elementi fondamentali, compaiano almeno in ballottaggio»): due nomi, **due cause opposte**, e riconoscerle prima e' meta' del lavoro. **Pulisic era gia' nel file** (0,178 di quota) e lo nascondeva il pavimento dell'app; **Rabiot non c'era affatto** pur avendo COMINCIATO l'ultima giornata. (a) Il **veto delle staffette** leggeva titolare E ballottaggi dallo stesso serbatoio filtrato, quindi chi scartava spariva da tutt'e due i canali - la regola dell'11/09 che il percorso del 12/09 non aveva ereditato, terza istanza. (b) **«Di piu'» era CONTATO e non pesato**: misurato su 49.810 slot-partita, il conteggio puro e' il punto PEGGIORE della griglia (0,5898 contro 0,6077 di ogni peso sotto 0,618) e sui 13.979 posti dove le due regole non concordano legge 0,375 contro 0,311; la soglia 0,618 e' `w + w^2 = 1` e non una scelta. La DISTANZA e' dovuta entrare nel dato (`SHEET_REVISION` 65). (c) **`recent_state`** (`started`/`sub`/`bench`/`away`) nel file e nell'ordine dei rivali, misurato su 4.324 ex titolari: 0,393 / 0,165 / 0,127. (d) Nell'APP, l'esenzione al pavimento **sul segnale che l'operatore aveva proposto lui** - il prezzo REALE del cartellino, relativo ai titolari del club: rho +0,103 contro +0,069 della quota di partenze, e il gradino di stagione NON avrebbe risolto il caso (dei 193 ballottaggi sotto il pavimento solo 8 hanno un gradino alto, e Pulisic non e' fra loro). `KEY_MAN_VALUE` = 2, non circolare perche' Pulisic entra a ogni soglia provata. Commit `73a5268` (toolkit, 886 test) e `517a093` (app, 970 test), bundle rigenerato alla revisione 65 e portato nell'app. Dettaglio: `formazioni-tipo-v1.md` §13 e `letture-app-v1.md` §41.
· precedente: **Aggiornato: 13 settembre 2026 — I QUINDICI RILIEVI SPEDITI, E IL COMPLETAMENTO NON ERA LA MAPPATURA.** La code-review di ieri notte aveva trovato quindici difetti latenti nella lettura per slot e non ne aveva spedito nessuno, perche' la correzione in blocco portava gli undici completi da 20/20 a 16/20. Chiuso il completamento per primo - erano un veto applicato prima del filtro di posizione e un posto che l'aritmetica della mappatura per riga lascia scoperto, e nessuna delle due cause era la mappatura - il blocco e' partito intero: mappatura per RIGA (512 combinazioni su 1331 si spostano, era l'identita' su tutte), la regola delle staffette che torna raggiungibile sulla board breve, due dritte `starter` che non si scacciano piu', piu' sette rilievi minori e i due buchi di test. Un SEDICESIMO l'ha trovato l'A/B e nessuna rilettura poteva vederlo: su una cartella NUOVA le dritte dell'operatore uscivano vuote, perche' il manifest che porta la stagione si scrive dopo i campetti. `backtest --verify` 22/22, board di stagione e giudice stampa identici (150/220), `SHEET_REVISION` 64. Dettaglio: `formazioni-tipo-v1.md` §12.9.
· precedente: **Aggiornato: 12 settembre 2026 — IL MODULO ERA UN FATTO PUBBLICATO E LO DEDUCEVAMO.** Giornata nata da una richiesta piccola («nella pagina CLUBS, quando seleziono una partita cliccando sul nome della colonna, ricostruisci nel campetto la formazione che ha giocato quella partita») e finita su tre acquisizioni e la riscrittura della board dell'ULTIMO PERIODO. Il filo è uno solo, e questo repository lo ha già pagato otto volte: **il dato c'era e mancava un lettore**. (a) **IL MODULO DELLA FONTE.** Le linee del campetto venivano dai conteggi `G/D/M/F`, che ne hanno TRE: misurato, **122 disegni su 178 (68,5%)** discordavano dal modulo vero, e il più frequente è proprio il `4-2-3-1` che tre linee non possono esprimere (81 su 178). `home.formation` è nel payload dalla prima corsa e il downloader lo scartava. Adottata la sua regola — «utilizziamo il modulo della fonte, altrimenti non ci troviamo più con il resto» — con il taglio per INTERVALLI DI POSTO e non per fette, perché a fette un nome mancante faceva scivolare tutta la linea e il disegno tornava ai tre conteggi proprio su `euro`, dove i nomi mancano per costruzione. (b) **DOVE HA GIOCATO UN SUBENTRATO, E DI CHI HA PRESO IL POSTO**, dal suo caso: «la Juve ha cominciato con un 442 ma poi si è trasformato in un 4-2-3-1, e sembra che González abbia giocato nella stessa posizione di Kolo Muani, **che non è uscito**». Kolo Muani ha giocato 90 minuti: il suo posto non si è mai liberato. `/event/{id}/average-positions` porta **due fatti in una risposta** — la posizione media di chiunque abbia messo piede in campo, subentrati compresi, e l'elenco dei cambi con chi esce e chi entra — e i minuti non lo ricostruiscono (un'uscita trova un ingresso complementare unico nel **30,3%**, e su quella partita nemmeno torna: cinque entrati contro quattro usciti, perché Cambiaso è entrato al 76′ ed è uscito lui stesso al 86′). Tre colonne (`avg_x`, `avg_y`, `came_for`), la catena risalita, e **ZERO** subentrati che risultino entrati per un uomo rimasto in campo 90′. `avg_y` è nello stesso verso di `lineup_slot`, misurato: **86,0%** di accordo su 683 linee. (c) **LA BOARD DELL'ULTIMO PERIODO SUI MODULI OSSERVATI**, sua richiesta esplicita, con la precisazione che conta: «deve rappresentare la formazione tipo della squadra reale nel breve termine — e non c'entrano CLASSIC o MANTRA». `formation_shapes_recent` leggeva i tre conteggi: **10 board su 20** disegnavano un modulo che il club non ha dichiarato in nessuna delle ultime tre (Atalanta `4-3-2-1` contro un `4-3-3` detto tre volte su tre, Napoli `4-2-1-3`, Parma `3-4-1-2` contro `4-4-2`). Adesso **20 su 20**, con `_two_rows` e `_reshape` spenti su quella finestra — un modulo che la fonte NOMINA non si deduce, e uno giocato la settimana scorsa la rosa lo copre per costruzione. **A/B su UNA variabile sola**: board di STAGIONE 0 moduli e 0 disegni su 20 club, board breve 11 moduli e 12 disegni. (d) **E LA SUA CORREZIONE HA TROVATO UN DIFETTO CHE AVEVO APPENA INTRODOTTO**: «perché nella Roma vedo Wesley sulla trequarti?». Il modulo era giusto, la SELEZIONE no — `lines('3-4-2-1')` chiede «sei centrocampisti e un attaccante» mentre i trequartisti finiscono in ATTACCO, quindi Soulè contendeva l'unico posto d'attacco a Malen e la trequarti la riempiva un esterno. Sull'orizzonte corto la selezione chiede ora le righe del modulo dichiarato (P·D·M·T·A): Wesley torna all'`Es`, Soulè va sulla trequarti, fuori ruolo **15 → 14** (erano 17 col solo punto c). **La forma larga è stata scritta e BOCCIATA dalla misura**: ogni attaccante candidato alla trequarti porta i fuori ruolo a **22** e fa schierare al Milan Pulisic, che nelle ultime tre ha ZERO minuti. (e) **LA SUA GRIGLIA A SEI LINEE, MISURATA**: con `avg_x` le quattro righe di un modulo leggono 40,7 · 48,7 · 61,4 · **63,1**, cioè trequarti e attacco non si separano — quindi i nomi delle righe restano una convenzione DICHIARATA e non una misura. E la quinta riga non serve: **nessun modulo a cinque numeri esiste** nell'archivio (0 su 178). **Acquisizioni**: `--layer formations` (una richiesta per partita, nessuna per turno, dalla stagione in corso all'indietro perché una scansione da nove ore si abbandona appena la fonte rifiuta) ha chiuso **178 → 2.267 partite-lato** col modulo; `--layer places` si è fermato sulla guardia dei rifiuti dopo 210 partite e ha comunque completato la stagione in corso (Serie A 31/31, Liga 41/41, Premier 30/30, Ligue 1 28/28; manca la Bundesliga e le stagioni vecchie, e riprende a una corsa successiva). **Verifica**: 860 test toolkit, 918 app, build pulito, i banchi `e2e-clubs-lineup`, `e2e-board-horizon`, `e2e-board-sidelined`, `e2e-clubs` verdi; `SHEET_REVISION` 62, tre fogli + export + `data:pull` rifatti. `engine_*` fermo: nessuna colonna nuova entra in `evaluate`. **Due asserzioni nuove provate RIMETTENDO il difetto**: col codice vecchio il banco legge «Dovbyk e Mbangula disegnato sotto Piccoli, che ha giocato 90′». **APERTO, con il suo numero**: Chukwueze non è nell'undici del Milan e dovrebbe esserci — 2 partite su 3 da titolare, la fonte lo mette a CENTROCAMPO con `avg_y` 16 (fascia destra) e quel posto va a Rabiot, un centrale; la causa è che l'undici mette un uomo in una linea guardando il suo CODICE e non la linea MISURATA, e `external_match_stats.position` è il nostro vocabolario a quattro linee, quindi la trappola che l'11/09 aveva fatto tenere «solo il lato» lì non c'è. Più le staffette dai cambi VERI (43 coppie con due o più cambi reciproci in tre giornate — Hojlund↔Lucca, Scamacca↔Krstović, Malen↔Castro S. — mentre `desc_relay` poggia ancora sull'assunzione «entra al minuto 90 − giocati», che al Milan separa due uomini stati in campo insieme 55 minuti), e le coppe sulla board (`desc_cup*` esiste, la board non lo legge, e oggi è INERTE: la Coppa d'Asia è 07/01→05/02/2027). **TRE ERRORI DI MISURA MIEI, a verbale perché sono il modo in cui si sbaglia**: il primo A/B muoveva DUE variabili (il codice e i dati, cambiati dalle acquisizioni della stessa giornata) e ho dovuto rifarlo con un interruttore temporaneo; ho ricostruito la formula della staffetta a mano invece di CHIAMARLA, e la mia ricostruzione dava un numero che non era quello del foglio; e un fixture dei test generava `fc_id` collidenti (`MedianoA`/`MedianoB`/`MedianoC` lo stesso id), quindi il primo test della catena sembrava un difetto del codice. **DUE SESSIONI SU UN ALBERO, e questa volta HEAD era ROSSO**: `club-eleven.ts` a HEAD legge `outToday` sei volte e `valuation-store.ts` a HEAD non lo definisce — l'altra sessione ha committato i consumatori e lasciato fuori le definizioni. Quindi questo commit porta anche i loro due file, perché separare avrebbe lasciato il rosso che la regola esiste per evitare.
· **precedente: 7 settembre 2026, sera tardi — LE PARTITE DI QUEST'ANNO ARRIVANO DOVE NON ARRIVAVANO, E LA STAMPA NON SCEGLIE PIÙ I RIVALI.** Dalla richiesta dell'operatore «vorrei che le partite della stagione corrente influenzino maggiormente le valutazioni dei calciatori» e da **cinque nomi**, poi da altre tre sue osservazioni sullo schermo. Il PESO — la cosa che la richiesta nomina — è l'unica causa già misurata e non è stata toccata: `season_prior_rounds` = 5 era stata rimisurata il 05/09 su questa stessa quantità (ottimo interno, e la percentuale fissa 50/50 che lui aveva proposto costa **+4,7%**). Tutto il resto sono errori di **UNITÀ**, e i cinque nomi non avevano una causa sola. (a) **`est_pv` NON LEGGEVA LE GIORNATE GIOCATE** (`SHEET_REVISION` 51). Varela, 2 partite su 2 da titolare e 2 gol, leggeva **10,2 giornate su 36** con una nota che diceva «nothing measured anywhere»: 139 righe su 601 stavano su un gradino di ripiego E avevano giocato, con scarto mediano **0,119** dalla quota della scala. Misurato fuori campione col null che la cascata usa DAVVERO, popolazione spaccata per gradino — `anchor` **+15,4%** (n=2272, 11/11 stagioni) · `shrunk` +10,7% · `abroad` +9,9% su default; +15,1/+16,9/ +20,9% su euro. Sei celle su sei, ogni stagione positiva, ottimo interno, **K=0 negativo**. Zero costanti nuove (la K la legge da `presence.DEFAULTS`, e un test lo asserisce). La guardia è «il motore non ha una previsione di presenze» e non «non è il core»: `pv_pred` porta già R20, e **111 righe su 325** sarebbero state contate due volte — Meret fra loro. (b) **LE GIORNATE SALTATE SI SOTTRAEVANO DA UN DENOMINATORE CHE NON ERA IL LORO** (rev 52). Dal caso Berardi, e la sua frase era la specifica: 26 giornate a voto su 38 saltandone **DIECI**, quota condizionale 26/28 = **0,929** contro lo 0,684 che entrava nel prior. Su una pre-stagione era giusta: la miscela del 04/09 aveva **rotto** una condizionalità che funzionava (terza istanza di «il denominatore segue il suo NUMERATORE»). `SeasonWindow.missed` la pesa con gli stessi pesi del denominatore, e il prior si riscala a K giornate di calcio **CONTENDIBILE** con un tappo a 1,0 — la prima versione, trovata dalla MISURA e non dalla rilettura, faceva leggere **1,000 a Pieragnolo** (33 giornate perse su 38). (c) **I MINUTI DEL RITIRO AL TASSO PER PRESENZA** (rev 53), da «perché Meret ha minuti attesi 80 e contemporaneamente Milinkovic-S. ha 79? È un paradosso!». Il paradosso non c'era — sono minuti QUANDO GIOCA — ma il numero era sbagliato: le misure sono **89,1′** e **90,0′**. Il ritiro imputava al tasso per GIORNATA, cioè affermava «ha cominciato quattro amichevoli, **39,6 minuti** ciascuna». Sui 22 portieri con ≥3 presenze: **11 sotto la misura e ZERO sopra** (Pessina 35 contro 88, Motta 58 contro 90). (d) **LA REGOLA «CHI LA STAMPA NOMINA VIENE PRIMA» È CANCELLATA** (rev 54, sua decisione). `duels` chiama ballottaggio due probabilità COMPARABILI, e 0,05 contro 0,01 lo sono — due portieri che non giocheranno nessuno dei due — quindi al Napoli il filtro teneva Contini e lasciava fuori **Meret, che gli stessi probabili danno a 1,00**. Tolta nei DUE punti di selezione (`eleven` e `_declared`): era la stessa regola. **80 maglie su 220** cambiano lista, 102 nomi aggiunti e 28 togliuti (il Como passa da `[Vigorito]` a **`[Butez, Audero]`**); la stampa resta A SCHERMO come fatto dichiarato. (e) **E LA SUA SECONDA OBIEZIONE HA RIDIMENSIONATO UN MIO NUMERO.** Il 10/18 sui portieri citato la mattina è **UNA cella di quattro**: riallargata a cinque campionati e **294 casi**, «due partite da 90′» vale **0,770** del resto di stagione contro un null di **0,129** (sei volte), e la sua verifica passata esisteva davvero (`starter_signs`, portieri 81,9% contro 22,3%). Dentro la cella debole il discriminante non è l'allenatore nuovo ma **se l'uscente era DISPONIBILE** (infortunato 0,230 · sano e non schierato **0,610**), e il caso vivo sta nella cella buona. La conferma migliore non l'avevo cercata: il punto (b) dà a Meret **0,638** e la sua cella ne realizza 0,610. Più la terza obiezione, misurata: un ripiego per infortunio tiene la maglia fino al rientro — **n=78**, da 0,891 dentro lo stop a **0,276** dopo (in calo in 73 casi su 78), il rientrante riprende 0,664 al ≥70% nel 63% — **popolazione viva oggi ZERO**, quindi in todolist da PRE-REGISTRARE e non spedita. **GIUDICE ESTERNO** (`press --against press`, stessa data, solo il codice cambiato): uomini **151 → 153 su 220** contro un null di 104. E la cosa più utile l'ha detta PRIMA delle correzioni — sul foglio vecchio la stampa metteva Meret e Berardi fra i suoi e non metteva Castro, cioè era d'accordo con l'operatore su tre dei cinque nomi. **Prezzo attribuito con un esperimento a una variabile**: la rev 52 leggeva 155, il punto (c) porta a 153, e con `friendly_rounds` = 0 si torna a **155** — quindi la correzione dei minuti non costa niente e i due uomini li costa il fatto che quella finestra ora **pesa di più**. E +2 dei +4 della rev 52 stanno sul Sassuolo, dove le odds leggono 3-5-2 **0,432** contro 4-3-3 **0,427** e i due produttori di campetti cadono su lati opposti (19 club su 20 concordano): quella forma è un `board_rulings.json`, non una misura. **Verifica**: `backtest --verify` **22/22**, **746 test** toolkit, `SHEET_REVISION` **54**, e il DB e il bundle sono AGGIORNATI — `update --daily` (7/7: ruoli granulari, Elo, calendario, probabili) più `update --offline --from packs`, quindi i tre fogli, i **quattro pacchetti** (erano alla 48) e la copia dell'app sono tutti alla 54. `engine_*` fermo: `evaluate` non importa né `presence` né `estimate`, la sweep non nomina mai `SeasonWindow`, e tutte le correzioni sono inerti a zero giornate giocate. Commit **`eaefb45`** pushato. Nessuna pubblicazione sul web: `deploy-pages.mjs` non è stato lanciato. **APERTO, e sono quattro cose dette e non nascoste**: `friendly_rounds` = 1,0 è DICHIARATO dall'operatore e dichiarato sotto la vecchia imputazione, quindi da **rimisurare** e non da girare per recuperare due uomini su una lettura sola; la forma del Sassuolo è un suo ruling; il canale del ripiego è da pre-registrare; e resta un'oddità **latente** — due uomini a 0,05 e 0,01 sono «in ballottaggio» per la definizione di `duels`, oggi inerte perché il titolare vero cade sul claim, e la forma giusta esiste già in casa (`ownsShirt` delle buste: due pretendenti di cui **uno che la board schiera**). **QUATTRO ERRORI DI MISURA MIEI, a verbale perché sono il modo in cui si sbaglia**: `MAX(real_md)` su tutte le competizioni non è il calendario di una (ho aperto dicendo che la 3ª giornata era giocata: era la_liga); il primo null era troppo debole (+33,8% invece di +15,1%); **una stima non è la funzione** (la mia ricostruzione dava 1,000 a Pieragnolo dove la funzione dà 0,500); e una suite lanciata su un albero che si sta modificando non è una suite. Più tre regole di metodo nate qui: **una popolazione piccola si allarga, non si spacca** (18 → 294 casi); **due popolazioni che escono identiche sono lo strumento che dice dove sta il vincolo** (togliere il filtro sul ripiego non allargava niente: il collo di bottiglia era l'infortunio); e **quando due neutralità non possono valere insieme si scelgono in chiaro** (il ritiro non può essere neutro sui minuti per giornata E per presenza, se la quota di presenze si muove). **DUE SESSIONI SU UN ALBERO, e stavolta senza metà da separare**: `1c6da62` è atterrato alle 15:49 mentre lavoravo e porta solo tre documenti loro, quindi la modifica a `CLAUDE.md` sta SOPRA la loro — misurato, 0 righe loro nel mio diff (+39/−12). Il banco rosso che il loro commit nominava onestamente (`test_blend_missed_rounds.py`, mio e in corso) ora è verde. **E all'ora di questa chiusura l'altra sessione ha lavoro NON committato in albero** (`cli.py`, `snapshot.py`, `CLAUDE.md`, la spec e `test_snapshot.py`: il segnale della rosa live, «di un'assenza da due letture piene il 3,4% si rimangia»), quindi questa chiusura porta **solo i due documenti di chiusura** e le regole di metodo sono finite qui invece che in `CLAUDE.md` — non si opera su un file che due mani stanno scrivendo.
...
Tre richieste dell'operatore in fila, nate da una sola osservazione: «i dieci attaccanti piu' cari hanno
partite attese fra 20 e 26, e i migliori dell'anno scorso ne hanno fatte fra 30 e 36 — come mai?».

**LA DIAGNOSI HA TROVATO TRE COSE, E IL MOTORE ERA LA PIU' PICCOLA.** Da 32,2 (il suo ricordo) a 22,0 (il
numero a schermo) ci sono quattro passi: **−2,9 la lista scelta ex post**, **−3,9 il denominatore**,
**−0,5 il motore**, **−2,9 l'assicurazione**. I «migliori attaccanti dell'anno scorso» sono selezionati
DALL'aver giocato — su 11 stagioni la top-10 per FVM finale realizza 32,2 presenze e quella per Qt.I
pre-asta 29,3 — e il FVM di una stagione passata e' l'ULTIMA LETTURA, quindi conosce l'esito.

**IL RIPORTO A STAGIONE PIENA** (`core/season-scale.ts`) e' la sua regola: «tutti i valori che leggiamo
sulle pagine devono essere rapportati all'intera stagione ... anche se il mercato dura 3 giorni, su base
38 (sia partite attese che surplus)». «Base 38» ha due significati e danno due numeri: riscalare, oppure
sommare le giornate viste. Divergono di ±2,6 (mediana 0,17) e si adotta il riscalamento, perche' **il
surplus un passato da sommare non ce l'ha** e le due colonne della stessa riga devono stare sulla stessa
base. La base e' la stagione della PIATTAFORMA (31 su euro, non 38). Si riporta l'ESTENSIVO nelle
giornate; non l'intensivo, non i prezzi, non le misure. Prezzo dichiarato: chi ha saltato le prime
cinque giornate legge il proprio RITMO e non il totale che fara'.

**IL NULL CHE RISPONDE A «SONO TROPPO BASSE»**, su 1.898 casi in 11 stagioni: chi fa 26-38 presenze,
l'anno dopo il 17% non ha una riga di Serie A e chi resta ne fa **26,4** (mediana 29, sd 9,2). Sui 165
quotati di quella fascia il motore prevede **26,2**: in centro, a due decimi. *Trentacinque presenze non
sono la previsione per l'anno dopo.* E il 31% di previsioni sopra 30 contro un vero del 47% non e' un
difetto: una previsione predice la media condizionale, quindi e' piu' stretta degli esiti per
costruzione.

**SPENTO IL PAVIMENTO DELL'ASSICURAZIONE** (`INSURANCE_FLOOR_ROUNDS` = 0, era 2,1), per sua decisione col
numero davanti: il 46% dei titolari la pagava identica, e una costante uguale per tutti abbassa senza
distinguere — non riordina e non evita nessun disastro. Resta la meta' personale, fino a 13,3 giornate.

**E IL NUMERO CHE AVEVO PUBBLICATO ERA SBAGLIATO.** Avevo annunciato 24,4 sommando 2,1 a chi stava al
pavimento; il pavimento non si sottrae intero, e chi ci sta sopra guadagna solo la differenza col
proprio scarto. Misurato: **22,3 → 22,8**, con Dimarco fermo al decimo. *Un numero dedotto da una
percentuale non e' un numero misurato, e qui la differenza era un fattore quattro.*

**VERIFICATO**: 1048 test app, 931 toolkit, banco Strategia senza problemi, e un'asserzione nuova che
non passa dal fattore — se il foglio prevede 33 e la stagione ne ha 38, un numero sopra 33 e' impossibile
senza il riporto, e a schermo il piu' alto legge 34. Prima della cura lo stesso banco leggeva 130 righe
fuori scala: e' cosi' che si e' saputo che il cambiamento era arrivato allo schermo. **Il primo verde non
aveva guardato niente** — 1038 test passavano perche' nessun fixture porta `matchdays`.

**APERTO**: lo schermo resta −3,6 dal null ed e' ormai tutta assicurazione personale (3,4 giornate di
media); spegnere anche quella lascerebbe il numero del motore, ed e' una decisione sua. Piu' i rossi
preesistenti gia' a verbale (due test di `press` che scadono con l'orologio, un riepilogo del banco della
card) e il fatto che `matchdays_input` entra nel manifest solo dopo un `export` — non serve aspettarlo,
la base si legge dal foglio.

## CHIUSURA — 22 settembre 2026 (sera): le sette parole, e la scala che c'era e non si vedeva

Richiesta dell'operatore: un'etichetta per ogni calciatore, sei definizioni dettate a voce. Ne sono
uscite sette parole, tre regole chiuse dai suoi nomi, una misura nuova sul peso della stagione in corso,
e sei difetti - quattro trovati da strumenti, due miei.

**LA PRIMA COSA UTILE E' STATA NON COSTRUIRE.** La scala esisteva dal 01/09 con sei parole sue, il
toolkit la calcolava, l'app aveva vocabolario, colonna e filtro - e `desc_category` non era in
`export.SHEET_COLUMNS`, quindi la colonna era vuota su tutte le 562 righe da tre settimane. *La sua
richiesta nasceva da quel vuoto*, e dirglielo ha trasformato «costruiscila» in una scelta sua fra
vederla e rifarla. E' la famiglia dei campetti, di `availability`, dell'asterisco e di
`transfers_history`, qui su una COLONNA invece che su una cartella o una tabella - la quinta volta che
questo repository la incontra, e la prima su una colonna.

**I NOMI SONO LA SPECIFICA, E QUANDO SI CONTRADDICONO SI MOSTRA LA CONTRADDIZIONE.** Due volte i suoi
verdetti non erano simultaneamente soddisfacibili, e le due volte la risposta non e' stata scegliere per
lui: «Rabiot top e Malen super» non sta in piedi ne' in assoluto (Rabiot e' 46esimo del listone) ne'
dentro il ruolo (Malen e' 5o), e la sua frase - «i super sono quelli fuori scala *per gli A*», piu'
«Dimarco si', Calhanoglu no perche' ha poche presenze» - ha sciolto il nodo in una riga. La seconda
volta («Kvernadze e Varela buoni» contro «Pinamonti riserva», stessa fantamedia) ha prodotto la misura
piu' importante della giornata.

**LA MISURA: IL PESO DELLE PRIME GIORNATE E' PER RUOLO, E CAMBIA DI DUE VOLTE.** Leave-one-season-out su
dieci stagioni, bersaglio le giornate DOPO la k-esima: a cinque giornate la stagione in corso vale il
21% per un attaccante e il 10% per un centrocampista (K 18,5 contro 43,5), con un guadagno del 6,0% su
10 stagioni di 10 contro l'1,7% su 7 di 10. Il meccanismo sta nella colonna dell'errore - la fantamedia
di un attaccante e' volatile (0,63 di MAE contro 0,28) quindi il segnale dell'anno prima e' piu' debole.
Il motore tiene la sua K unica di 40: questa vive in un `desc_*`, e portarla dentro `evaluate` e' R28,
da pre-registrare, **perche' la mia misura e' sul predittore NUDO e un baseline piu' debole fa sembrare
un canale piu' forte** (la lezione di R24, applicata a me stesso prima che qualcuno la usi).

**LA COMPRESSIONE DELLE PRESENZE E' MISURATA E GIUSTA, e la risposta e' un NULL.** Lui: «quasi nessuno
dei migliori supera le 25 presenze previste» mentre l'anno scorso ne facevano 30. Tre pezzi: i due numeri
non erano sulla stessa base (33 giornate che restano contro 38 giocate); la compressione esiste (chi
stava a 0,80-1,00 riceve −0,169 di quota, e la sd della previsione e' il 64% di quella vera); **ed e'
quello che succede davvero** - chi ha giocato il 90% di una stagione ne gioca il 75% l'anno dopo, e il
motore prevede 0,728 dove la storia realizza 0,747, mezza giornata su 33. Il caveat rafforza invece di
indebolire: quel null esclude chi e' sparito dal listone, quindi la persistenza vera e' piu' bassa. E i
sei casi piu' lontani hanno tutti giocato ZERO delle cinque giornate di quest'anno.

**I CONTEGGI SONO UNO STRUMENTO, non un riepilogo.** Pubblicarli ha trovato il difetto che nessun test
vedeva: `scommessa` usciva **0**, vuota per costruzione, perche' il foglio una fantamedia la da' a tutti
(dove il motore non prezza, `est_fm` ripiega sull'ancora del ruolo) - quindi 147 uomini che nessuno ha
mai visto giocare prendevano 103 `scarto`, 31 `riserva` e 7 `solido`, cioe' un giudizio. E' `bandiera`
(20/08) su una parola invece che su una scala. La cura distingue l'ancora NUDA dall'ancora PIU' le
giornate di quest'anno: chi stiamo guardando adesso non e' un ignoto.

**LA CODE-REVIEW HA PAGATO SU DUE RILIEVI SU OTTO**, ed erano i due che si sarebbero visti solo a
schermo: le sbarre erano una tabella sola per due piattaforme mentre la scala euro sta piu' in alto
(mediana attaccanti 7,14 contro 6,61, perche' EuroLeghe e' una selezione di top club), e `squad-table`
smorzava con soglie scritte sull'INDICE della scala - `rank >= 4` era `scommessa` con sei parole ed e'
`riserva` con sette, cioe' meta' listone smorzato come uno scarto. *Una soglia scritta sull'indice di una
scala e' una soglia che la scala successiva rompe in silenzio.* Misurando il primo e' saltato fuori un
fatto che non cercavo: **su euro 80 portieri su 110 hanno esattamente la stessa fantamedia attesa**, tutti
dal core - quindi li' la categoria di un portiere distingue poco, perche' il motore non li distingue.

**E DUE ERRORI MIEI, della stessa famiglia e il secondo dopo averla scritta nel codice.** Ho tarato una
sbarra su un numero ARROTONDATO due volte: prima Svilar (5,329 contro un p97 stampato 5,33) e Varela
(6,878 contro 6,88), poi il pavimento di `semi` su «0,72» letto dalla mia stessa tabella quando il valore
vero di Scamacca e' 0,718. *Il percentile trova la zona, il caso dichiarato fissa il valore, e il caso va
letto alla precisione con cui decide.* Il secondo e' peggiore perche' riguarda cio' che ho riportato a
lui: **un `str.replace` senza `assert` e' un no-op silenzioso**, e due test delle regole nuove non sono
mai entrati nel file mentre dicevo che c'erano.

## CHIUSURA — 22 settembre 2026: i turni giocati entrano, e un aggregato era congelato alla prima giornata

Due richieste dell'operatore in fila: «aggiorna il db alla luce dei turni serie-a e euro giocati», e poi
«risolvi le anomalie» — la riga che `validate` stampava in coda a ogni corsa e che nessuno aveva mai aperto.

**L'AGGIORNAMENTO.** I voti erano fermi a Serie A md4 e euro md3 e il layer per-partita al 14-17/09, mentre
il turno del 19-21 era stato giocato: `update --daily` i voti non li rilegge per definizione, e l'ultima
corsa completa era del 16. Fatta una corsa MIRATA invece del piano intero (18h50 dichiarate a freddo, 9h53
il sottoinsieme, e in pratica molto meno perche' la cache copriva tutto fino al 17): fuori gli archivi
settimanali e i layer lunghi, dentro quello che i turni giocati muovono davvero. Arrivati **Serie A md5**
(20 club, 336 righe), **euro md4** (37 club, 607 righe) e il per-partita al **20/09 su tutti e cinque i
campionati**. Il resume di `ratings` ha esercitato per la prima volta sulla rete la cura di v9.97 — salta
le giornate COMPLETE e si ferma al primo foglio vuoto — il che chiude l'aperto del 16/09.
*Il controllo che conta non e' che il DB contenga i turni, ma che i FOGLI li leggano*: le giornate da
prevedere passano a **33 = 38 − 5** su Serie A e **27 = 31 − 4** su euro. Il denominatore che si accorcia
e' la prova, e viene dal manifest e non da un conteggio fatto a mano.

**LE 653 ANOMALIE ERANO UN DIFETTO SOLO, E NON ERANO NEI DATI.** Tutte dello stesso tipo (i voti contano
piu' presenze del `pv` di stagione), zero anomalie di Mv, e 615 su 653 nella stagione in corso: piu' della
meta' dei quotati euro, cioe' **la firma di una causa nostra e non di 615 casi singoli**. La forma della
tabella lo ha detto in una query: il `pv` di `season_stats` 2026-27 valeva **solo 0 o 1**, con quattro
giornate giocate, su entrambe le piattaforme, mentre ogni altra stagione ha la sua distribuzione piena
(0-38 su default, 0-32 su euro).
Il test decisivo: quel `pv` coincide **esattamente con i voti della sola GIORNATA 1** su 574 righe euro e
319 default — **il 100%** — mentre coi voti totali coincide nel 16%. `derive_from_ratings` scriveva solo
dove la riga non ESISTEVA, quindi la riga nasceva col conteggio del giorno in cui era stata creata e
nessuna corsa successiva poteva piu' toccarla. E' «un derivato stantio e' peggio di uno vuoto» (v9.73)
applicato a un AGGREGATO invece che a una colonna.

**LA CURA HA UN CRITERIO CHE DISTINGUE DUE VERSI CHE NON SONO LO STESSO FATTO.** I voti che ne contano
*di piu'* significano che l'aggregato e' indietro rispetto a cio' che abbiamo misurato; *di meno* significa
che la nostra copertura e' parziale e il listone sa di piu' (111 righe di euro 2024-25), e li' il listone
resta autoritativo. Non e' una regola inventata per l'occasione: e' la stessa asimmetria che
`check_ratings_consistency` gia' applicava saltando le parziali e segnalando solo l'altro lato. Una riga col
`pv` NULL resta ferma — non puo' rivendicare un conteggio, ma qualcuno l'ha scritta. La prova che il
criterio rispetta l'autorita' del listone e' che **i due test che la proteggevano da prima passano
invariati**: una riga a `pv` 30 contro un voto in archivio non viene toccata.

**IL METODO, che vale piu' della cura.** La misura che ha deciso il disegno e' stata «cosa cambierebbe un
ricalcolo, stagione per stagione»: **17 stagioni-piattaforma su 21 non si muovono di una riga**, il che dice
che ricalcolare dai voti concorda col listone quasi ovunque e che le differenze sono localizzate. Poi la
cura e' stata applicata su una **copia privata del DB** (558 MB, un secondo) e misurata li' — anomalie
653 → 0, `pv` 0-5 e 0-4 — e solo dopo sul database vero, dove ha prodotto un risultato **identico**. La
controprova e' stata fatta rimettendo il difetto: cade esattamente il test nuovo e **nessun altro**, perche'
gli altri diciassette descrivono comportamenti che non cambiano.

**E I NUMERI DEL GATE SI MUOVONO, il che va dichiarato e non nascosto.** `beta_mantra_T2` 0,446 → 0,444
(367 → **369** coppie), `pv_gain_vs_naive_T1` 0,0211 → 0,0197, `pv_gain_crossfit_T2` −0,0209 → −0,0194,
`pv_bias_naive_starters_T2` 5,47 → 5,50: le 38 righe storiche alimentano `pv_prev`, che e' un INPUT del
motore. `backtest --verify` resta **22/22** e ogni controllo sta dentro la propria tolleranza. La
distinzione che conta: **non e' una regola che peggiora, e' un dato che cambia** — il confronto non e'
modello contro modello, sono righe che dicevano `pv` 5 dove i voti ne contano 22, con le due Mv d'accordo
al centesimo. Riallineare i valori `expected` e' una decisione dell'operatore, non una conseguenza.

**DUE APERTI, diagnosticati e non toccati perche' fuori dalla richiesta.** I **due test di `press` che
scadono con l'orologio** (una partita fissata al 2026-09-18 e una presa «futura» chiesta il 22; il test
vicino passa un `today=` esplicito, questi due no) — verificati preesistenti mettendo da parte la modifica,
**cadono lo stesso**. E **`stop at round N` che non si ferma** in `positions`: fa `continue` e cammina fino
a fine calendario, il che e' giusto nella sostanza (un turno vuoto puo' essere un rinvio) e falso nella
parola, al prezzo di ~31 richieste a vuoto per lega.

## CHIUSURA — 16 settembre 2026: l'audit delle funzioni di surplus, e una colonna che leggeva il calcio giocato accanto a una che no

Richiesta dell'operatore: «adesso abbiamo formazione a breve termine e formazione stagionale tipo per
valutare titolarita' e minutaggio dei calciatori e conosciamo qualche partita per avere una traccia della
mediavoto e della fantamedia dei calciatori piu' veritiera … analizza le varie funzioni surplus e similari
per vedere se e' tutto ok per valutare correttamente la forza di tutti i calciatori».

**QUELLO CHE REGGE, VERIFICATO E NON DEDOTTO.** L'invariante `(fm − rimpiazzo) × pv × confidenza`
riproduce `engine_surplus` e `est_surplus` su **1.533 righe motore e 2.075 stimate, 0 fallimenti** (scarto
massimo 0,09, che e' l'arrotondamento delle tre colonne); nessuna riga senza surplus, nessuna presenza
fuori calendario. La scala a sei parole e' tornata **monotona** con le presenze attese su tutt'e due le
piattaforme (Serie A 0,804 · 0,749 · 0,693 · 0,653 · 0,524 · 0,291), quindi la miscela del 04/09 regge e i
«313 disaccordi su 358» di allora non ci sono piu'. E lo spostamento d'ambiente di quest'anno e' piccolo
rispetto al peso che R25 gli da': l'attacco sta +0,25 di fantavoto sul 2025-26 e i portieri −0,49, ma a
quattro giornate la miscela pesa il 9%, quindi nella sottrazione col rimpiazzo passa ~0,02.

**IL DIFETTO PIU' GROSSO ERA UN'ASIMMETRIA, e l'app lo stava gia' dicendo.** Il 07/09 le giornate viste
erano entrate nelle PRESENZE di ripiego e non nella FANTAMEDIA: **145 righe su 561** del foglio Serie A,
scarto mediano **0,488**, 32 oltre 1,0. La prova che non era una scelta e' che `swing.inSeason`
correggeva proprio quelle righe — quindi **sullo stesso schermo** lo SWING leggeva il calcio giocato e il
surplus, l'Overall, `pi_fm` e l'ordinamento della plancia no. La regola che ne esce: *quando l'app corregge
una colonna del foglio, quella correzione e' una segnalazione sul foglio* — o sale nel toolkit, o le due
letture della stessa riga continuano a non essere d'accordo. Qui e' salita, e la guardia dell'app si e'
**semplificata** (una domanda sulla piattaforma invece che sulla riga) invece di aggiungere un caso.

**E LA SECONDA CURA E' UN RAPPORTO SENZA CAMPIONE.** Il ramo `P` di `minutes.per_appearance` restituisce la
media misurata grezza — misurato e giusto finche' le presenze sono un CONTEGGIO, fuori dominio da quando
`presence.blend_seasons` le rende una frazione: 16 righe a **120 minuti esatti** (Adrian, 12,0 / 0,1) e
altrettante a 3-10, con il difetto che cresce al calare del campione (sotto mezza presenza **53 righe su
209** fuori da [70, 95], sopra le dieci **nessuna**). Retto su `START_MINUTES["P"]` con la taglia del
proprio campione: **+40,9% di MAE, 4 finestre su 4**, e zero movimento dove il campione c'e'.

**LA STESSA CURA SUI GIOCATORI DI MOVIMENTO E' MISURATA E RESPINTA** (0 su 4, monotona), ed e' la parte
istruttiva: il loro errore a campione corto e' piu' grande (20-25 minuti, sempre verso l'alto) ma sta nel
LIVELLO e non nel residuo. *Due popolazioni con lo stesso sintomo non hanno per forza la stessa causa, e la
cura si misura su ciascuna.*

**IL TERZO BUCO E' PRE-REGISTRATO E NON ADOTTATO** (R27, gate §7-septquinquagies): `evaluate` non importa
`presence`, quindi la board non tocca `engine_pv_pred`, e il motore e' compresso verso il centro — +1,0 …
+1,6% su euro (4 su 4) correggendo per gruppo oltre il livello. Non adottata per la regola aurea **e** per
un ostacolo strutturale che decide il lavoro: il gate non la raggiunge, perche' la board nasce guidando il
pannello Tk sopra un foglio gia' scritto. Il `claim` invece si raggiunge (`sweep.build_inputs`), ed e'
anche la forma con piu' probabilita' di fallire.

**Verifiche**: `backtest --verify` **22/22 reproduced** — letto per intero, che e' la regola pagata il
07/09 con un `tail` che mangiava il blocco dei controlli — 918 test toolkit, 992 app, build pulito.
**Controprova su entrambi i lati**: rimettendo il difetto cadono esattamente i test che descrivono la cura
(2 su `minutes`, 1 su `est`, 2 nell'app) e nessun altro. Un guardiano esistente e' caduto
(`test_the_keeper_is_not_rescaled`, che chiedeva `== 90.0`) ed e' stato riscritto **sull'intento** — il
tasso di partenza non tocca il portiere — invece che sul valore. Artefatti rigenerati offline: tre fogli a
revisione 71, `export`, `data:pull`; **0 minutaggi sopra i 95'** su 2.073 e 0 righe che si contraddicono.

**Aperti**: i quattro pacchetti del viaggio nel tempo sono a revisione 70 (la scatola dell'app lo dice, che
e' il comportamento giusto); e il rimpiazzo si misura sulle stagioni di INPUT mentre la fantamedia del
motore legge anche il campionato in corso — oggi vale ~0,02, da ri-guardare a stagione inoltrata.

## CHIUSURA — 16 settembre 2026 (notte): una giornata votata a META' si leggeva come completa, e le euro non erano indietro

Due domande dell'operatore nella stessa sessione, e la seconda ha scoperto il difetto piu' grosso:
«abbiamo lavorato molto sulla serie-a, c'e' qualcosa da allineare sulle leghe euro?» e, mezz'ora dopo,
«aggiorna anche il db, non vedo le ultime partite della serie a».

**IL DIFETTO.** Il resume di `ratings` chiedeva `SELECT DISTINCT matchday`: **una riga bastava a
dichiarare scaricata una giornata**. I voti arrivano partita per partita, quindi la lettura del sabato
archiviava mezza giornata e da quel momento nessuna corsa la riguardava piu' — e `update --daily`, che
e' quello che gira la notte, i voti non li rilegge affatto e lo dichiara nella propria ultima riga.
Misurato prima di toccare niente: **Serie A md4 a 8 club su 20** (134 righe di 339) e **euro md3 a 20
su 37**. `ratings.matchdays_done` (spec «Novita' v9.97»): completa = almeno tanti CLUB quanti la MEDIANA
delle ALTRE giornate della stessa (stagione, piattaforma), che e' la forma di `complete_squads` e per la
stessa ragione — 20 club su Serie A ma 33-37 su euro, quindi una soglia fissa sbaglierebbe da sola.
Costo misurato prima di adottarlo: **5 giornate su 145** si rileggono, e tre di quelle cinque sono
storiche euro che non porteranno mai una riga — prezzo dichiarato per non avere una costante.
Recuperato con `--refresh`: **+1354 righe su Serie A e +1832 su euro**, e l'effetto sui fogli e' grosso e
tutto nella stessa direzione, perche' un voto mancante si legge come «non ha giocato»: **245 righe di
953 su euro** e **140 di 561 su Serie A** cambiano `engine_pv_pred` (Kane 24,2 → 27,3 · Malen 24,0 →
26,5: tre giornate sotto per i titolari), 41 gradini di titolarita' su euro, il surplus su 117 righe di
Serie A. La fantamedia si muove su `default` (115 righe) e NON su euro: R25K40 legge le giornate viste
ed e' adottata li' sola, che e' la conferma strutturale della diagnosi. I campetti **zero** su 37 club:
la board sceglie sui minuti del livello per-partita, che era completo.

**LE EURO NON SONO INDIETRO, e la risposta e' misurata invece che dedotta** (`formazioni-tipo-v1.md`
§18.3-bis). Le tre prese pre-registrate del 12-13/09 contengono **12 club di Serie A e 11 dei quattro
campionati esteri** e nessuno le aveva spaccate: gli esteri leggono **80,0% degli uomini e 10 moduli su
11** in tutte e tre, contro il 74-79% e l'8 su 12 della Serie A. Le tre cure di settembre poggiano su
`club_match_lineups.formation`, coperta al **100% su tutti e cinque i campionati**, quindi nessuna e'
arrivata a una piattaforma sola; e il foglio euro prezza PIU' righe di quello Serie A (`engine_pv_pred`
100% contro 70%, `engine_surplus` 99% contro 52%), perche' li' il core prezza tutti (R0c). Quello che
euro NON ha e' un GIUDICE: `press_formations` porta 20 club tutti di Serie A, quindi `press --against
press` li' non esiste e le pre-registrazioni sono il solo giudice pulito che euro abbia mai avuto — un
argomento per continuare a prenderle (quella del 16/09 porta 27 club esteri su 47). Chiusa anche una
voce di todolist: **il buco del Como sul foglio euro non c'e' piu'** (37 club quotati, 37 sul foglio,
zero uomini mancanti), e si riaprira' il prossimo agosto su una promossa del perimetro euro.

**Verifica negativa che vale la pena avere in archivio**: delle 26 tabelle esportate, **10 non sono
copiate in `app/public/data`** — e nessuna delle dieci e' letta da una riga di `app/src`, quindi la
allowlist di `pull-bundle` e' giusta e non c'e' una nuova istanza del difetto dei campetti.

**E DUE ERRORI DI MISURA MIEI, tenuti a verbale perche' sono il modo in cui si sbaglia.** Ho dichiarato
«il processo e' appeso» leggendo `UserModeTime` via CIM e confrontandolo con `Get-Process.CPU`, che sono
due metriche diverse: campionata la STESSA due volte, cresceva di 15 secondi ogni 15 — *un processo si
dichiara appeso su due letture della stessa metrica, e il tempo di CPU si legge per INTERVALLI*. Era a un
passo dall'uccidere venticinque minuti di lavoro. E ho lanciato la suite con `--timeout=600`, che quel
...
passi un flag. E resta che `--daily` non rilegge i voti per scelta: chi vuole i voti dopo una giornata
deve lanciare `ratings` o l'`update` intero, il che e' documentato e non automatico.

## CHIUSURA — 13 settembre 2026 (notte): i nuovi acquisti titolari, una regola respinta dal gate e un marchio

Giornata nata da una frase dell'operatore su un nome — «nell'Atalanta Kessié è uno degli ultimi
acquisti, è un giocatore forte e di grande esperienza e dovrebbe essere titolare … riusciamo a
riconoscere i nuovi acquisti presi per essere titolari?» — e finita con **una regola del motore
pre-registrata e RESPINTA** e **un marchio di reporting spedito**. Le due cose vanno lette insieme: il
difetto che la regola descrive è vero, e il gate dice che non si può pagare con una regola del motore.

**IL CASO, E I DUE NUMERI.** Il foglio legge Kessié `riserva` e 17,3 presenze su 34 mentre
`probable_starter` lo dà a **1,00**; e non è isolato — **33 righe su 156** con probabile ≥0,85 sono
`riserva` o `panchina`. Lo sconto viene da R20, che legge 1 voto su 4 giornate; ma Kessié entra in rosa
il 31 agosto (`tm_appearances`, riga `absent`) e le prime due giornate **non le ha saltate: non c'era**.

**R26 — «LE GIORNATE VISTE SONO QUELLE IN CUI ERA IN ROSA» — NON PASSA** (gate §7-sexquinquagies,
pre-registrazione `3aa63d7`, esito `22bf9e3`). Alla K adottata media **+0,151%** contro un pavimento di
0,5%, 9 finestre su 13, peggiore −0,842%; nessun punto della griglia arriva al pavimento e il migliore è
K3, cioè il bordo. Sugli uomini che TOCCA vale +4,30% con 8 finestre su 10, ma la peggiore è −36,76% su
dodici uomini: direzione plausibile, numero non identificato. `backtest --verify` resta **22/22** e
`ADOPTED` è intatta; le sei chiavi restano dichiarate fra i candidati come le quattro di R24.

**E L'ATTESA PRE-REGISTRATA È SMENTITA**, che è la parte che vale: avevo scritto «effetto maggiore a
settembre che a febbraio, e se il gate legge il contrario la spiegazione non è questa regola». Il gate
legge il contrario. Domina la NUMEROSITÀ e non la proporzione — a settembre la regola tocca 1-9 uomini
di ~400, a febbraio 9-14 perché il mercato di gennaio cade dentro la finestra vista. Avevo ragionato su
quanto la correzione sposta UN uomo mentre il gate misura quanto sposta la MEDIA.

**LA DIAGNOSI CHE RESTA** (dettaglio e tabelle: `letture-app-v1.md` §42). Su 1.485 arrivi e 8 stagioni
il segnale più forte è la **Qt.I nel ruolo** (+0,431 grezzo, +0,249 a parità dei minuti esteri), che il
motore per regola non legge; le partenze da titolare valgono **+0,015** una volta noti i minuti, che è
R24 respinta incontrata da un'altra parte. Le due ipotesi dell'operatore sono confermate su 1.214
arrivi: il **subentro raddoppia** (0,511 contro 0,230 di chi c'era e non giocava) e un acquisto
importante arrivato tardi gioca poi **0,729**, quanto chi era titolare dal primo giorno (0,716).

**IL MARCHIO SPEDITO** (`2c474f4`): `PlayerFlag` `signing`, icona `user-add`, in `ui-flags` — quindi
ovunque si disegni un giocatore. `transfers_history` aggiunta a `pull-bundle` (**dodicesima istanza** di
«il dato c'era e non arrivava all'app»). 243 uomini su 532, mediana 12 per club, Inter 7 contro Venezia
17. Il limite è dichiarato: quella tabella è un DIFF FRA ROSE, quindi ha **quattro date distinte in
tutto** (il 1º luglio di ogni anno) e i 90 giorni dicono «questa sessione di mercato», non «tre mesi fa».

**IL DIFETTO CHE IL BANCO HA TROVATO ERA MIO**: Bakker leggeva NUOVO ACQUISTO sulla rosa dell'Atalanta
mentre l'aveva lasciata («dall'Atalanta a svincolato»), perché contavo qualunque movimento senza
guardare dove finisse. Ora vince l'ultimo movimento e la destinazione dev'essere un club vero. *Un
marchio che dice l'opposto del fatto è peggio di un marchio assente, e un conteggio non lo vede: erano 7
marcati contro 6 attesi.*

### Quello che ho pubblicato e ho dovuto ritirare

* «Kessié da 17,8 a 22,7» era **19,2**: la stima veniva dalle letture di rosa e il livello per-partita
  lo dà tesserato dal 31 agosto.
* La prima misura degli arrivi leggeva «0,9% arriva al 60%» e non voleva dire niente: misuravo l'esito
  su 34 giornate anche per chi ne aveva 19, cioè **commettevo io l'errore che stavo misurando**.
* La prima `rounds_in_squad` leggeva **187** arrivi tardivi su 532, Juan Jesus compreso. Due cause: le
  letture di rosa non sono complete per club a ogni data (fonte tolta), e serviva una guardia PER CLUB.
  Con la guardia: 15, tutti veri.

### Aperto, con il numero accanto

1. **`desc_starter_prob` non è in `export.SHEET_COLUMNS`** mentre è popolata su **459 righe di 562** nel
   CSV del toolkit: l'app non l'ha mai vista, e sarebbe la colonna che spiega le 33 righe con probabile
   alta marcate `riserva`. Offerta e non richiesta (come il 07/09). **Cosa NON fare**: metterla dentro
   una valutazione — la regola della maglia è stata respinta dalla misura.
2. **Lo screen «acquisto importante in rampa»** è misurato e non implementato: oggi accenderebbe 6
   righe su 532 (Kessié, Woltemade, Perri, Theate, Gonçalves P., Jones C.).
3. **R26 si rimisura quando le finestre in-season saranno più di quattordici**: le sei chiavi sono già
   fra i candidati, quindi la prossima corsa le legge senza che nessuno riscriva niente.

### Nota di sessione

Albero condiviso con un'altra sessione (`MarketTrend`, il pavimento dei ballottaggi): i suoi file sono
sempre restati fuori dai miei tre commit, verificati con `git diff | grep` sul vocabolario di ciascuna
metà (0 righe incrociate in entrambi i versi). La metà app è stata verificata **da sola in un worktree
su HEAD** con `npm ci` (20 s), perché l'albero condiviso non compilava per la loro metà: build pulito,
967 test, banco `e2e-clubs` verde, screenshot guardato. E `public/data` contato prima e dopo lo smontaggio
del worktree (151 = 151), perché `git worktree remove` cammina dentro le giunzioni.

## CHIUSURA — 13 settembre 2026 (sera): un veto che cancellava i rivali, e un pavimento che non sa distinguere

Commit `73a5268` (toolkit) e `517a093` (app). Foglio Serie A a `SHEET_REVISION` **65**, export verificato e
bundle nell'app (26,9 MB, 16/16 tabelle). **886 test toolkit, 970 app.**

**LA GIORNATA È NATA DA UNA DOMANDA DI RAGIONAMENTO E NON DA UNA RICHIESTA DI CODICE**: «come possiamo
identificare gli elementi fondamentali?» e «se non giocano ci deve essere un motivo». Le risposte utili sono
venute prima dal DATO che dal codice — Rabiot aveva giocato tutte e tre le giornate e cominciato l'ultima,
Pulisic era in panchina NON utilizzata da tre, cioè disponibile e non scelto — quindi i due nomi erano due
casi opposti e solo uno era un difetto.

**COSA È CAMBIATO**
1. **Il veto delle staffette sceglie chi GIOCA, non chi è un RIVALE** (`_from_slots`). Terza istanza di «le
   regole del primo percorso non si ereditano da sole». Da sola: undici **fermo** su 220 maglie, 6
   ballottaggi nuovi — ed è la cura che soddisfa la richiesta alla lettera.
2. **`RECENT_DECAY` = 0,5 più la distanza nel dato** (`SHEET_REVISION` 65, quarto campo di
   `desc_recent_slots`). Un foglio sotto la 65 legge zero per tutti, cioè degrada al conteggio di prima.
3. **`recent_state`** nel file (`boards.json`) e nell'ordine dei rivali a parità di gradino.
4. **App: `KEY_MAN_VALUE` = 2**, esenzione al pavimento dei ballottaggi per chi il mercato vero prezza come
   un uomo del club. Nessuna acquisizione: `market_value_history` viaggia dal bundle da agosto (233 KB) e
   `MarketValues.trend()` è il lettore unico.

**ATTRIBUZIONE, una variabile per volta** (Milan): base → Rabiot assente; solo veto → ballottaggio di
Loftus-Cheek; solo peso → titolare. **Sui 20 club**: board di STAGIONE identica (0 moduli, 0 uomini, 0
ballottaggi), **nessuna colonna del foglio cambia** (`engine_*` compresi, misurato e non dedotto), board
breve 20 uomini dentro e 20 fuori, 102 maglie con ballottaggi diversi.

**COSA RESTA APERTO**
1. **EuroLeghe e Leghe Mantra nel bundle sono ancora del 12/09 sera.** Ho rigenerato il solo foglio Serie A:
   la corsa con il refresh di rete si è APPESA (2,2 secondi di CPU in 25 minuti) e l'ho abbandonata come
   prescrive la regola di casa. Quella fase va capita a parte — e i probabili di oggi non sono stati riletti
   per nessuno dei tre fogli.
2. **I marchi `recent_state` non si vedono a schermo.** Il fatto viaggia nel file e la card lo può leggere,
   ma nella lista dei ballottaggi non c'è un'icona che distingua «subentra ogni domenica» da «fermo da tre
   giornate». Tocca `ui-flags`, dove un'altra sessione stava lavorando.
3. **Non c'è un giudice esterno per questo blocco**, ed è dichiarato: la board breve prevede la PROSSIMA
   partita e la 4ª giornata era in corso. La decisione poggia sulla misura fuori campione (49.810
   slot-partita, tre ordini di grandezza più di una giornata) e sull'attribuzione a una variabile. La
   pre-registrazione del §12.9.5 resta il canale per il giudizio sul fatto.

**TRE ERRORI DI MISURA MIEI, a verbale perché sono il modo in cui si sbaglia**
- **Un'asserzione che non può fallire**: uno dei tre test nuovi restava verde col difetto rimesso, perché i
  due uomini avevano lo stesso claim e l'ordinamento stabile teneva l'ordine della lista. Riscritto.
- **Ho dato per appesa una corsa che era solo lenta**: la seconda (9,3 s di CPU in 10 minuti) l'ho
  abbandonata, e la terza — identica — ha finito in 12 minuti girando a piena CPU. Il discriminante è il
  tempo di CPU e va letto due volte prima di decidere.
- **Un `timeout 900` ha ucciso una corsa lasciando un log VUOTO**, che si legge come un fallimento invece
  che come un troncamento.
...
**728 test** del toolkit · tre fogli a `SHEET_REVISION` **50** coi campetti (20/20 · 20/20 · 37/37) ·
bundle esportato e verificato (457.549 righe, 25 tabelle) e portato nell'app (13,8 MB) ·
`club_elo` ferma al 14/01/2026 e lo dice da se' · i quattro pacchetti del viaggio nel tempo restano alla
revisione **48** e lo dichiarano nel proprio manifest. Un solo albero, tutto di questa sessione.

### Quello che e' entrato, e nessuno dei due tocca il motore

**1. L'ANCORA DEL NUOVO ARRIVATO** (`est.newcomer_anchor`, `default`, D/C/A — gate §7-unquinquagies).
Misurato leave-one-window-out su 713 uomini che il core non prezza: l'ancora di ruolo e' **troppo alta**
per chi arriva (A −0,26 · C −0,06 · D +0,06) e l'Elo del club alla data d'asta vale **0,17 di fantamedia
per 100 punti** (A; 0,11 C, 0,12 D). Contro l'ancora di ruolo **A +16,8% (8/10) · C +10,4% (9/10) · D
+10,0% (9/10)**; l'ancora di CLUB che spedivamo valeva +0,7% sugli attaccanti — la media di 3-5 attaccanti
dell'anno prima e' rumore col nome del club, e una promossa non ne ha nessuna mentre ogni club ha un Elo.
Non e' la famiglia forza-club respinta quattro volte (R5/R5b/R16/R16b): quelle erano misurate su uomini
CON una `fm_prev` in cui il club sta gia' dentro. `older` resta sull'ancora di club, dove l'Elo misura
peggio (−5,3%). Le due finestre negative sugli attaccanti sono le due stagioni COVID, dette e non
spiegate via.

**2. LE CONFIDENZE DELLA CASCATA SONO CALIBRATE** (gate §7-duoquinquagies), su una domanda sua che vale
come metodo: «possiamo misurare gli esiti su una stagione vecchia e vedere qual e' la soluzione che piu'
rispecchia la realta'?» — e la costante era una che aveva dichiarato lui il 05/08. La confidenza
moltiplica il surplus, quindi si calibra come RAPPORTO fra reso e predetto grezzo, su dieci finestre e
tutti i quotati, chi non ha mai giocato contato come lo zero che ha reso:

| gradino | in vigore | calibrato | spedito |
|---|---|---|---|
| `core` | 1,00 | **0,94** | 1,00 (deve riprodurre `engine_*`) |
| `older` | 0,85 | 0,93 | **0,90** |
| `shrunk` | 0,53-0,97 | 0,77 | invariato |
| `anchor` | **0,50** | **0,69** (0,73 relativo al core) | **0,75** |

Il deliverable e' PIATTO fra 0,73 e 0,80 (+1,66 · +1,69 · +1,86% contro lo 0,50) e non monotono, quindi
si adotta il valore misurato e non il picco del banco. Ogni valore batte lo 0,50, **nessuna penale**
compresa (+0,98%) — l'ottimo e' interno e lo 0,50 era il punto peggiore della griglia.

**E LA MIA OBIEZIONE ERA MAL POSTA, che e' la parte da ricordare.** Avevo presentato la scelta dicendo
che alzando l'ancora la scala si sarebbe rovesciata per chi ha pochi voti. Calibrato per banda, `shrunk`
legge **1-4 voti 0,45** · 5-9 0,92 · 10-14 0,89: tre voti meritano MENO di niente, perche' pochi voti
sono **evidenza CONTRARIA** (un uomo che il suo allenatore non ha schierato) mentre «niente» puo' essere
un titolare che arriva dall'estero. Niente da comprimere; quello che resta sbagliato e' la FORMA (una
soglia fra 4 e 5 voti, non una pendenza) ed e' in todolist.

### Le sue due segnalazioni, con esiti opposti

- **«Abbiamo i suoi voti sintetici, che e' gia' qualcosa e non e' niente!»** — VERO: Ramos ha 30 partite
  di Ligue 1 col voto sintetico e un FM-equivalente di 6,394, e la nota della riga diceva «nothing
  measured anywhere» a **276 quotati**. Ma il NUMERO non si muove: sopra shift + Elo il calcio estero non
  aggiunge (binario −0,2 · quota estera −0,2 · FM-equivalente −0,8, e su C e D tutte peggiorano), perche'
  chi ha 15+ partite su file rende **+0,48 di fantamedia in piu'** in quanto va in club forti, ed e'
  l'Elo a leggerlo. La cura e' la NOTA (todolist), non la valutazione. Gate §7-quaterquinquagies.
- **«Qt.I e FVM non misurano la capacita' di fare bonus»** — ACCOLTA, e la prova era nella tabella che
  gli avevo mostrato un'ora prima: la differenza fra Qt.I sopra e sotto la mediana era quasi tutta «96 su
  262 non giocano mai», cioe' la quotazione predice le PRESENZE. La sua alternativa (la fetta di
  produzione della squadra per minuto) e' stata costruita: **il prodotto e' respinto** (−10,8% A · −10,4%
  C · −14,4% D), **la fetta non viaggia con l'uomo** (r +0,059 al cambio club sugli attaccanti: e' una
  proprieta' del POSTO), e sopravvive il contesto come termine additivo sui soli attaccanti che cambiano
  club (+5,84%, 7/10, n=84) — che e' cio' che l'Elo gia' porta, perche' `CLUB_MV_SHARE` dice che per un
  attaccante il livello di un club e' due terzi bonus. Gate §7-treduoquinquagies.

### Tre difetti trovati, e due erano dello strumento

- **LO ZERO DI UN PARAMETRO CENTRATO E' PARTE DEL PARAMETRO** (gate §7-unquinquagies bis). Trovato sul
  PRIMO foglio scritto dopo l'adozione, e solo perche' e' stato verificato che la riga di Ramos
  riproducesse il numero calcolato a mano: **6,859 contro 6,785**. `estimation_layer` mediava l'Elo su
  TUTTE le osservazioni, e `snapshot` costruisce la popolazione sulle rose osservate (786 uomini contro
  601 righe), quindi lo zero scendeva da **1690 a ~1647** e ogni nuovo arrivato prendeva +0,07 gratis. La
  pendenza era giusta; sbagliato era il punto da cui si misura la distanza.
- **Il club preso da `rosters` invece che da `listone_quotes`** nella misura della fetta — la falla del
  03/09 — col sintomo che Barcellona, Chelsea e Tottenham comparivano in una misura di Serie A.
- **Un denominatore che passa per zero**: la produzione NETTA di un club puo' annullarsi (Empoli 2023-24:
  0,0006 per 90), quindi la fetta andava **da −320 a +1089** e Pearson leggeva +0,012 su dodici celle.
  Uno zero uniforme non si crede: si guarda la forma del dato.

### Cosa resta aperto, in ordine di leva

1. **Il MOTORE sovrastima il surplus del 6%** (`core` calibra 0,94 su 2669 righe, pendenza 0,93). Non
   applicato — `est_*` deve riprodurre `engine_*` — quindi e' una voce del GATE, con la domanda spinosa
   che un fattore <1 non cambia nessun ordinamento e cambia ogni numero a schermo.
2. **I quattro pacchetti del viaggio nel tempo sono alla revisione 48**: `timepack --all --refresh`,
   ~20-25 minuti di DB. Niente e' rotto e niente e' silenzioso — la revisione viaggia nel pacchetto.
3. **`club_elo` ferma al 14/01/2026**, e ora pesa di piu' perche' l'ancora la legge. E' un'acquisizione.
4. **La FORMA dello `shrunk`** e' una soglia fra 4 e 5 voti e non una pendenza (1018 righe, misurabile).
5. **La nota della riga `anchor`** dice il falso a 276 quotati.

## CHIUSURA — 7 settembre 2026: R25 adottata, e la coppia fm/mv miscela insieme

**Stato misurato alla chiusura**, non dedotto: `backtest --verify` **22/22** dopo l'adozione ·
**727 test** del toolkit e **809** dell'app · `ng build` pulito ·
tre fogli a `SHEET_REVISION` **48** con i campetti · quattro pacchetti del viaggio nel tempo rifatti
(erano tutti sotto revisione) · bundle esportato e verificato (457.193 righe, 25 tabelle) e portato
nell'app (13,8 MB). Un solo albero, tutto di questa sessione.

### Quello che e' entrato nel MOTORE, ed e' la prima adozione da giorni

**R25K40 su `default`** (`evaluate.ADOPTED`, gate §7-quinquagies bis). La griglia allargata era
pre-registrata dal 6 settembre e non ancora corsa; corsa come scritta, l'ottimo e' **INTERNO** — la
media scende oltre il 40 su tutt'e due i giochi — quindi la ragione procedurale che teneva la regola
fuori (un parametro al BORDO non si adotta) e' caduta. Il punto adottato e' quello con la media piu'
alta fra quelli che reggono su tutt'e due: robusto 11/12 su classic (peggiore −0,6%), **STRICT 12/12**
su mantra. K25 rende di piu' in media (+5,6%) e non regge su classic; il K120 che passa strict e' il
termine che converge all'inerzia, non un K migliore — «migliora ovunque di poco» e' la firma di un
termine che si spegne.

**E la conseguenza che vale piu' dell'adozione: la coppia fm/mv miscela INSIEME o non miscela.** Con
R25 nel motore, `est_mv` che leggeva solo la stagione scorsa avrebbe scaricato TUTTA la novita'
in-season sul tasso bonus derivato `fm − mv` — esattamente la famiglia del difetto v9.59, una meta' che
assorbe quello che e' di tutt'e due. Misurata fuori campione prima di spedirla (13 finestre in-season):
**+4,7% di MAE sulla MV del resto di stagione a K=40, 13 finestre su 13, peggiore +0,55%**. La `K` la
legge da `evaluate.ADOPTED`, quindi su euro sta spenta da se' e una futura ri-adozione con un altro K
arriva da sola. `Observation.mv_seen` era cablato dal giorno di R25 e nessuno lo leggeva.

### Quattro richieste dell'operatore sull'app, e una sua ipotesi respinta dalla misura

- **Lo SWING passa a PUNTI SOPRA IL 6 A GIORNATA** («se un calciatore ha una fantamedia di 10 allora il
  suo swing dovrebbe essere 4»), col suo esempio come test. Ribasatura esatta su colonne dello stesso
  foglio, surplus sempre letto e mai ricalcolato. Prezzo dichiarato: non ordina piu' identico al
  surplus. Poi **due decimali** sulla pastiglia della Strategia, perche' a giornata i valori vivono
  sotto il 2 e a un decimale mezza lista leggeva uguale.
- **Il «6» del portiere e' un 5** (`swing.KEEPER_BASE`), e l'ha trovato lui: «quelli che giocano
  prendono i malus dei gol subiti, quindi nei primi posti ci sono tutti portieri che non giocano». Causa
  di SCALA e non di formula — la FMa dei titolari veri sta fra 4,91 e 5,24, tutti sotto il 6 — e lo zero
  giusto era gia' misurato: il fielded dei portieri legge **5,01/5,03** per due strade indipendenti. Un
  test porta il caso E la controprova.
- **Due OPZIONI DI LEGA dichiarate**, accanto al modificatore di difesa: `rFactor` (dove la lega non lo
  paga, il termine di costanza non esiste — il suo `k` e' indicizzato su quella scala) e `cleanSheet`
  (il +1 a porta inviolata, che **non e' nel fantavoto**: 1.218 portieri su 1.222 a porta inviolata
  leggono `voto + bonus` senza premio). Il +1 entra **al DIFFERENZIALE** contro la media del
  campionato, perche' anche il portiere che giocherebbe al posto suo le porte inviolate le incassa —
  la regola delle squalifiche, e cio' che paga e' il CALENDARIO. Terzo lettore del canale delle
  coppie-portieri (`cleanSheetOutlook` / `cleanSheetBaseline`), non una seconda lettura.
- **La sua ipotesi sui portieri e' stata MISURATA E RESPINTA**, e il controesempio e' la stessa coppia
  un anno prima. «Meret due partite da 90' e Milinkovic-Savic zero dovrebbe bastare»: sulle coppie in
  cui dopo due giornate il leader visto non e' quello del prior, la maglia resta a chi l'ha presa **10
  volte su 18** — una monetina — e nel **2025-26 Napoli** Meret prese le prime due e Milinkovic-Savic
  torno' a giocarne 27 contro 9. Verificato anche il sospetto strutturale («un club schiera UN portiere,
  quindi la' due giornate valgono piu'»): la curva del peso ottimale per i portieri ha la **stessa
  forma** dei ruoli di movimento, quindi nessun K speciale. Detto per intero: su questo caso il modello
  sara' piu' lento dell'occhio per qualche turno, e dirlo e' meglio che tarare una soglia su un nome.

### Aperto, con il numero accanto

1. **I probabili di oggi come colonna di REPORTING accanto allo SWING.** Sono l'unico segnale che
   distingue i due portieri ADESSO (Meret 1,00 contro 0,05) e il foglio non li porta piu'
   (`desc_starter_prob` vuota su questo bundle). Offerto e non richiesto: si fa se lo chiede. **Cosa
   NON fare**: metterli dentro una valutazione — la regola della maglia e' stata respinta dalla misura
   e un input che la reintroducesse per un'altra porta sarebbe la stessa cosa senza il gate.
2. **La TAGLIA dei due modificatori non e' dichiarata** (solo la loro presenza): `STEADY_SHARE` = 2/11
   sono i due punti dell'R-Factor della SUA lega spalmati sull'undici, e una lega con un R-Factor di
   scala diversa vuole un altro valore. La scala vive solo in `bench/auction/rules.py`.
3. **Il tasso di conversione in gol resta nel file e non a schermo** (0,159 gol per fantapunto): serve
   al tooltip e ai test, e collega la scala nuova a quella vecchia. Chi lo tocca rimisuri le due
   costanti della giornata-rosa, che un test difende.
...
scritture · **724 test** del toolkit passati (uno saltato: vuole un display) e **757**
dell'app · build dell'app pulito · fogli a
`SHEET_REVISION` **47** con i campetti disegnati (20/20 · 20/20 · 37/37) · bundle esportato e verificato
(455.664 righe, 25 tabelle) e portato nell'app (13,8 MB). `engine_*` non si è mosso di un decimale in
tutta la giornata.

**Le quattro decisioni dell'operatore, e come sono state prese**: una per volta, ognuna con il numero
davanti invece che al buio.

| | decisione | dove sta scritta |
|---|---|---|
| la GRAFIA degli slug | **pre-registra e fai girare il gate** → eseguita per intero, adottata | gate §7-quattuorquadragies, spec «v9.79» |
| le tre preferenze del 15/08 | **restano spente**: ognuna ha già una casa migliore | `letture-app-v1.md` §7 |
| lo ZERO realizzato sul foglio | **no**: ±0,35 e cambia segno per ruolo, è un fatto sulla LEGA | todolist |
| la data «terza giornata» | **no**: la ragione è la convenzione, non l'1% di errore | todolist |

**Cosa resta aperto, in ordine di quanto costa.**
* **I quattro pacchetti del viaggio nel tempo sono a revisione 46 contro 47** e l'app lo DICHIARA, quindi
  non è un difetto nascosto: rifarli è `timepack --all --refresh`, ~35 minuti e il lock di scrittura.
  Muoverebbero le stesse colonne che si sono mosse sui fogli (`est_*`, `desc_arrival_tier`).
* **Il deploy del sito è dell'operatore** (`app/scripts/deploy-pages.mjs`, dalla sua macchina): il bundle
  locale è nuovo, quello pubblicato no.
...
## CHIUSURA — 13 settembre 2026 (notte): i quindici rilievi spediti, e il completamento non era la mappatura

Foglio a `SHEET_REVISION` **64**, tre leghe rigenerate (562 · 562 · 956 righe), export e bundle nell'app
(26,8 MB). **880 test toolkit** (uno skip), **961 app**, build pulito, `backtest --verify` **22/22** letto
nel blocco dei controlli e non dalla coda. Dettaglio, tabelle e numeri di ogni rilievo:
`formazioni-tipo-v1.md` §12.9; le lezioni durature in `CLAUDE.md`.

**COSA HA CHIESTO L'OPERATORE**: risolvere i quindici rilievi latenti della code-review di ieri notte
(§12.8), «chiudere prima il completamento e poi spedire il blocco». Fatto in quest'ordine, ed e' la
ragione per cui il blocco e' uscito intero.

**IL COMPLETAMENTO NON C'ENTRAVA CON LA MAPPATURA**, che era la premessa da cui ieri notte era rimasto
aperto. La regressione misurata non era 16/20 ma **17/20**, e i tre club scoperti avevano tutti un posto di
CENTROCAMPO vuoto per DUE cause distinte, trovate fotografando il singolo posto invece dell'aggregato:
(a) il veto delle staffette era applicato PRIMA del filtro di posizione, quindi all'Atalanta toglieva
l'unico centrocampista libero e lasciava un difensore e due ali che `can_replace` rifiutava - il ripiego
«se il veto svuota tutto si rinuncia» non scattava perche' il serbatoio non era vuoto, era vuoto di gente
utile; (b) un posto che NESSUNO ha mai occupato non aveva un serbatoio, e non e' un difetto dei dati:
una riga di due uomini letta dentro una di tre cade sui suoi estremi, quindi il 6 di un 4-3-3 non riceve
un voto da nessun 4-2-3-1. Ora quel posto va prima alla sua RIGA (alla Fiorentina e' Fagioli, secondo sul
5 e sul 7 e senza questo ramo non disegnato affatto) e solo dopo a chi la finestra non ha visto giocare.
**20/20 undici completi su Serie A, 36/37 su euro** - il Monaco resta a dieci perche' quel posto e' di un
uomo che il listone EuroLeghe non quota, e un posto vuoto si vede mentre un uomo inventato no.

**L'A/B SU UNA VARIABILE HA RICHIESTO DI RICOSTRUIRE LA BASE**: il foglio di ieri era stato sovrascritto
dalla rigenerazione, quindi la base e' stata rifatta in un **worktree su HEAD** con lo stesso DB e lo stesso
`--no-refresh`. Risultato: board breve 220 -> 220 uomini, **211 in comune su 220** (7 club muovono un uomo),
**0 moduli diversi**; board di STAGIONE identica e giudice stampa identico (**MATCH 11 · ALT 3 · DIFF 6 ·
150/220**); ballottaggi indisponibili elencati 142 -> 115, che e' `SIDELINED_DUELS` che lavora.

**E L'A/B HA TROVATO UN SEDICESIMO RILIEVO CHE NESSUNA RILETTURA POTEVA VEDERE.** Il Sassuolo usciva con
Berardi dentro, ma rifacendo il foglio in una cartella NUOVA Berardi spariva anche col codice nuovo:
`_load_player_rulings` chiede `manifest.target_season`, e `snapshot` scrive il manifest DOPO i campetti -
quindi **su ogni cartella nuova le dritte dell'operatore uscivano vuote**, e su una riusata valevano quelle
della corsa precedente. Terza istanza dopo il calendario di `platform_target` (v9.75) e curata allo stesso
modo: la stagione la passa chi la sa, iniettata solo dove manca.

**DOVE SIAMO, in una riga per pezzo:**
- la mappatura fra moduli e' PER RIGA e sposta **512 combinazioni su 1331** (era l'identita' su tutte);
  il test la CONTA invece di asserire un intervallo, e verifica su ogni coppia di moduli del regolamento
  che la linea sia conservata;
- `desc_recent_slots` porta `partita:modulo:slot`, e la partita e' cio' che permette alla staffetta di
  distinguere un fatto («hanno cominciato insieme») da un'inferenza sui minuti;
- la clamp del calendario vive in **`features.target_matchdays`**, dentro `prepare`: prima era in un
  consumatore, quindi il pannello d'asta e `estimates` leggevano il numero crudo;
- `SHEET_REVISION` 64 dichiara anche il fix del calendario NEGATIVO della revisione precedente, che muove
  `engine_*` su 393 righe e non era stato dichiarato.

**LE DUE PRE-REGISTRAZIONI.** Quella del 12/09 descrive la revisione 63 e cinque dei suoi 23 club cambiano
un uomo, quindi ne e' stata presa una seconda alla 64 (`preregistrazione-board-breve-2026-09-13.md`), alle
00:45 e con **nessuna** delle 23 partite ancora cominciata. Si scorano tutte e due sulle stesse distinte -
e' il primo confronto appaiato fra due versioni di questa board - col limite scritto accanto: nove nomi su
253 non promuovono ne' bocciano una mappatura.

**COSA RESTA APERTO**
1. Il Monaco a dieci uomini: corretto, e va ricontrollato il giorno che il listone euro quota quel posto.
2. La board breve continua a non avere un giudice in `press.py`: le due prese di questi due giorni sono a
   mano, e §12.7 chiede che diventi il quarto riferimento del modulo.
3. Le tre voci del §10.10 (N. Gonzalez al centro, il ballottaggio con Woltemade, Cisse x Moreira) restano
   da rileggere sui fogli alla 64.

**UN ERRORE DI STRUMENTO MIO, a verbale**: il worktree della base e' stato lanciato con la sola
`EUROLEGHE_DATA_DIR` e ha aperto un database VUOTO, creandone uno da 376 KB accanto a quello vero - cioe'
esattamente il decoy che la review del 06/09 aveva tolto. `CLAUDE.md` lo dice gia' («serve anche
`EUROLEGHE_DB_PATH`») e l'ho letto dopo averlo sbagliato. Il file e' stato verificato vuoto (78 tabelle,
zero rose) e cancellato; la base e' stata poi ricostruita col percorso giusto.
