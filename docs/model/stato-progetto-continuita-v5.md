...
**Aggiornato: 13 settembre 2026 (notte) — I NUOVI ACQUISTI TITOLARI: UNA REGOLA RESPINTA DAL GATE E UN MARCHIO SPEDITO.** Dalla domanda dell'operatore su Kessié. **R26** («le giornate viste sono quelle in cui era IN ROSA») e' pre-registrata e **NON PASSA**: +0,151% contro un pavimento di 0,5%, 9 finestre su 13, e l'attesa che avevo scritto prima della corsa e' SMENTITA (i guadagni stanno a febbraio e non a settembre, perche' domina la numerosita' e non la proporzione). `--verify` 22/22, `ADOPTED` intatta. Quello che si spedisce e' un MARCHIO: `signing` su ogni riga di giocatore, con `transfers_history` che era nel bundle e non arrivava all'app (dodicesima istanza). Il difetto piu' grosso l'ha trovato il banco ed era mio - Bakker leggeva «nuovo acquisto» sulla rosa che aveva LASCIATO. Diagnosi completa in `letture-app-v1.md` §42.
